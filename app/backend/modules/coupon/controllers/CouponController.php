...
//            });
    ...
    {
        if (!$kwd = trim(request()->kwd)) {
            return $this->errorJson('请输入品牌ID或品牌名称');
        }
        $list = Brand::uniacid()->select('id', 'name', 'logo')->where(function ($query) use ($kwd) {
            $query->where('name', 'like', "%{$kwd}%");
            if (intval($kwd) == $kwd) {
                $query->orWhere('id', $kwd);
            }
        })->get();

        $list->each(function ($v) {
            $v->logo = yz_tomedia($v->logo);
        });
...
//                $coupon->goods_names = Goods::getGoodNameByGoodIds($coupon->goods_ids); //因为商品名称可能修改,所以必须以商品表为准 //todo category_names和goods_names是不可靠的, 考虑删除这2个字段
        ...
        if ($couponRequest) {
            ...
            $coupon->brand_ids = array_values(array_filter(array_unique($couponRequest['brand_ids']))); //去重,去空值
            $coupon->is_all_brand = $couponRequest['is_all_brand'] ? 1 : 0;
            $coupon->brand_id = intval($couponRequest['brand_id']);
            //消费积分兑换优惠券
...
//            });
            if ($validator->fails()) {
                ...
                if ($coupon->save()) {
                    ...
                    if ($coupon['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE && ($coupon['time_days'] !== 0)) {
                        $member_coupon = MemberCoupon::where('coupon_id', $coupon['id'])->where('is_expired', 1)->get();
                        foreach ($member_coupon as $memberCoupon) {
                            if (time() < Carbon::createFromTimestamp(strtotime($memberCoupon['get_time']) + ($coupon['time_days']+$coupon['wait_days']) * 3600 * 24)->endOfDay()->timestamp) {
                                $memberCoupon->is_expired = 0;
                                $memberCoupon->save();
                            }
                        }
        ...
        $coupon['brand_list'] = [];
        if ($coupon->brand_ids) {
            $coupon['brand_list'] = Brand::uniacid()->select('id', 'name')->whereIn('id', $coupon->brand_ids)->get();
        }
    ...
    public function export()
    {
        ...
        if (empty($couponId) && empty($couponName) && ($getFrom == null) && empty($nickname) && ($searchSearchSwitch == 0) && empty($member_keyword)) {
    ...
    public function addCouponCount(Request $request)
    {
        ...
        if ($total <= 0) {
            return $this->errorJson('请输入正确的数量');
        }
