...
//        $values = $this->getRow();
        ...
        $values = array_filter($values, function ($val) {
            return !is_null($val[0]) || !is_null($val[1]);
        });
        ...
        if ($this->genre() == 2) { //1 是会员ID， 2 是手机号
            ...
            //不是本平台的会员直接充值失败 #32639
            $existUid = Member::uniacid()->whereIn('uid', array_column($values, 0))->pluck('uid')->toArray();
            foreach ($values as $key => $val) {
                if (in_array($val[0], $existUid)) continue;
                $data[] = [
                    'uniacid' => \YunShop::app()->uniacid,
                    'amount' => trim($val[1]),
                    'member_id' => trim($val[0]),
                    'recharge_id' => &$recordsId,
                    'remark' => '充值失败:该用户不是本平台会员',
                    'status' => 0,
                    'created_at' => time(),
                    'updated_at' => time()
                ];
                $this->handleTotal += 1;
                $this->handAmount += trim($val[1]);
                $this->failureTotal += 1;
                unset($values[$key]);
            }
...
//        $highestColumn = $sheet->getHighestColumn();
     ...
     */
    private function batchRechargeIntegral($memberId, $rechargeValue)
    {
        return (new IntegralChangeServer())->universal([
            'source' => \Yunshop\Integral\Common\Services\ConstService::SOURCE_EXCEL_RECHARGE,
            'type' => $rechargeValue > 0 ? ConstService::TYPE_INCOME : ConstService::TYPE_EXPENDITURE,
            'uid' => $memberId,
            'uniacid' => \YunShop::app()->uniacid,
            'change_value' => abs($rechargeValue),
            'operator' => ConstService::OPERATOR_SHOP,
