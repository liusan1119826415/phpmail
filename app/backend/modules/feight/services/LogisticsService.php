...
namespace app\backend\modules\feight\services;
...
class LogisticsService
{
    ...
    public function getList($search)
    {
        ...
        $query = VueOrder::where('parent_id',0)->with(['supplierPrice'])->where('order_type',1)/*->where('choose_logistics',1)*/->whereIn('status',[1,2])->whereIn('orderStatus',[1,2,3]);
        ...
        $list->transform(function ($order) {
            ...
            if($order->supplierPrice){
                $order->min_freight_price = $order->supplierPrice->where('bidding_status', '>=', 2)->min('freight_price');
                $order->logistics_num = $order->supplierPrice->where('bidding_status', '>=', 2)->count();
                $hasBiddingStatus2OrMore = $order->supplierPrice->where('bidding_status', '>=', 2)->isNotEmpty();
                $order->bidding_status_flag = $hasBiddingStatus2OrMore ? 1 : 0;
            }else{
                $order->bidding_status_flag = 0;
            }
