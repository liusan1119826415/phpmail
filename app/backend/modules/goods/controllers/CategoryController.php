...
//        dd($goods_ids->toArray(),$update_ids->toArray(),$insert_ids,$update_ids);
        ...
        if ($update_ids->isNotEmpty()) {
            GoodsCategory::whereIn('goods_id', $update_ids)->delete();
            foreach ($update_ids as $goodsId) {
    ...
    public function addCategory()
    {
        ...
        return $this->successJson('ok', [
            ...
            'level' => $level,
            'label_group' => [],
            'component_list' => ComponentService::getComponentList()
        ]);
//        return view('goods.category.info', [
//            'item' => $categoryModel,
//            'parent' => $parent,
...
//        ])->render();
    ...
    public function batchDeleteCategory()
    {
        ...
        foreach ($ids as $id) {
            Category::uniacid()->where('id',$id)->delete();
        }
        return $this->successJson('删除分类成功');
    }
