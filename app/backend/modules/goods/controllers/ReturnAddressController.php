...
//        ])->render();
    ...
    public function ajaxAllAddress()
    {
        ...
        $plugins_id= request()->input('plugins_id');
        if (isset($plugins_id) && is_numeric($plugins_id)) {
        ...
        if ($orderId) {
            switch ($plugins_id) {
                case 31:
                case 32:
                    $merchantId = StoreOrder::select('store_id')->where('order_id', $orderId)->value('store_id');
                    $where['store_id'] = $merchantId;
                    break;
                case 33:
                case 36:
                    $merchantId = HotelOrder::select('hotel_id')->where('order_id', $orderId)->value('hotel_id');
                    $where['hotel_id'] = $merchantId;
                    break;
                case 92:
                    $merchantId = SupplierOrder::select('supplier_id')->where('order_id', $orderId)->value('supplier_id');
                    $where['supplier_id'] = $merchantId;
                    break;
                default:
            }
        }
