...
class Category extends \app\common\models\Category
{
    ...
    {
        $models = CategoryModel::uniacid()->select(['id', 'name', 'enabled', 'parent_id'])
            ->where(['plugin_id' => $pluginId ,'deleted_at' => null])->get()->toArray();
    ...
    public static function getHowCategoryGroupArray($cateIds)
    {
        $models = CategoryModel::uniacid()->select(['id', 'name', 'enabled', 'parent_id'])
            ->where(['plugin_id' => 0 ,'deleted_at' => null])
            ->whereIn('id',$cateIds)
            ->orderBy('display_order', 'desc')
