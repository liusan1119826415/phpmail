...
//        }
    ...
    public function saveOption($spec_items, $stock, $supplier_id)
    {
        ...
        // 第一遍遍历：找出每个规格组中价格最低的选项（只考虑modelType == 0）
        foreach ($optionPost as $oKey =>  $option) {
            ...
            foreach ($optiondata as $k => $item){
                ...
                if ($modelType == 0) {
                    ...
                    $currentPrice = floatVal($option['product_price']);

                    // 记录每个规格组的最低价格（只考虑modelType == 0）
                    if (!isset($lowestPriceForSpec[$currentSpecItemId]) ||
                        $currentPrice < $lowestPriceForSpec[$currentSpecItemId]['price']) {
                        $lowestPriceForSpec[$currentSpecItemId] = [
                            'price' => $currentPrice,
                            'option_key' => $oKey,
                            'item_key' => $k,
                            'spec_item_ids' => $spec_item_ids,
                            'specs_id_array' => $specs_id_array
                        ];
                    }
                }
            }
           ...
           // $productType = $option['productType'];
            foreach ($optiondata as $k => $item) {
                ...
                if (request()->hasFile($bufferKey)) {
                    ...
                    // 上传到 OSS
                    $d3ModelUrl_weld = $this->uploadOssTwo($fullPath, $fileName);

                }


                $thumb3dModelKeyUrl = "widgets.option.option.{$oKey}.modelTypes.{$k}.option.thumb3dModelUrl";

                if (request()->hasFile($thumb3dModelKeyUrl)) {

                    $file = request()->file($thumb3dModelKeyUrl);
                    ...
                    app(Dispatcher::class)->dispatch($job);*/
                    // 上传到 OSS
                    $thumb3dModelUrl = $this->uploadOssTwo($fullPath, $fileName);
                ...
                if (request()->hasFile($d3ModelUrlKey)) {
                    ...
                    app(Dispatcher::class)->dispatch($job);*/
                    $d3ModelUrl_ori = $this->uploadOssTwo($fullPath, $fileName);
                ...
                // 判断当前选项是否是该规格组中价格最低的（只对modelType == 0的选项进行比较）
                $isLowestPrice = false;
                // 只有modelType == 0的选项才有可能被设置为默认
                if ($modelType == 0 && isset($lowestPriceForSpec[$currentSpecItemId]) &&
                    $lowestPriceForSpec[$currentSpecItemId]['option_key'] == $oKey &&
                    $lowestPriceForSpec[$currentSpecItemId]['item_key'] == $k) {
                    $isLowestPrice = true;
                }
...
//                    "product_sn" => $option['product_sn'],
                    ...
                    "is_default" => $isLowestPrice ? 1 : 0, // 修改这里：只有第一次出现才设置为1
                    'virtual' => 0,
                    'modelType' => $modelType,
                    ...
                    'install_guide_url'=>$option['install_guide_url'],
                    'install_guide'=>$option['install_guide']?serialize($option['install_guide']):serialize([]),
                ...
                if ($d3ModelUrl_ori) {
                    $goodsOption['d3ModelUrl_ori'] = $d3ModelUrl_ori;
                }
                if($thumb3dModelUrl){
                    $goodsOption['thumb3dModelUrl'] = $thumb3dModelUrl;
                }
                ...
                if ('SP' == strtoupper($optionTag)) {
                    ...
                    $originalCadModels[$option_id] = null;
                } else {
