...
class GoodsWidgetManager
{
    ...
    public function handle($request = null)
    {
        ...
        $widgetColumns = $widgetClassCollect->groupBy(function (BaseGoodsWidget $widget) {
            ...
            $count = $group->count();
            $collect =  $group->sortByDesc(function (BaseGoodsWidget $widget, $k) use ($count) {
                //框架排序如果值相同超过 16 条排序会错乱,所以如果为0 默认按当前键排序
                $sort = $widget->getOrderBy();
                if ($sort === 0) {
                    return -$k;
                } elseif ($sort > 0) {
                    return $sort + $count;
        ...
        foreach ($this->getGroup() as $key => $name) {
            if ($widgetColumns[$key]) {
                $optiondata = [];
                foreach ($widgetColumns[$key]  as $k =>$val){
                       if($val['template_code'] == "option"){
                           $child_option = $val['data']['option'];

                           $optiondata['has_option'] = $val['data']['has_option'];
                           $optiondata['option'] = $child_option;
                           $optiondata['specs'] = $val['data']['specs'];
                       }
                }
                foreach ($widgetColumns[$key]  as $k =>$val){
                    if($val['template_code'] == "goods"){
                        $widgetColumns[$key][$k]['data']['option'] = $optiondata;
                    }
                }
                $result[] = [
                    'group' => $key,
    ...
    {
        $group = [
            'base' => __('goods/goods.info'), //'tool' => '商品工具', 'marketing' => '营销设置',
