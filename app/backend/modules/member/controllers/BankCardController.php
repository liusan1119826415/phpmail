...
class BankCardController extends BaseController
{
    public function index()
    {
        ...
        $display_info = [
            'displayIdCard' => in_array('idCard', $bank_info) ? 1 : 0,
            'displayMobile' => in_array('mobile', $bank_info) ? 1 : 0,
        ];
        if (request()->ajax()) {
            return $this->successJson('ok', ['display_info' => $display_info]);
        }

        return view('member.bank.edit', ['display_info' => json_encode($display_info)])->render();
    }
