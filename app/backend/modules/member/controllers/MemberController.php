...
use app\backend\modules\member\models\MemberPluginsHomePage;
...
use app\common\models\MemberTiktokModel;
...
use app\common\services\wechat\WxaQrCodeService;
...
class MemberController extends BaseController
{
    ...
    public function index()
    {
        ...
        $is_email_register = 0;
        if (app('plugins')->isEnabled('email-register')) {
            $is_email_register = 1;
        }
        return view('member.index', [
            ...
            'groups' => $groups,
            'levels' => $levels,
            'is_email_register' => $is_email_register,
            'search_member_opt' => json_encode($this->getSearchMemerOpt()),
        ])->render();
    }

    private function getSearchMemerOpt()
    {
        $opt = [
            [
                'value' => 0,
    ...
    {
        $item = request()->item;
        $mobile = trim(\YunShop::request()['mobile']);
    ...
    public function detail()
    {
        ...
        $is_email = 0;
        if (app('plugins')->isEnabled('email-register')) {
            $email_set = Setting::get('plugin.email_register');
            if (!$email_set || $email_set['is_open']) {
                $is_email = 1;
            }
        }

        $member['coupon_total'] = MemberCoupon::uniacid()->where('uid', $member['uid'])->count();
        $member['income_total'] = Income::uniacid()->where('member_id', $member['uid'])->sum('amount');
        if ($type) {
            return $this->successJson('ok', [
                ...
                'is_email' => $is_email,
                'button_list' => $this->getButtonList($member),
            ]);
        }
    ...
    public function memberMerge()
    {
        ...
        $exception = DB::transaction(function () use ($yz_member, $relation_set) {
            ...
            if (Schema::hasTable('yz_member_tiktok')) {
                MemberTiktokModel::where('member_id', $yz_member->mark_member_id)->update(['member_id' => $yz_member->member_id]);
            }
            ...
            $salt = $main_member->salt ?: $abandon_member->salt;
            $password = $main_member->password ?: $abandon_member->password;
    ...
    public function memberChart()
    {
        ...
        switch ($type) {
            ...
            case 2 :
                ...
                break;
            case 3 :
                $count1 = Member::uniacid()->select(['uid', 'mobile'])
                    ->join('yz_member', 'mc_members.uid', '=', 'yz_member.member_id')
                    ->whereNull('yz_member.deleted_at')
				...
				if (app('plugins')->isEnabled('aggregation-cps')) {
					$count1 = Member::uniacid()->select(['uid', 'mobile'])
						->join('yz_member', 'mc_members.uid', '=', 'yz_member.member_id')
						->whereNull('yz_member.deleted_at')
						->join('yz_member_aggregation_app', 'mc_members.uid', '=', 'yz_member_aggregation_app.member_id')
						->where('yz_member_aggregation_app.uniacid', \YunShop::app()->uniacid)
						->count();
				}
            ...
            case 7 :
                $count = Member::uniacid()->select(['uid', 'mobile'])
                    ->join('yz_member', 'mc_members.uid', '=', 'yz_member.member_id')
                    ->whereNull('yz_member.deleted_at')
                    ->join('yz_member_tiktok', 'mc_members.uid', '=', 'yz_member_tiktok.member_id')
                    ->where('yz_member_tiktok.uniacid', \YunShop::app()->uniacid)
                    ->count();
                break;
            case 6 :
                if (!app('plugins')->isEnabled('wechat-customers')) {
...
FROM
    ...
    public function recordDatas()
    {
        ...
        $list = MemberRecord::select(['id', 'uid', 'parent_id', 'after_parent_id', 'status', 'created_at'])
            ->search($search)
            ->uniacid()
            ->orderBy('id', 'desc')
            ->paginate(20)
    ...
    public function changeLevel()  //修改会员等级
    {
        ...
        $shopInfoModel = MemberShopInfo::getMemberShopInfo($uid);

        //判断会员等级是否升级
        $is_upgrade = false;

        $new_level = MemberLevel::find($level_id)->level;
        if ($shopInfoModel->level_id != $level_id && $new_level > $shopInfoModel->level->level) {
            $is_upgrade = true;
        }
        $shopInfoModel->level_id = $level_id;
        if ($shopInfoModel->save()) {
            if ($is_upgrade) {
                //会员等级升级触发事件
                event(new MemberLevelUpgradeEvent($shopInfoModel, true));
    ...
    {
        $data = request()->data;
        $member = \app\common\models\MemberShopInfo::uniacid()->where('member_id', $data['uid'])->first();

        $member->province_name = Address::find($data['province_id'])->areaname;
        $member->city_name = Address::find($data['city_id'])->areaname;
        $member->area_name = Address::find($data['district_id'])->areaname;
        $member->province = $data['province_id'];
        $member->city = $data['city_id'];
        $member->area = $data['district_id'];
        $member->address = $data['address'];

        if ($member->save()) {
            return $this->successJson('会员所在地修改成功');
        } else {
            return $this->errorJson('失败');
        }
    }
    ...
    {
        if (!app('plugins')->isEnabled('member-tags')) {
            return $this->errorJson('请先开启会员标签插件');
        }

        if (!request()->member_id) {
            return $this->errorJson('参数错误');
        }
        $group = MemberTagsGroupModel::uniacid()
            ->orderBy('sort', 'desc')
            ->orderBy('id', 'desc')
            ->get()
            ->toArray();
        $member_tags_ids = MemberTagsRelationModel::uniacid()
        ...
        foreach ($group as $key => $value) {
            ...
            $data[$key]['tags'] = MemberTagsModel::uniacid()
                ->select(['id', 'title', 'type'])
                ->where('group_id', $value['id'])
                ->whereIn('id', $member_tags_ids)
                ->get()
                ->toArray();
        }
    ...
    public function getButtonList($member)
    {
        //支付密码、充值积分、充值余额、登录密码：弹窗模式；
        //优惠券明细、会员订单、余额明细、积分明细、收入明细、提现记录、海报分享记录（新海报）、会员微贴（微社区）、会员视频（分享视频）、会员直播（带货直播）：如未安装对应播件的，不显示对应按钮，跳转到模块/插件相关记录列表列表，筛选该会员的相关记录；
        //在新窗口打开
        //在新窗口打开
        //会员名片、微社区主页、短视频主页、主播主页、微店首页、门店首页：弹窗模式，显示该会员对应的模块前端二维码、链接、小程序路径。
       //type = 5 都是页面跳转

        $dir = 'app/public/qr/detail';
        $data = [
            [
                'type' => 1,
                'name' => "支付密码",
            ...
            [
                'type' => 5,
                'name' => "优惠券明细",
                'go_url' => yzWebFullUrl('coupon.member-coupon.index', ['member_id' => $member['uid']]),
                'order_by' => 5
            ],
            [
                'type' => 5,
                'name' => "会员订单",
                'go_url' => yzWebFullUrl('order.order-list.index', ['member_id' => $member['uid']]),
                'order_by' => 6
            ],
            [
                'type' => 5,
                'name' => "余额明细",
                'go_url' => yzWebFullUrl('finance.balance-records.index', ['member_id' => $member['uid']]),
                'order_by' => 7
            ],
            [
                'type' => 5,
                'name' => "积分明细",
                'go_url' => yzWebFullUrl('point.records.index', ['member_id' => $member['uid']]),
                'order_by' => 8
            ],
            [
                'type' => 5,
                'name' => "收入明细",
                'go_url' => yzWebFullUrl('income.income-records.index', ['member_id' => $member['uid']]),
                'order_by' => 9
            ],
            [
                'type' => 5,
                'name' => "提现记录",
                'go_url' => yzWebFullUrl('withdraw.records', ['member_id' => $member['uid']]),
                'order_by' => 10
            ],
        ];

        if (app('plugins')->isEnabled('new-poster')) {
            $data[] = [
                'type' => 5,
                'name' => "海报分享记录",
                'go_url' => yzWebFullUrl('plugin.new-poster.admin.poster-scan.index', ['recommender_id' => $member['uid']]),
                'order_by' => 11
            ];
        }
        if (app('plugins')->isEnabled('micro-communities')) {
            $data[] = [
                'type' => 5,
                'name' => "会员微贴",
                'go_url' => yzWebFullUrl('plugin.micro-communities.admin.trick.index',
                    [
                        'search' => [
                            'uid' => $member['uid']
                        ]
            ...
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'micro-communities')
                ->first();
            $qr = new QrCodeHelper(yzAppFullUrl("myVideo/{$member['uid']}", ['mid' => $member['uid']]), $dir);
            $data[] = [
                'type' => 6,
                'name' => "微社区主页",
                'plugin_name' => 'micro-communities',
                'h5_qr_url' => $qr->url(),
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl("myVideo/{$member['uid']}", ['mid' => $member['uid']]),
                'min_url' =>  "/packageC/micro_communities/microhomepage/microhomepage?micuid={$member['uid']}",
                'order_by' => 16
            ];
        }
        if (app('plugins')->isEnabled('video-share')) {
            $data[] = [
                ...
                'name' => "会员视频",
                'order_by' => 13,
                'go_url' => yzWebFullUrl('plugin.video-share.admin.videoGoods.index', ['member_id' => $member['uid']])
            ];
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'video-share')
                ->first();
            $qr = new QrCodeHelper(yzAppFullUrl("myVideo/{$member['uid']}", ['mid' => $member['uid']]), $dir);
            $data[] = [
                'type' => 6,
                ...
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl("myVideo/{$member['uid']}", ['mid' => $member['uid']]),
                'min_url' =>  "/packageC/video_goods/myVideo/myVideo?uid={$member['uid']}",
                'order_by' => 17
            ];
        }
        if (app('plugins')->isEnabled('room')) {
            $data[] = [
                'type' => 5,
                'name' => "会员直播",
                'go_url' => yzWebFullUrl('plugin.room.admin.room-manage.index',
                    [
                        'search' => [
            ...
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'room')
                ->first();
            $room = Anchor::uniacid()->where('member_id', $member['uid'])->first();
            $room_id = $room ? $room->id : 0;
            $qr = new QrCodeHelper(yzAppFullUrl("anchorDetail/{$member['uid']}", ['mid' => $member['uid']]), $dir);
            $data[] = [
                'type' => 6,
                'name' => "主播主页",
                'plugin_name' => 'room',
                'h5_qr_url' => $qr->url(),
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl("anchorDetail/{$member['uid']}", ['mid' => $member['uid']]),
                'min_url' => "/packageD/directSeeding/anchorDetail/anchorDetail?id={$member['uid']}",
                'order_by' => 18
            ];

        }
        if (app('plugins')->isEnabled('business-card')) {
            $card = BusinessCardModel::uniacid()
            ...
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'business-card')
                ->first();
            $business_card = BusinessCardModel::uniacid()
                ->where('member_id', $member['uid'])
                ->first();
            $card_id = $business_card ? $business_card->id : 0;
            $qr = new QrCodeHelper(yzAppFullUrl(BusinessCardModel::CARD_QRCODE_URL, $param_arr), $dir);
            $data[] = [
                'type' => 6,
                'name' => "会员名片",
                'plugin_name' => 'business-card',
                'h5_qr_url' => $qr->url(),
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl(BusinessCardModel::CARD_QRCODE_URL, $param_arr),
                'min_url' => "/packageB/member/business_card/BusinessCard/BusinessCard?card_id={$card_id}",
                'order_by' => 15
            ];
        }
        ...
        if (app('plugins')->isEnabled('micro')) {
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'micro')
                ->first();
            $micro_shop =  MicroShop::uniacid()->where('member_id', $member['uid'])->first();
            $qr = new QrCodeHelper(yzAppFullUrl("microShopShare/home/{$micro_shop->id}", ['mid' => $member['uid']]), $dir);
            $data[] = [
                'type' => 6,
                'name' => "微店首页",
                'plugin_name' => 'micro',
                'h5_qr_url' => $qr->url(),
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl("microShopShare/home/{$micro_shop->id}", ['mid' => $member['uid']]),
                'min_url' => "/packageC/microshop/microShop_home/microShop_home?sid={$micro_shop->id}",
                'order_by' => 19,
            ];
        }
        if (app('plugins')->isEnabled('store-cashier')) {
            $url = MemberPluginsHomePage::uniacid()->where('member_id', $member['uid'])
                ->where('plugin_name', 'store-cashier')
                ->first();
            $store = Store::uniacid()->where('boss_uid', $member['uid'])->first();
            $qr = new QrCodeHelper(yzAppFullUrl("o2o/home/homeseller/{$store->id}", ['mid' => $member['uid']]), $dir);
            $data[] = [
                'type' => 6,
                'name' => "门店首页",
                'plugin_name' => 'store-cashier',
                'h5_qr_url' => $qr->url(),
                'min_qr_url' => $url && $url->min_url ? $url->min_url : '',
                'h5_url' => yzAppFullUrl("o2o/home/homeseller/{$store->id}", ['mid' => $member['uid']]),
                'min_url' => "packageC/o2o/HomeSeller/HomeSeller?store_id={$micro_shop->id}",
                'order_by' => 20
            ];
        }

        return array_merge(collect($data)->sortBy('order_by')->toArray());
    }

    public function getMiniAppPageUri()
    {
        $plugin_name =  request()->plugin_name;
        $member_id = request()->member_id;
        $url_model = MemberPluginsHomePage::getPageModel($member_id, $plugin_name);
        switch ($plugin_name) {
            case 'business-card':
                $business_card = BusinessCardModel::uniacid()
                    ...
                    ->first();
                if (!$business_card) {
                    return $this->errorJson('该会员没有名片！');
                }
                $page = 'packageB/member/business_card/BusinessCard/BusinessCard';
                $file_name = 'business_card_home_page_min_qr_' . $business_card->id;
                $scene = 'card_id=' . $business_card->id . 'mark=card' . ',mid=' . $member_id;
                break;
            case 'micro':
                $miro_shop = MicroShop::uniacid()->where('member_id', $member_id)->first();
                if (!$miro_shop) {
                    return $this->errorJson('该会员没有微店！');
                }
                $page = 'packageC/microshop/microShop_home/microShop_home';
                $scene = 'sid=' . $miro_shop->id . ',mid=' . $member_id;
                $file_name = 'micro_shop_id_' . $miro_shop->id;
                break;
            case 'store-cashier':
                $store = Store::uniacid()->where('boss_uid', $member_id)->first();
                if (!$store) {
                    return $this->errorJson('该会员没有门店！');
                }
                $page = 'packageC/o2o/HomeSeller/HomeSeller';
                $scene = 'store_id=' . $store->id . ',mid=' . $member_id;
                $file_name = 'store_home_page_' . $store->id;
                break;
            case 'room':
                $room = Anchor::uniacid()->where('member_id', $member_id)->first();
                if (!$room) {
                    return $this->errorJson('该会员不是主播！');
                }
                $page = 'packageD/directSeeding/anchorDetail/anchorDetail';
                $scene = 'id=' . $member_id . ',mid=' . $member_id;
                $file_name = 'room_home_page_' . $member_id;
                break;
            case 'video-share':
                $page = 'packageC/video_goods/myVideo/myVideo';
                $scene = 'uid=' . $member_id . ',mid=' . $member_id;
                $file_name = 'video_share_home_page_' . $member_id;
                break;
            case 'micro-communities':
                $page = 'packageC/micro_communities/microhomepage/microhomepage';
                $scene = 'micuid=' . $member_id . ',mid=' . $member_id;
                $file_name = 'micro_communities_home_page_' . $member_id;
                break;
        ...
        $dir = 'storage/app/public/member_detail/'.\YunShop::app()->uniacid; //商城根目录下
        $code_helper = new MiniCodeHelper($dir, $file_name, $page, $scene, 120);
        try {
            $url = $code_helper->url();
            $save_data = [
                'min_url' => $url,
            ...
            $url_model->fill($save_data);
            $url_model->save();
            return $this->successJson('获取二维码成功', ['url' => $url]);
        } catch (\Exception $e) {
            return $this->errorJson('生成二维码失败', $e->getMessage());
        }
    }
    public function changePassword()
    {
        try {
            ...
            $member = $member = Member::uniacid()->where('uid', $member_id)->first();
            //随机数
            $data['salt'] = Str::random(8);
            //加密
            $data['password'] = md5($password . $data['salt']);
            ...
            return $this->errorJson('修改密码失败', $e->getMessage());
        }
    }
}
