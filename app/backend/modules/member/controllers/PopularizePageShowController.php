...
class PopularizePageShowController extends BaseController
{
    ...
    {
        $info = Setting::get("popularize.basic");

        $set = request()->input('set');
        if (!empty($set)) {
            if (Setting::set("popularize.basic", $set)) {
                return $this->successJson('保存成功', ['data' => true]);
            } else {
                throw new ShopException('保存失败');
            }
    ...
    protected function getData()
    {
        ...
        foreach ($config as $key => $item) {
            $income_page = new $item['class'];

            if ($this->excludePlugin($income_page)) {
                continue;
            }

            $incomeFactory = new PopularizePageShowFactory($income_page, $lang_set);
    ...
    /**
     * 排除插件
     * @param IIncomePage $incomePage
     * @return bool
     */
    protected function excludePlugin(IIncomePage $incomePage): bool
    {
        $method = \Route::current()->getActionMethod();

        switch ($method) {
            case 'miniSet':
                return !method_exists($incomePage, 'getMiniUrl') || empty($incomePage->getMiniUrl());
        }
        return false;
    }
...
json;
    ...
    public function getResult($info, $item = 0)
    {
        ...
        $basics = [
            ...
            'promoted_orders_show_phone' => $info['promoted_orders_show_phone']
        ];
