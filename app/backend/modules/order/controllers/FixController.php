...
class FixController extends BaseController
{
    ...
    public function failedJobs()
    {
        ...
        $sub_day = 7;
        if (intval(request()->sub_day) > 0) {
            $sub_day = intval(request()->sub_day);
        }

        //  一周内失败的订单收货事件
        $orderJobs = \app\common\models\OrderCreatedJob::where('status', 'waiting')
            ->where('created_at', '>', Carbon::now()->subDays($sub_day)->getTimestamp())
            ->where(function ($query) {
                if (request()->order_sn) {
                    $query->where('order_sn', 'like', trim(request()->order_sn));
                }
                if (request()->order_id) {
        ...
        unset($orderJobs);
        $orderJobs = \app\common\models\OrderPaidJob::where('status', 'waiting')
            ->where('created_at', '>', Carbon::now()->subDays($sub_day)->getTimestamp())
            ->where(function ($query) {
                if (request()->order_sn) {
                    $query->where('order_sn', 'like', trim(request()->order_sn));
        ...
        unset($orderJobs);
        $orderJobs = \app\common\models\OrderSentJob::where('status', 'waiting')
            ->where('created_at', '>', Carbon::now()->subDays($sub_day)->getTimestamp())
            ->where(function ($query) {
                if (request()->order_sn) {
                    $query->where('order_sn', 'like', trim(request()->order_sn));
        ...
        unset($orderJobs);
        $orderJobs = \app\common\models\OrderReceivedJob::where('status', 'waiting')
            ->where('created_at', '>', Carbon::now()->subDays($sub_day)->getTimestamp())
            ->where(function ($query) {
                if (request()->order_sn) {
                    $query->where('order_sn', 'like', trim(request()->order_sn));
    ...
    public function deductReturn()
    {
        $order= Order::find(request()->input('id'));

        if ($order && $order->status == Order::CLOSE) {
            $result = (new \app\common\modules\order\OrderDeductReturn($order, 'point'))
                ->closeReturnTotal();
            if ($result['status'] && $result['data']['amount']) {
                 (new \app\common\services\finance\PointService( [
                    'point_income_type' => \app\common\services\finance\PointService::POINT_INCOME_GET,
                    'point_mode' => \app\common\services\finance\PointService::POINT_MODE_ROLLBACK,
                    'member_id' => $order->uid,
                    'order_id' => $order->id,
                    'point' => $result['data']['amount'],
                    'remark' => '订单：[' . $order->order_sn . ']关闭，返还积分抵扣积分' . $result['data']['amount'],
                ]))->changePoint();
            } else {
                dd('失败：'.$result['msg']);
            }
    ...
    public function payFail()
    {
        ...
        if ($order->status != 0) {
            throw new AppException('该订单状态不支持修复');//不是未支付的不能修复，否则出现订单事件两次执行
        }
//        $order->status = 0;
//        $order->save();
        if (!$order) {
