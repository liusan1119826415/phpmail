...
//       $where[] = ['order_express_id','=',null];
       ...
       $orderGoodsCollection = OrderGoods::uniacid()->select($select)->where('order_id', $order_id)
           ->whereNull('order_express_id')->with(['expressPackage','manyRefundedGoodsLog'])->get()->makeVisible('order_id');


       $orderGoodsCollection->makeHidden(['after_sales','expressPackage','manyRefundedGoodsLog','goods']);
...
//       $order_package = OrderPackage::uniacid()->where('order_id', $order_id)->whereNotNull('order_express_id')->get();
...
//       $order_goods = array_values($order_goods->toArray());
   ...
   public function orderExpress()
   {
       ...
       $orderExpress = OrderExpress::where('order_id', $order_id)
           ->with(['hasManyOrderPackage'])->get();

       if ($orderExpress->isNotEmpty()) {
           $orderExpress->makeHidden(['hasManyOrderPackage']);
           $orderExpress->map(function ($orderExpress, $key) {
               $orderExpress->active_name = '包裹-'. ($key + 1);
               $orderExpress->package_goods = $orderExpress->hasManyOrderPackage->map(function ($package) {
                   return [
                       'order_goods_id' => $package->order_goods_id,
                       'total' => $package->total,
                       'goods_title' => $package->orderGoods->title,
                       'goods_thumb' =>$package->orderGoods->thumb,
                       'goods_option_title' => $package->orderGoods->goods_option_title,
                   ];
               })->toArray();
           });
       ...
       return $this->successJson('list', [
           'have_package'=> $orderExpress->isNotEmpty(),
           'order_package'=> $orderExpress
       ]);
   }
    /**
