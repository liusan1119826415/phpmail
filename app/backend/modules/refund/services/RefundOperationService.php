...
use app\backend\modules\refund\services\operation\RefundBatchSendBack;
...
use app\backend\modules\refund\services\operation\RefundPartial;
...
class RefundOperationService
{
     ...
     * @throws AppException
     */
    public static function batchSendBack($params)
    {
        $refundSend = RefundBatchSendBack::find($params['refund_id']);
    ...
    public static function refundPartial($id)
    {
        $refundApply = RefundPartial::find($id);
        if(!isset($refundApply)){
            return false;
        }
        $result = DB::transaction(function () use ($refundApply) {
