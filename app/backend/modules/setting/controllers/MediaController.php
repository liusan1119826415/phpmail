...
class MediaController extends BaseController
{
    ...
    public function tags()
    {
        ...
        $list = CoreAttachTags::uniacid()
            ->where('uid',$uid)
            ->with(['hasManySource' => function($query) use ($uid) {
                if (\YunShop::app()->isfounder !== true) {

                    $query->where('uid', $uid)->with(['tag']);
                } else {
                    $query->with(['tag']);
                }
            }])
...
//        });
        if (\YunShop::app()->isfounder !== true) {
            $allCount = $allCount->where('uid', $uid);
         //   $unCount = $unCount->where('uid', $uid);
        }
