...
class SmallProgramController extends BaseController
{
    ...
    public function synchronization(){
        $title_list = [
            ...
            '16309' => ['keyword' => ['1', '2', '3', '4'], 'scene' => '余额变动通知'],
            ];
        ...
        $corres_list = [
            ...
            ["uniacid"=>\YunShop::app()->uniacid,"small_type"=>1,"template_name"=>"余额变动通知","created_at"=>$time,"updated_at"=>$time],
            ...
            ["uniacid"=>\YunShop::app()->uniacid,"small_type"=>7,"template_name"=>"余额变动通知","created_at"=>$time,"updated_at"=>$time],
        ];
        ...
        //兼容 不需要默添加模板的情况，客户手动添加模板才发消息
        //$tempTitleArr = array_column($tempList['data'], 'title','priTmplId');
        foreach ($tempList['data'] as $k1 => $v1) {

            if ($v1['title'] == '优惠券到账通知') {
                $mini_app_temp = MinAppTemplateMessage::uniacid()->where("template_id", $v1['priTmplId'])->where("title", '优惠券到账通知')->first();
                if (!$mini_app_temp) {
                    MinAppTemplateMessage::create([
                        'uniacid' => \YunShop::app()->uniacid,
                        'title' => $v1['title'],
                        'template_id' =>  $v1['priTmplId'],
                        'is_default' => 1,
                        'small_type' => 1,
                        'data' => $v1['content'],
                    ]);
                }
                $hasTempId[] =  $v1['priTmplId'];
            }
        }
