...
';
            ...
            if ($setting_data) {
                SiteSetting::set('mongodb', $setting_data);
            }
            return $this->successJson('修改成功,如没生效可尝试清理缓存后重试');
        }
        ...
        $setting = SiteSetting::get('mongodb');
        $setting['is_open'] = $setting['is_open'] ?? 0;

        return view('siteSetting.mongoDB_config', ['set' => json_encode($mongoDB),'setting' => json_encode($setting)]);
    }
     ...
     *  2023/11/28 15:06
     */
    public function yzSupplyMessageConfig()
    {
        $setting = SiteSetting::get('yz_supply_message');
        $setting['is_open'] = $setting['is_open'] ?? 2;
        return view('siteSetting.yz_supply_message_config', [
            'setting' => json_encode($setting),
        ])->render();
    }
    ...
    public function mqttConfig()
    {
        ...
        $setting['is_switch'] = $setting['is_switch'] ?? 0;
        return view('siteSetting.mqtt', [
            'setting' => json_encode($setting),
        ])->render();
    }
	...
	public function domain()
	{
		...
		if (request()->ajax()) {
			if (empty($domain['old_domain']) || empty($domain['new_domain'])) {
				return $this->errorJson(' 域名不能为空');
			}

			$preg = "/^http(s)?:\\/\\/.+/";
			if (preg_match($preg, $domain['old_domain']) || preg_match($preg, $domain['new_domain'])) {
				return $this->errorJson(' 域名格式错误');
			}

			\Artisan::call('change:domain', ['dDomain' => $domain['new_domain'], 'oDomain' => $domain['old_domain']]);

			return $this->successJson(' 域名更新成功');
		}

		return view('siteSetting.domain');
	}
