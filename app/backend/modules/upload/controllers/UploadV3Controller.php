...
class UploadV3Controller extends BaseController
{
    ...
    public function upload()
    {
        ...
        $is_high = request()->input('is_high',0);
        ...
        if ($type == 'image') {
            ...
            $respone = [
                ...
                'webp_path'=>$upload_res['webp_path'],
                'webp_absolute_path'=>$upload_res['webp_absolute_path'],
                'webp_thumb_path'=>$upload_res['webp_thumb_path'],
                'webp_thumb_absolute_path'=>$upload_res['webp_thumb_absolute_path'],
                'is_image' => 1,
                'filesize' => 'null',
            ...
            if($is_high == 1){
                $respone['high_relative_path'] = $upload_res['high_relative_path'];
                $respone['high_absolute_path'] = $upload_res['high_absolute_path'];
            }
            ...
            return $this->successJson('上传成功', $respone);
        } elseif ($type == 'video') {
            try {
        ...
        }elseif ($type == "zip" || $type == "dwg" || $type == "pdf" || $type == "ppt"){
            $mtype = "";
            if($type == "zip"){
                $mtype = 10;
            }elseif($type == "dwg"){
...
//                $response = $this->sendToPythonApi($file);
//
//                if (!$response['success']) {
//                    return $this->errorJson("文件处理失败: " . $response['message']);
//                }
//                $tempFilePath = storage_path("app/public/tmp");
//                $tempFilePath = $tempFilePath."/dwg_file.".$extension;
//                file_put_contents($tempFilePath,$response['file_content']);
//                $file = new \Illuminate\Http\File($tempFilePath);
    ...
    private function sendToPythonApi($file)
    {
        ...
        $apiUrl = "http://127.0.0.1:8000/process-dwg";
    ...
    private function mimeCheck($resp)
    {
        if (!strexists($resp['headers']['Content-Type'], 'image')) {
            throw new ShopException('提取资源失败, 仅支持图片提取.');
        }
        switch ($resp['headers']['Content-Type']) {
            ...
            case 'image/tiff':
                ...
                break;
            default:
                throw new ShopException('提取资源失败, 资源文件类型错误.');
                break;
        }
        return $ext;
    }
    ...
    public function getFileList()
    {
        ...
        $pageSize = request()->pageSize;
        $type = request()->type;
        $core_attach = $core_attach->search(request()->date)->where('uniacid', $this->uniacid)->where('type', $type);
        $tagId = request()->tag_id;
    ...
    public function getNewImage()
    {
        ...
        $core_attach = $core_attach->select('id','attachment','filename','is_high','filename_high','attachment_high')
            ->orderby('created_at', 'desc')
            ->orderby('id', 'desc')
            ->paginate($pageSize);
        foreach ($core_attach as &$attach) {
    ...
    {
        $ids = request()->ids;
        $res = \app\platform\modules\application\models\CoreAttach::whereIn('id',$ids)->delete();
        if($res){
