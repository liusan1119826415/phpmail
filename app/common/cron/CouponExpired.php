    ...
    public function expired()
    {
        ...
        foreach ($coupons as $coupon) {
            ...
            if ($coupon['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE && ($coupon['time_days'] !== 0)) {
                MemberCoupon::where('coupon_id', $coupon['id'])
                    ->where('uid', '<>', 0)
                    ->where(DB::raw('ifnull(`get_time`, 0) + (' . ($coupon['time_days']+$coupon['wait_days']) . ' * 86400)'), '<=', $time)
                    ->where('is_expired', 0)
                    ->where('near_expiration', 1)
                    ->update(['is_expired' => 1]);
            }
    ...
    public function nearExpired()
    {
        ...
        foreach ($coupons as $coupon) {
            ...
            if ($coupon['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE && ($coupon['time_days'] !== 0)) {
                MemberCoupon::where('coupon_id', $coupon['id'])
                    ->where('uid', '<>', 0)
                    ->where(DB::raw('ifnull(`get_time`, 0) + (' . ($coupon['time_days']+$coupon['wait_days']) . ' * 86400) - ' . $time), '<=', 259200)
                    ->where(['is_expired' => 0, 'near_expiration' => 0])
                    ->update(['near_expiration' => 1]);
            }
        }
