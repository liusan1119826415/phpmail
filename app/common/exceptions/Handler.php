...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if ($exception instanceof ApiException) {
            return $this->errorJson($exception->getMessage(), array_merge(['error_code'=> 'ERR000','error_msg'=> $exception->getMessage()], $exception->getData()));
        }
        ...
        //api异常
        if (\YunShop::isApi() && app()->environment() !== 'production') {
    ...
    protected function renderNotFoundException(NotFoundHttpException $exception)
    {
        ...
        if (\Yunshop::isApi() || request()->ajax()) {
            return $this->errorJson('不存在的接口',['error_code'=> 'ERR404','error_msg' => '不存在的接口']);
        }

        //对外接口不存在
        if (\YunShop::isOutsideApi()) {
            return $this->errorJson('不存在的接口', ['error_code'=> 'ERR404','error_msg' => '不存在的接口']);
        }
    ...
    protected function renderMemberNotLoginException(MemberNotLoginException $exception)
    {
        ...
        if (request()->isFrontend() || request()->ajax()) {
            ...
            $data = ['login_status' => 0, 'login_url' => Url::absoluteApi('member.login.index', $queryString), 'extra' => $extra, 'scope' => Utils::wechatScope()];
