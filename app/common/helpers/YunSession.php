...
class YunSessionMysql extends YunSession
{
    ...
    public function write($sessionid, $data)
    {
        ...
        if($mem = $this->chk_member_id_session($data)){
            $member_id = unserialize(explode('|', $mem)[1])['data'];
        }
        ...
        $sql = 'REPLACE INTO ' . DB::getTablePrefix() . "core_sessions (`sid`, `uniacid`, `openid`,`member_id`, `data`, `expiretime`) 
                   VALUES ('{$row['sid']}', {$row['uniacid']}, '{$row['openid']}','{$row['member_id']}','{$row['data']}', {$row['expiretime']})";
