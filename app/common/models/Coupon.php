...
class  Coupon extends BaseModel
{
    ...
    const COUPON_SINCE_RECEIVE_CAN_USE = 2;//有效期 - 领取n天后x天内可使用
     ...
     * @var array
     */
    protected $appends = ['coupon_type_name','coupon_img_url'];
...
//        'back_redpack','back_when','descnoset','deleted_at'];
    ...
    public function getCouponImgUrlAttribute()
    {
        return yz_tomedia($this->coupon_img);
    }
    ...
    public static function getUsageCount($couponId)
    {
        return static::uniacid()
            ...
            ->withCount(['hasManyMemberCoupon' => function ($query) {
                return $query->where('used', 0)->where('is_expired', 0);
            }]);
    }
    ...
    public static function getTimeLimit($couponId)
    {
        ...
        switch ($time_limit) {
            ...
            case self::COUPON_DATE_TIME_RANGE:
                ...
                break;
            case self::COUPON_SINCE_RECEIVE_CAN_USE:
                return [
                    'type' => self::COUPON_SINCE_RECEIVE_CAN_USE,
                    'time_end' => static::uniacid()->where('id', '=', $couponId)->value(DB::raw('time_days+wait_days')),
                ];
            default:
                return [
                    'type' => self::COUPON_SHOP_USE,
