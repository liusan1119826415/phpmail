...
class Goods extends BaseModel
{
    ...
    {
        return ['title' => __('goods.title'), 'price' => '价格', 'cost_price' => '成本价', 'sku' => '商品单位', 'thumb' => '图片', 'weight' => '重量', 'stock' => '库存',];
    }
    ...
    public function hasManyStyleRelations()
    {
        return $this->hasMany(GoodsStyleRelations::class, 'goods_id');
    }
    ...
    {
        return $this->hasMany(GoodsRelation::class, 'goods_id')
            ->with('relatedGoodsInfo')
            ->orderBy('sort_order');
    }
     ...
     * 获取所有被关联的商品（反向关系）
     */
    public function relatedByGoods()
    {
        return $this->hasMany(GoodsRelation::class, 'related_goods_id')
            ->with('goods')
            ->orderBy('sort_order');
    }
     ...
     * 获取所有关联的商品（包括正向和反向）
     */
    public function getAllRelatedGoods()
    {
        $directlyRelated = $this->relatedGoods->pluck('relatedGoodsInfo');
        $inverselyRelated = $this->relatedByGoods->pluck('goods');

        return $directlyRelated->merge($inverselyRelated)->unique('id');
    }
     ...
     * 关联商品
     */
    public function relateTo($relatedGoodsId, $relationType = 1, $sortOrder = 0)
    {
        // 检查是否已经存在关联
        $existingRelation = GoodsRelation::where(function($query) use ($relatedGoodsId) {
            $query->where('goods_id', $this->id)
                ->where('related_goods_id', $relatedGoodsId);
        })
            ->orWhere(function($query) use ($relatedGoodsId) {
                $query->where('goods_id', $relatedGoodsId)
                    ->where('related_goods_id', $this->id);
            })
            ->exists();

        if (!$existingRelation && $this->id != $relatedGoodsId) {
            return GoodsRelation::create([
                'goods_id' => $this->id,
    ...
    public function scopeIsPlugin($query, $is_plugin = 0)
    {
        return $query->where($this->getTable() . '.is_plugin', $is_plugin);
    }
    ...
    public function showPluginGoods()
    {
                ...
                case 'keyword':
                    $query->where(function ($query1) use ($value) {
                        ...
                        if (empty($splice_word)) {
                            $query1->whereTitle($value);
                        } else {
                            $splice_array = explode(',', $splice_word);
                            $query1->where(function ($query2) use ($splice_array) {
                                foreach ($splice_array as $val) {
                                    $query2->whereTitle($val);
                                }
                            });
                        }
                        $query1->orWhere('yz_goods.id',$value);
                    });
                    break;
                case 'goods_id':
...
//                        $id = $value['thirdid'][0] ? $value['thirdid'][0] : $id;
                        ...
                        $goodsCategoryIdsQuery = GoodsCategory::select('goods_id', 'category_id')
                            ->whereRaw('FIND_IN_SET(?, category_ids)', [$id])
                            ->groupBy('goods_id');

                        $query->select(['yz_goods.*', 'subquery.category_id as category_id'])
                            ->joinSub($goodsCategoryIdsQuery, 'subquery', function ($join) {
                                $join->on('yz_goods.id', '=', 'subquery.goods_id');
                            });

//                        $query->select(['yz_goods.*', 'yz_goods_category.id as goods_category_id', 'yz_goods_category.goods_id as goods_id', 'yz_goods_category.category_id as category_id', 'yz_goods_category.category_ids as category_ids'])
...
//                            $query->join('yz_goods_spec','yz_goods.id','yz_goods_spec.goods_id')->select('yz_goods.*','yz_goods_spec.goods_id');
    ...
    {
        return $query->where('yz_goods.title', 'like', '%' . $title . '%');
    }
        ...
        foreach ($filters as $key => $value) {
            switch ($key) {
                ...
                case 'goods_id':
                    $query->where('yz_goods.id', '=', $value);
                    break;
                case 'search_goods_ids':
                    $query->whereIn('yz_goods.id',$value);
                    break;
                case 'brand_id':
                    $query->where('brand_id', $value);
                    break;
                ...
                case 'category':
                    if (array_key_exists('parentid', $value) || array_key_exists('childid', $value) || array_key_exists('thirdid', $value)) {
                        ...
                        $id = $value['childid'] ? $value['childid'] : $id;
                        $id = $value['thirdid'] ? $value['thirdid'] : $id;
                        $query->addSelect(['yz_goods_category.id as goods_category_id',
...
//                        })->groupBy('yz_goods.id');
    ...
    public static function getGoodsByName($keyword)
    {
        return static::uniacid()->select('id', 'title', 'thumb', 'market_price', 'price', 'real_sales', 'sku', 'plugin_id', 'stock')
            ->whereTitle($keyword)->where('status', 1)//->where('is_plugin', 0)
...
//            ->whereIn('plugin_id', ['0', '32', '92'])//屏蔽门店、码上点餐、第三方插件接口的虚拟商品
    ...
    public static function getGoodsByNameLevel($keyword)
    {
        $where = function ($query) use ($keyword) {
            if (!empty($keyword) && intval($keyword) == $keyword) {
                return $query->whereTitle($keyword)->orWhere('id', $keyword);
            } else {
                return $query->whereTitle($keyword);
            }
        };
    ...
    public static function getGoodsByNameForLimitBuy($keyword)
    {
        ...
        return static::uniacid()->select('id', 'title', 'thumb', 'market_price', 'price', 'real_sales', 'sku', 'plugin_id', 'stock')
            ->whereTitle($keyword)->where('status', 1)->with(['hasOneGoodsLimitBuy' => function ($query) {
    ...
    public function generalValidate(Member $member, $total)
    {
        if (empty($this->status)) {
            throw new AppException($this->title . '(ID:' . $this->id . ')已下架');
        }
...
//        $keyword = \YunShop::request()->keyword;
        return Goods::select(['id', 'title', 'thumb', 'plugin_id'])->pluginId($pluginId)
            ->whereTitle($keyword)->get();
    }
...
//    }
    ...
    public function getVipPriceAttribute()
    {
        //todo 要求列表多规格商品显示最低规格价的vip价，先把会员价插件的计算搬过来
        if (\YunShop::app()->getMemberId()) {
            $member = \app\frontend\models\Member::current();
        } else {
            $member = new \app\frontend\models\Member();
    ...
    {
        return sprintf("%.2f", bankerRounding($price));
        //舍去两位小数后的位数，不进行四舍五入
...
//        }
    ...
    public function getVipLevelStatusAttribute()
    {
        ...
        if (!app('plugins')->isEnabled('price-authority') || !$set['is_jurisdiction']) {
            return (new GoodsPriceShow($this->plugin_id,\YunShop::app()->getMemberId(),$this))->getVipLevelStatus();
//            return $vip_status;
        }
        //查询会员等级
