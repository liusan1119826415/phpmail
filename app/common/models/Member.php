...
class Member extends BackendModel
{
    ...
    protected $hidden = ['password', 'salt', 'pay_password'];
    ...
    //抖音
    public function hasOneTiktok()
    {
        return $this->hasOne(MemberTiktokModel::class, 'member_id', 'uid');
    }
    ...
    public function hasManyMemberChildren()
    {
    ...
    {
        return $this->hasOne(MemberParent::class, 'member_id', 'uid');
    }
    ...
    {
        if (($search['level_id'] !== '' && $search['level_id'] !== null) || $search['group_id'] || $search['member_level'] || $search['member_group']) {
    ...
    {
        if ($this->getAttributeFromArray('avatar')) {
            return yz_tomedia($this->getAttributeFromArray('avatar'));
        }
        if (\Setting::get('shop.member.headimg')) {
            return yz_tomedia(\Setting::get('shop.member.headimg'));
        }
        return yz_tomedia(Setting::get('shop.shop')['logo']);
    }
...
//            'realname'  => 'required|between:2,10',
    ...
    public function getMemberIdForInviteCode()
    {
        ...
        if($set['can_bind_mobile'] == 1 && \YunShop::request()->invite_code){
            //邀请码可填手机号
            $data = Member::uniacid()
                ->select(['uid'])
                ->where('mobile',\YunShop::request()->invite_code)
                ->first();
