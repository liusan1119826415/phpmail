...
//        });
	 ...
	 * @return void
	 */
	public static function checkOrderConfirm($uid, $orderId = 0)
	{
		$set = self::getSetInfo()->first();
		if (empty($set) || $set->status == 0) {
			return;
		...
		if ($member->parent_id == 0) {
			...
			if ($become_child == 1
                && empty($member->inviter)
                && self::orderGoodsCheck($orderId,$set)
            ) {
			...
			if ($parent_is_agent
                && $become_child == 1
                && !$member->inviter
                && $member->member_id != $parent->member_id
                && self::orderGoodsCheck($orderId,$set)
            ) {
    ...
    public static function orderGoodsCheck($order_id, $set = null)
    {
        if (!$set){
            $set = self::getSetInfo()->first();
        }
        if (!$set->order_goods_check) {
            return true;
        }
        $order_goods_ids = $set->order_goods_ids;
        if (!is_array($order_goods_ids)) {
            $order_goods_ids = json_decode($order_goods_ids, true);
        }
        if (!$order_goods_ids) {
            return false;
        }
        return (bool)OrderGoods::uniacid()->where('order_id', $order_id)->whereIn('goods_id', $order_goods_ids)->first();
    }
