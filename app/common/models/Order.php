 ...
 * @property Collection refundGoodsLog
...
class Order extends BaseModel
{
    ...
    protected $appends = ['status_name', 'pay_type_name','new_status'];
    ...
    const CLOSE = -1;
    //const WAIT_SUBMIT = 10;  //提交订单
    const WAIT_PAY = 0;  //待付款
    const WAIT_SEND = 1; //待发货

    const WAIT_CONFIRM = 4; //待确认图纸


    const WAIT_RECEIVE = 2; //待验收
    const COMPLETE = 3; //交易完成
    const REFUND = 11;
    ...
    {
        return ['create_time', 'refund_time', 'operate_time', 'send_time', 'return_time', 'end_time', 'pay_time', 'send_time', 'cancel_time', 'create_time', 'cancel_pay_time', 'cancel_send_time', 'finish_time','first_pay_time','last_pay_time','confirm_time','product_time'] + parent::getDates();
    }
    ...
    {
        return $this->hasMany(SupplierLogisticPrice::class,'order_id','id');
    }

    public function hasOneSupplierPrice()
    {
        return $this->hasOne(SupplierLogisticPrice::class,'order_id','id')->where('supplier_id',Session::get('supplier')['id']);
    }

    public function installOrder()
    {
        return $this->hasOne(InstallOrder::class,'order_id','id');
    }


    public function hasOneSupplierInstallPrice()
    {
        return $this->hasOne(SupplierInstallPrice::class,'order_id','id')->where('supplier_id',Session::get('supplier')['id']);
    }

    public function supplierInstallPrice()
    {
        return $this->hasMany(SupplierInstallPrice::class,'order_id','id');
    }

    public function supplierInstallPriceBidding4()
    {
        return $this->hasMany(SupplierInstallPrice::class, 'order_id', 'id')
            ->where('bidding_status', 4);
    }

    // 订单与订单付款阶段的关联
    public function orderPayments()
    {
        return $this->hasMany(OrderStage::class, 'order_id', 'id');
    }
     ...
     * 创建订单的付款阶段
     */
    public function createPaymentStages()
    {
        ...
        $orderPayments = [
            new OrderStage([
                'order_id'=>$this->id,
                'stage' => 1,
                'price' => $this->getPrice() * $trade['deposit_ratio']/100, // 30% 定金
            ]),
            new OrderStage([
                'order_id'=>$this->id,
                'stage' => 2,
                'price' => $this->getPrice() * (1-$trade['deposit_ratio']/100), // 70% 尾款
            ])
        ];

        $this->orderPayments()->saveMany($orderPayments);
    }
...
//    {
//
//        // 如果 pay_time 为 0 或者时间戳为零，返回 null 或其他默认值
//        $val = ($value && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 08:00:00' && $value != 0) ? $value : "-";
//
//        return $val;
//    }
//
//
//    public function getConfirmTimeAttribute($value)
//    {
//        // 如果 pay_time 为 0 或者时间戳为零，返回 null 或其他默认值
//        return ($value && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 08:00:00' && $value != 0) ? $value : "-";
//    }
//
//
...
//    }
//
//    public function getFirstPayTimeAttribute($value)
//    {
//        return ($value && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 08:00:00' && $value != 0) ? $value : "-";
//    }
//
//    public function getLastPayTimeAttribute($value)
//    {
//        return ($value && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 08:00:00' && $value != 0) ? $value : "-";
//    }
//
//    public function getLastSendTimeAttribute($value)
//    {
//        return ($value && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 08:00:00' && $value != 0) ? $value : "-";
//    }
//
...
//    }
    ...
    public function scopeHidePluginIds($query, $plugin_ids = [], $other_plugin_ids = [])
    {
        if (empty($plugin_ids)) {
            ...
            //藏品中心(168,169),美团团购(177),170(商家让利),178(ycc资产),184(小鹅通同步订单),185(新拍卖充值),187(抖音团购)
            //186赛事活动
            $plugin_ids = [
                33, 40, 41, 43, 54, 59, 69, 46, 70, 106, 96, 77, 78,
                115, 74, 99, 39, 127, 128, 62, 147, 151, 133, 144, 154, 155,
                156, 157, 158, 159, 161, 63, 64, 65, 160, 164, 168, 169, 170, 177, 178, 179, 184, 185, 186, 187];
        }
    ...
    public function scopeHideIsPlugins($query, $is_plugins = [])
    {

        $is_plugins = array_merge([41], $is_plugins);

        return $query->whereNotIn('is_plugin', $is_plugins);
    }
    ...
    public function projectDoor()
    {
        return $this->belongsTo(ProjectDoor::class,'door_id','id');
    }
    ...
    public function orderMainGoods()
    {
        return $this->hasMany(self::getNearestModel('OrderGoods'), 'order_main_id', 'id');
    }
    ...
    public function myOrderAddress()
    {
        return $this->hasOne(OrderAddress::class, 'order_main_id', 'id');
    }
    ...
    public function orderInvoice2()
    {
        return $this->hasOne(OrderInvoiceV2::class, 'order_id', 'id');
    }
    ...
    public function getPayTypeNameAttribute()
    {
        ...
        if ($this->pay_type_id == 3) {
            ...
            return ($lang['credit'] ?: '余额') . $append;
        }
        if ($this->pay_type_id == 111 && app('plugins')->isEnabled('tag-balance')) {
            return \Yunshop\TagBalance\services\SettingService::diyName() . $append;
        }

        if ($this->pay_type_id == 133) {
            $plugin_name = \Setting::get('plugin.agency-balance.plugin_name');
            return $plugin_name ? $plugin_name . '支付' : '代理余额支付';
        }
...
//            $refund_count = $query->refund()->count();
            $refund_status[] = [
                'status' => Order::REFUND,
                'status_name' => __('order.refund_list'),
    ...
    public function setPayTypeIdAttribute($value)
    {
        ...
        if ($this->pay_type_id != $this->getRawOriginal('pay_type_id')) {
    ...
    public function canRefund()
    {
        ...
        //收货后禁止退款，代收货状态禁止退款
        if ($this->status == self::WAIT_RECEIVE) {
    ...
    public function getAllStatusAttribute()
    {
        return collect([
            [
                ...
                'id' => self::WAIT_PAY,
                'name' => __('order.wait_pay'),
            ], [
                'id' => self::WAIT_SEND,
                'name' => __('order.wait_send'),
            ], [
                'id' => self::WAIT_RECEIVE,
                'name' => __('order.wait_receive'),
            ], [
                'id' => self::COMPLETE,
...
//        }
    ...
    public function hasOneMemberChild()
    {
        return $this->hasOne(MemberChildren::class, 'child_id', 'uid');
    }
}
