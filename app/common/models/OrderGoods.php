...
class OrderGoods extends BaseModel
{
    ...
    protected $hidden = ['order_id','manyRefundedGoodsLog'];
...
//    const FINAL_REFUND = 3;//最终
    ...
    {
        return $this->belongsTo(FloorSpace::class,'space_id');
    }
    ...
    public function getAfterSalesAttribute()
    {
        ...
        if ($this->refund_id) {
            ...
            $after_sales['refund_sn'] = $this->hasOneRefund->refund_sn;
            $after_sales['status'] = $this->hasOneRefund->status;
            $after_sales['create_time'] = $this->hasOneRefund->create_time->format('Y-m-d H:i:s');
            $after_sales['price'] = $this->hasOneRefund->price;
...
//            ->where('status','!=', RefundGoodsLog::CANCEL_STATUS)
    ...
    public function orderGoodsChildrens()
    {
        return $this->hasMany(OrderGoods::class, 'parent_id', 'id');
    }
    ...
    public function expressPackage()
    {
        return $this->hasMany(OrderPackage::class,'order_goods_id', 'id');
    }

    public function goodsCoupon()
    {
        return $this->hasOne(GoodsCoupon::class,'goods_id','goods_id');
    }
    ...
    public function scopeOrderGoods(Builder $query)
    {
        return $query->select([
            ...
            'payment_amount','deduction_amount','vip_price','product_sn','type', 'weight'
        ])->with(['goods'=>function ($query) {
            //在这里添加'weight','product_sn','goods_sn'字段，因为快递打印那边要用
            return $query->select(['id','title','status','type','thumb','sku','market_price','price','cost_price','weight','product_sn','goods_sn','alias','lead_time']);
        }]);
    }
