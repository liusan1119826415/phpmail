...
use app\common\exceptions\ErrorConst;
...
//        }
        ...
        $this->orders->each(function (\app\common\models\Order $order) {
           /* if ($order->status > Order::WAIT_PAY && ($order->status == 1 && $order->orderStatus == 1 ) ) {
                throw new AppException('(ID:' . $order->id . ')订单已付款,请勿重复付款', [ErrorConst::ORDER_PAY_NOTICE => ErrorConst::ORDER_REPEAT_PAY]);
            }*/
            if ($order->status == Order::CLOSE) {
                throw new AppException('(ID:' . $order->id . ')订单已关闭,无法付款',[ErrorConst::ORDER_PAY_NOTICE => ErrorConst::ORDER_CLOSE, 'order_pay_id' => $this->id]);
            }
...
//            \Log::debug('支付前监听结束',$order->id);
    ...
    public function payOrderOne()
    {
        return $this->hasOne(PayOrder::class, 'out_order_no', 'pay_sn')->where('status',2);
    }
     ...
     * @throws AppException
     */
    public function getPayResult($pay_type_id, $payParams = [])
    {
        if (is_string($pay_type_id)) {
            $code = $pay_type_id;
            $pay_type_id = app(PaymentManager::class)->get($code)->getId();
        }
        if ($this->created_at->timestamp + 36000 < time()) {
        ...
        $key = implode(array_merge($this->order_ids,[$this->pay_type_id]),'');
        //再发起支付前记录
        /*$restrictAccess = \app\common\services\RequestTokenService::limitRepeat('f_order_pay_'. $key, 60);
        if (!$restrictAccess) {
            throw new AppException('支付已发起，避免重复支付需等待60秒');
        }*/

        $pay = PayFactory::create($this->pay_type_id);
        if (is_null($pay)) {
            $pay = app(PaymentManager::class)->get($code);

            if (!$pay) {
                throw new AppException("支付方式[{$pay_type_id}]不存在");
            }

            $result = $pay->processPayment($query_str);

        } else {
			if (method_exists($pay, 'processPayment')) {
				$result = $pay->processPayment($query_str);
			} else {
    ...
    public static function updateSuccess($id,$pay_type_id)
    {
        if ($id && $pay_type_id) {
            OrderPay::where('id', $id)->update([
...
//        $this->save();
    ...
    public function save(array $options = [])
    {
        ...
        if ($this->getRawOriginal('status') != self::STATUS_REFUNDED and $this->status == self::STATUS_REFUNDED) {
