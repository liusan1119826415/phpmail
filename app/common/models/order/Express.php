...
class Express extends BaseModel
{
    ...
    public function getExpress($express = null, $express_sn = null, $phoneLastFour = '')
    {
        ...
        if (empty($phoneLastFour) && 'SF' == $express){
            ...
            $member_name = $this->getMobile($this->order_id, 1);
        }

        $result = app('logistics')->getTraces($express, $express_sn,'',$phoneLastFour, $this->is_encryption, $member_name);
     ...
     */
    public function getMobile($order_id, $type=0)
    {
        if (empty($order_id)){
        ...
        if ($type==1) {
            return  $address['address']['realname'];
        }
    ...
    public function setIsEncryptionAttribute($value)
    {
        $this->attributes['is_encryption'] = empty($value) ? 0 : 1;
    }
