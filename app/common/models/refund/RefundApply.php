 ...
 * @property Collection hasManyReturnExpress
 *
 */
class RefundApply extends BaseModel
{
    protected $table = 'yz_order_refund';
    protected $hidden = ['updated_at','uniacid', 'uid', 'order_id'];
    protected $fillable = [];
    ...
    {
        return ['create_time', 'refund_time', 'operate_time', 'send_time', 'return_time', 'end_time', 'cancel_pay_time', 'cancel_send_time','pass_time','supplier_pass_time','reject_time'] + parent::getDates();
    }
...
//        }
    ...
    {
        $model = self::select(['id','order_id', 'uid','refund_type', 'status',
    ...
    //换货-用户寄回发货
    public function  hasManyReturnExpress()
    {
        return $this->hasMany(ReturnExpress::class, 'refund_id', 'id');
    }
    ...
    {
        return $this->hasOne(PayRefundRecord::class, 'refund_sn', 'refund_sn');
    }
    ...
    protected function getRefundTypeName()
    {
        return [
            ...
            $this->order->order_type == 5?"取消订单":self::REFUND_TYPE_RETURN_GOODS => __('order.refund_and_return'),
    ...
    protected function getStatusNameMapping()
    {
        /*return [
            ...
            self::CANCEL => '用户取消',
            self::REJECT => '已驳回',
            self::WAIT_CHECK => __('order.wait_check'),
...
//            self::CONSENSUS => '已手动退款',
    ...
    final public function isPartRefundV2()
    {
        return $this->is_all === 2;
    }
