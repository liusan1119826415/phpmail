...
class RefundGoodsLog extends BaseModel
{
    ...
    protected $appends = ['goods_id','goods_title','goods_option_title','goods_thumb','component_data','space_name','floor_name','space_id','floor_id'];
    ...
    {
        $log = self::where('order_goods_id', $orderGoods['id'])->where('refund_id', $refund->id)->first();
    ...
    {
       return $this->orderGoods->spaces->name;
    }

    public function getSpaceIdAttribute()
    {
        return $this->orderGoods->spaces->id;
    }

//    public function getSupplierIdAttribute()
//    {
//        return $this->orderGoods->goods->supp_id;
//    }


    public function getFloorIdAttribute()
    {
        return $this->orderGoods->floors->id;
    }

    public function getFloorNameAttribute()
    {
        return $this->orderGoods->floors->name;
    }

    public function getComponentDataAttribute()
    {
        return $this->getMaterialColor($this->orderGoods->components);
    }



    protected function getMaterialColor($components)
    {
        if($components){
            if($this->orderGoods->type == 1){
                $data = json_decode($components,true);
