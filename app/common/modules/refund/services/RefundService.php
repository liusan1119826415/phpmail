...
class RefundService
{
    ...
    public function payV2($refund_id)
    {
        ...
        $payAdapter->setAttribute('pay_name', $this->refundApply->order->pay_type_name);
        $result = $payAdapter->pay($this->refundApply->order->hasOneOrderPay->pay_sn, $this->refundApply->order->hasOneOrderPay->amount, $this->refundApply->price, $this->refundApply->refund_sn);
        if (!$result['status']) {
            \Log::debug("订单退款失败======".$result['msg']);
    ...
    public function pay($refund_id)
    {
        ...
        switch ($this->refundApply->order->pay_type_id) {
            ...
            case PayType::VIDEO_SHOP_PAY:
                ...
                break;
            case PayType::TAG_BALANCE_PAY:
                $result = $this->tagBalancePay();
                break;
            case PayType::ICBC_WECHAT_PAY:
                $result = $this->icbcWechatPay();
                break;
            case PayType::MERCHANT_LOAN_PAY:
                $result = $this->merchantLoanPay();
                break;
            case PayType::HUIBEI_WECHAT:
            case PayType::HUIBEI_ALIPAY:
    ...
    private function tagBalancePay(){
        $pay = PayFactory::create($this->refundApply->order->pay_type_id);
    ...
    private function icbcWechatPay()
    {
        $pay = PayFactory::create($this->refundApply->order->pay_type_id);
        ...
        if (!$result) {
            throw new AdminException('微信-工行支付退款失败');
        }
    ...
    private function merchantLoanPay()
    {
        throw new AdminException('商户贷支付不支持退款');
    }
...
//        }
    ...
    private function authPayRefund()
    {
        ...
        if (is_array($result) && isset($result['msg']) && $result['msg'] == '失败') {
            throw new AdminException($result['pay_msg'] ? : '微信借权支付退款失败');
        }
     ...
     * @throws \app\common\exceptions\AppException
     */
    private function HuibeiPayRefund()
    {
        $pay = PayFactory::create($this->refundApply->order->pay_type_id);
        ...
        if (!$result) {
            throw new AdminException('慧呗支付退款失败!');
        }
