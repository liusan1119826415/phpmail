...
use app\common\events\order\AfterOrderCreatedErrorEvent;
...
//        $this->setRelation('dispatch', $this->getDispatch());
      //  $this->last_deliver_user_info = $this->getLastDeliverUserInfo();
        ...
        $this->pay_price = $this->orders->sum('price') * $trade['deposit_ratio']/100;
       ...
       // $this->first_default = $this->firstDefault();

        event(new AfterTradeCreatedEvent($this));
    }

    protected function firstDefault()
    {
        $pre_ids = $this->orders->pluck('pre_id')->toArray();

        $masterKey = substr(md5(implode('', $pre_ids)), 8, 16);
    ...
    private function getAmountItems()
    {
        ...
        $items = [
            [
                ...
                'name' => __('order.total_goods_price'),
                'amount' => $this->orders->sum('order_goods_price'),
            ], [
                'code' => 'total_dispatch_price',
                'name' => __('order.total_dispatch_price'),
                'amount' => $this->orders->sum('dispatch_price'),
            ], [
                'code' => 'total_prepaid_freight',
                'name' => "总预计物流费用",
                'amount' => $this->orders->sum('prepaid_freight'),
            ], [
                'code' => 'total_prepaid_install_fee',
                'name' => "总预计安装费用",
                'amount' => $this->orders->sum('prepaid_install_fee'),
            ]
        ];
    ...
    {
        $point_name = \app\common\services\LangService::getLangSet('member_center')['credit1'] ?: '积分';
        return $point_name;
    }
    ...
    public function generate()
    {
        try {
            ...
            $order_main_id = $this->createParentOrder();
            OrderCreatedJob::create([
                'order_id' => $order_main_id,
            ]);
            $this->dispatch(new OrderCreatedEventQueueJob($order_main_id));
            $this->orders->map(function (PreOrder $order) use ($order_main_id){
                /**
                 * @var $order
                 */
                $order->generate($order_main_id);
            ...
            $event = new AfterOrderCreatedErrorEvent($this->orders);
            event($event);
            DB::rollBack();
            $event->saveLogs();
            \Log::error('order-save-error:'.$e->getMessage().' error file in '.$e->getFile().'('.$e->getLine().')');
    ...
    public function createParentOrder()
    {
        ...
        $data = [
           ...
           "order_sn"=>"COIN".OrderService::createOrderSN(),
           "price"=>$this->orders->sum('goods_price')*$trade['deposit_ratio']/100,
           "order_goods_price"=>$this->orders->sum('order_goods_price'),
           "goods_price"=>$this->orders->sum('goods_price'),
           "goods_total"=>$this->orders->sum('goods_total'),
           "create_time"=>time(),
           'project_id'=>$this->request->input('project_id'),   //项目id
           'status'=>\app\common\models\Order::WAIT_CONFIRM,//待确认图纸
        ];
        $order = \app\common\models\Order::create($data);
        $this->createPaymentStages($order,$this->orders->sum('goods_price'));
        return $order->id;
    }


    public function createPaymentStages($order,$total_price)
    {
        $trade = \Setting::get('shop.trade');
        $orderPayments = [
            new OrderStage([
                'order_id'=>$order->id,
                'stage' => 1,
                'price' => $total_price * $trade['deposit_ratio']/100, // 30% 定金
            ]),
            new OrderStage([
                'order_id'=>$order->id,
                'stage' => 2,
                'price' => $total_price * (1-$trade['deposit_ratio']/100), // 70% 尾款
            ])
        ];
