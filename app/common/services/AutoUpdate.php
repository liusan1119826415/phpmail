...
class AutoUpdate
{
        ...
        foreach ($this->_updates as $key => $update) {
            ...
            if (file_exists($this->_tempDir . 'md5.txt')) {
                ...
                if (empty($error)) {
					...
					$destinationDir = $this->_tempDir . 'data/wwwroot/' . $chk_url . '/storage/' . $update['upgrade'] . '/' . $default_dir . '/';

					if (!is_dir($destinationDir)) {
						app(Filesystem::class)->makeDirectory($destinationDir, 0755, 1);
					}

					// Install update
                    $yZip = new YZip();
                    $yZip->unzip($this->_tempDir, $destinationDir);
                    ...
                    if (2 == $client) {
                        ...
                        if (is_dir(storage_path($cp_source_path))) {
                            $cp_index_path = $cp_source_path . '/admin.html';
    ...
    public function checkBackUpdate()
    {
        ...
        if ($versions === null || $versions === false) {
            ...
            //$update = @file_get_contents($updateFile, $this->_useBasicAuth());
            $plugins = [];
            foreach (app('plugins')->getPlugins() as $key => $value) {
