...
class ImageZip {
    ...
    public static function makeThumbWebp($srcFile, $mode = 'main')
    {
        ...
        $config = [
            'thumb' => ['width' => 300, 'quality' => 85, 'webp' => true],
            'medium_thumb' => ['width' => 512, 'quality' => 100, 'webp' => true],
            'goodsmain' => ['width' => 748, 'quality' => 95, 'webp' => true],
            'main' => ['width' => 748, 'quality' => 95, 'webp' => false],
            'detail' => ['width' => 1200, 'quality' => 95, 'webp' => false],
            'original' => ['width' => 3000, 'quality' => 100, 'webp' => false],
        ];
        ...
        if ($mode === 'detail') {
            if ($src_width < $target_width) {
                // 宽度小于 1200，直接复制，避免放大失真
                $ext = pathinfo($srcFile, PATHINFO_EXTENSION);
                $jpegPath = preg_replace('/\.(jpg|jpeg|png|gif|bmp)$/i', '', $srcFile) . "_$mode.$ext";
                copy($srcFile, $jpegPath);
                return $jpegPath;
            }
            // 宽度大于等于 1200，需要等比例缩放
            $width = $target_width;
