...
class PermissionService
{
     ...
     * @return array
     */
    public static function getNoPermissionList($menus,$is = 0)
    {
        $noPermissions = [];
        if ($menus) {
            foreach ($menus as $key => $m) {
                if ((\YunShop::app()->role == 'clerk' && $m['alone_admin']) || $is == 1) {
                    $noPermissions[] = $key;
                    $noPermissions = array_merge($noPermissions, self::getNoPermissionList($m['child'],1));
                    continue;
                }
