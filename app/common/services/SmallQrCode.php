...
class SmallQrCode
{
    ...
    public function getSmallQrCode($small_name,$postData)
    {
        ...
        do {
            ...
            $access_token = getMiniAccessToken($retry_count?true:false);
            ...
            $retry_count += 1;
            //如果是access_token 报错刷新再请求一次
        }while((isset($result['errcode']) && $result['errcode'] == 40001) && $retry_count == 1);
        ...
        if (isset($result->errcode)) {
            ...
            \Log::debug('----SmallQrCode-小程序码生成失败------', $result);

            $data['message'] = '错误码' . $result->errcode . ';错误信息' . $result->errmsg;
            $data['code'] = 1;
            return $data;
        }
    ...
    public function getSmallQrCode11111111($small_name,$postData)
    {
