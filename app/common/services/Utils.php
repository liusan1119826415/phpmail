...
class Utils
{
	...
	public static function getCertInfo()
	{
		...
		if (!extension_loaded('openssl') || !is_callable('openssl_x509_parse')){
			return ['status' => -1, 'msg' => '请开启openssl扩展'] ;
		}
		...
		if ($client == false) {
			return ['status' => -1, 'msg' => $domain . '未查到可靠信息','err'=>[
				'errno'=>$errno,
		...
		if (empty($params['options']['ssl']['peer_certificate'])) {
			return ['status' => -1, 'msg' => $domain . '获取信息失败，请确保可以正常访问'];
		}
		$cert = $params['options']['ssl']['peer_certificate'];
		$cert_info = @openssl_x509_parse($cert);
		return ['status' => 0, 'data' => $cert_info];
	}

	public static function certHint()
	{
		$data = self::getCertInfo();

		if ($data['status'] == -1) {
			return $data;
		}

		$hintToTime = strtotime("-7 day", $data['data']['validTo_time_t']);

		$data = [
			'status' => 1,
			'msg' => '证书有效期至'. date('Y-m-d', $data['data']['validTo_time_t'])
		];

		if (time() >= $hintToTime) {
			$data = [
				'status' => 0,
				'msg' => '当前站点HTTPS证书还剩7天到期，请尽快处理！到期后将影响使用！！！'
			];
		}
	...
	public static function CertValidToDate()
	{
		$data = self::getCertInfo();
		...
		$data = [
			'status' => 0,
			'data' => date('Y-m-d', $data['data']['validTo_time_t'])
		];

		return $data;
	}

	public static function wechatScope()
	{
		$baseList = [
			'plugin.store-cashier.frontend.cashier.center.getPaySet'
		];

		if (in_array(request()->input('route'), $baseList)) {
