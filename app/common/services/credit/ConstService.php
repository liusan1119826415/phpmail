...
class ConstService
{
    ...
    const PERFORMANCE = 131;//业绩奖励

    const BALANCE_FREEZE = 132;//冻结余额
    ...
    {
        $shop = LangService::getLangSet('member_center');
    ...
    public function sourceComment()
    {
        $result = [
                ...
                self::SHAREHOLDER_DIVIDEND                       => \Setting::get('plugin.shareholder')['custom_name'] ?: '股东分佣',
                self::PARTNER_REWARD                             => '股东奖励',
                self::TEAM_DIVIDEND                              => \Setting::get('shop.lang.zh_cn.team_dividend.title') ?: '经销商管理',
                self::COMMISSION                                 => \Setting::get('shop.lang.zh_cn.commission.title') ?: '分销',
        ...
        foreach ($plugin_data as $key => $value) {
            $result[$key] = call_user_func($value)['name'];
        }
