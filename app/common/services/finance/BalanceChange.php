...
class BalanceChange extends Credit
{
    ...
    public function validatorData()
    {
        ...
        if ($this->new_value < 0) {
            throw new AppException("用户（{$this->memberModel->uid}）".(Setting::get('shop.lang.zh_cn.member_center.credit') ?: '余额')."（{$this->memberModel->credit2}）不足({$this->change_value})");
        }
        if (!$this->relation()) {
    ...
    public function transfer(array $data)
    {
        ...
        $data['change_value'] = $data['origin_change_value'];
...
//        event(new MessageEvent($this->memberModel->uid, $template_id, $params, $url = $news_link));
    ...
    private function sendSmsMessage()
    {
        try {
            ...
            $res = app('sms')->sendMemberRecharge($this->memberModel->mobile, $data);
            if ($res['status'] == 0) {
                \Log::debug('会员充值短信通知发送失败', $res['json']);
            }
            return true;
        } catch (\Exception $e) {
