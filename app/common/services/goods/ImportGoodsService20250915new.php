...
//                  'file' => $e->getFile()
    ...
    private function processGoodsOption($goodsOption, $productType = 1)
    {
        ...
        foreach ($goodsOption as $option) {
            ...
            // 存储唯一的材质名称（即使为空也要存储）
            if (!in_array($material, $specMaterials[$specs]['materials'])) {
                $specMaterials[$specs]['materials'][] = $material;
            }

            // 存储完整的材质+类型
            $materialKey = $material . '|' . $modelType;
            if (!in_array($materialKey, $specMaterials[$specs]['materialTypes'])) {
                $specMaterials[$specs]['materialTypes'][] = $materialKey;
            }
        }

        // 构建 specs 格式（正确的关联关系）
        $specsData = $this->buildSpecsData($specMaterials, $allSpecs);

        // 构建 option 格式
        $optionData = $this->buildOptionData($goodsOption, $specMaterials, $productType);
    ...
    private function parseOptionTitle($optionTitle)
    {
        ...
        if (strpos($optionTitle, '_') !== false) {
            ...
            // 处理多种下划线分隔的情况
            if (count($parts) >= 3) {
                // 格式：规格_尺寸_材质 如：1.2米_500*200_大班台
                $result['specs'] = trim($parts[0] ?? ''); // 1.2米
                $result['material'] = trim($parts[2] ?? ''); // 大班台
            ...
            if (preg_match('/\((.*?)\)/', $optionTitle, $matches)) {
                $modelTypeName = $matches[1];
                $result['specs'] = preg_replace('/\(.*?\)/', '', $optionTitle);
     ...
     * 构建规格树形数据（正确的关联关系）
     */
    private function buildSpecsData($specMaterials, $allSpecs)
    {
        if (empty($allSpecs)) {
            return [];
        }
        ...
        foreach ($allSpecs as $specIndex => $spec) {
            ...
            // 检查该规格是否有材质
            $hasMaterials = isset($specMaterials[$spec_origin]['materials']) &&
                !empty($specMaterials[$spec_origin]['materials']) &&
                !(count($specMaterials[$spec_origin]['materials']) === 1 &&
                    empty($specMaterials[$spec_origin]['materials'][0]));

            if ($hasMaterials) {
                // 有材质的情况：创建带children的结构
                $specItem = [
                    "id" => "SV" . $specIndex . "&SC0",
                    "parent_id" => 0,
                    "specid" => "SC0",
                ...
                // 添加该尺寸对应的材质
                foreach ($specMaterials[$spec_origin]['materials'] as $matIndex => $material) {
                    // 跳过空的材质（无下划线的情况）
                    if (empty($material)) {
                        continue;
                    }
                    ...
                    $specItem['children'][] = $childItem;
                }

                // 如果所有材质都为空，则退回到无材质结构
                if (empty($specItem['children'])) {
                    $specItem = [
                        "id" => "SV" . $specIndex . "&SC0",
                        "parent_id" => 0,
                        "specid" => "SC0",
     ...
     * 构建选项数据（正确的关联关系）
     */
    private function buildOptionData($goodsOption, $specMaterials, $productType = 1)
    {
        $optionData = [];
        $optionIndex = 0;
        ...
        foreach ($groupedOptions as $groupKey => $group) {
            ...
            if (isset($specMaterials[$specs]['materials'])) {
                // 找到材质在对应尺寸中的索引（从materials数组中查找）
                $matIndex = array_search($material, $specMaterials[$specs]['materials']);

                if ($matIndex !== false) {
                    // 找到尺寸在所有尺寸中的索引
                    $specKeys = array_keys($specMaterials);
...
//            }
//            // 2. 如果根目录没有3个文件，检查子目录（需要有括号内容）
//            /*else if (!empty($folderPattern)) {
//                \Log::debug("根目录未找到3个文件，开始查找子目录，匹配模式: {$folderPattern}");
//
//                foreach ($files as $file) {
//                    $filePath = $titleFolder . '/' . $file;
//
//                    // 如果是目录，并且目录名包含括号内的内容
//                    if (is_dir($filePath) && strpos($file, $folderPattern) !== false) {
//                        \Log::debug("找到匹配的子目录: {$file}");
//
//                        $subFiles = scandir($filePath);
//                        $subFiles = array_filter($subFiles, function($subFile) {
//                            return $subFile !== '.' && $subFile !== '..';
//                        });
//
//                        $subDwgFile = null;
//                        $subMaxFile = null;
...
//                            $subExtension = strtolower($subFileInfo['extension'] ?? '');
//
//                            if (is_file($subFilePath)) {
//                                if ($subExtension === 'dwg') {
//                                    $subDwgFile = $subFilePath;
//                                } elseif ($subExtension === 'max') {
//                                    $subMaxFile = $subFilePath;
//                                } elseif (in_array($subExtension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'])) {
//                                    $subImageFile = $subFilePath;
//                                }
//                            }
//                        }
//
//                        // 如果子目录中找到3个文件，上传并使用
//                        if ($subDwgFile && $subMaxFile && $subImageFile) {
//                            \Log::debug("在子目录找到3个文件: {$subDwgFile}, {$subMaxFile}, {$subImageFile}");
//
//                            // 上传DWG文件
//                            $res = uploadOss($subDwgFile, uniqid() . ".dwg", 0);
//                            $dwg_upload_url = $res['relative_path'] ?? "";
//
//                            // 上传MAX文件
//                            $res = uploadOss($subMaxFile, uniqid() . ".max", 0);
//                            $d3max_url = $res['relative_path'] ?? "";
//
//                            // 上传图片文件
//                            $thumb_data = $this->processSingleImage($subImageFile);
//                            break; // 找到一个匹配的目录就退出
//                        } else {
//                            \Log::debug("子目录中未找到完整的3个文件");
//                        }
//                    }
//                }
//            } */else {
...
//        }
        ...
        // 如果还是没有找到图片，使用原来的处理方式
      /*  if (empty($thumb_data['thumb'])) {
            \Log::debug("使用默认的processOptionThumb方法");
            $thumb_data = $this->processOptionThumb($title);
        }*/
