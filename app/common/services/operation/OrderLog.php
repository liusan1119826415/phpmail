...
class OrderLog extends OperationBase
{
    ...
    {
        $this->setLog('mark', $this->model->getRawOriginal('id'));
    }

    protected function special()
    {
        $this->setLog('mark', $this->model->getRawOriginal('order_id'));
        $this->setLog('field_name', '修改订单备注');
        $this->setLog('field', 'remark');
        $this->setLog('old_content',$this->model->getRawOriginal('remark'));
    ...
    protected function modifyField()
    {
        ...
        foreach ($this->recordField() as $key => $item) {
            if ($model->isDirty($key)) {
                ...
                $this->modify_fields[$key]['old_content'] = $model->getRawOriginal($key);
