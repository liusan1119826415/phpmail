...
class SaleLog extends OperationBase
{
    ...
    {
        $this->setLog('mark', $this->model->getRawOriginal('goods_id'));
    }
    ...
    protected function modifyField()
    {
        ...
        foreach ($this->recordField() as $key => $item) {
            ...
            if ($model->isDirty('ed_full') || $model->isDirty('ed_reduction')) {
                $this->modify_fields['ed_full_reduction']['old_content'] = '满'.$model->getRawOriginal('ed_full').'元立减'.$model->getRawOriginal('ed_reduction').'元';
