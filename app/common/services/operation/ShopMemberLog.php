...
class ShopMemberLog extends OperationBase
{
    ...
    protected function modifyDefault()
    {
        if (is_array($this->model)) {
            ...
            $this->setLog('mark', $this->model->getRawOriginal('member_id'));
        }
    }
    ...
    protected function modifyField()
    {
        $model = $this->model;
         ...
         if (is_null($model->getRawOriginal())) {
             return [];
         }
        ...
        foreach ($this->recordField() as $key => $item) {
            if ($model->isDirty($key)) {
                ...
                if ($key == 'level_id') {
                    $old_level_name = MemberLevel::where('id', $model->getRawOriginal('level_id'))->value('level_name');
                    ...
                    $this->modify_fields[$key]['old_content'] = ['key'=> $model->getRawOriginal($key),0=>$nick_name.'推广员权限关闭',1=>$nick_name.'推广员权限开启'];
                    $this->modify_fields[$key]['new_content'] = ['key'=> $model->{$key},0=>$nick_name.'推广员权限关闭',1=>$nick_name.'推广员权限开启'];
                } elseif ($key == 'is_black') {
                    $this->modify_fields[$key]['old_content'] = ['key'=> $model->getRawOriginal($key),0=>$nick_name.'取消黑名单',1=>$nick_name.'加入黑名单'];
                    $this->modify_fields[$key]['new_content'] = ['key'=> $model->{$key},0=>$nick_name.'取消黑名单',1=>$nick_name.'加入黑名单'];
                } else {
                    $this->modify_fields[$key]['old_content'] = $nick_name.':'.$model->getRawOriginal($key);
