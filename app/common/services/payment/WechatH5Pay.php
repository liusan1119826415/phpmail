    ...
    public function doPay($data)
    {
        ...
        if ($result['return_code'] == 'SUCCESS' && $result['result_code'] == 'SUCCESS'){
            ...
            if ($mweb_url) {
                ...
                } else if (substr($data['order_no'], 0, 2) == 'RI') {
                    // 实名认证支付
                    $url = Url::absoluteApp('personBank',['from' => 'bindRealName']);
                    $mweb_url .= '&redirect_url='.urlencode($url);

                } else {
                    $orderPay = OrderPay::where('pay_sn', $data['order_no'])->first();

                    $redirect = is_null($orderPay) ? Url::absoluteApp('member') :
                        Url::absoluteApp('orderPaySuccess',['pay'=> $orderPay->id]);

                    $mweb_url .= '&redirect_url='.urlencode($redirect);
                }
            }
