...
//        }
        ...
        if (!$data['pay_sn'] && $data['order_no']) {
            $data['pay_sn'] = $data['order_no'];
        }

        if (is_array($data['extra'])) {
            if ($data['extra']['need_mul']) {
                $data['amount'] = bcmul($data['amount'], 100, 0);
                unset($data['extra']['need_mul']);
            }
            if (!$data['auth_code'] && $data['extra']['auth_code']) {
                $data['auth_code'] = $data['extra']['auth_code'];
                unset($data['extra']['auth_code']);
            }
        ...
        if ($response['result_code'] != 'SUCCESS') {
            // todo 订单取消
            throw new AppException('微信支付失败：'.$response['return_msg'].$response['err_code_des']);
        }
    ...
    public function doRefund($out_trade_no, $totalmoney, $refundmoney)
    {
        ...
        if (\Setting::get('shop.wechat_set.is_independent')) {
