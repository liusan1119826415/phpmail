...
class PayedService
{
    ...
    public function confirmPay()
    {
        if ($this->withdrawModel->status == Withdraw::STATUS_PAYING || $this->withdrawModel->status == Withdraw::STATUS_AUDIT) {
            ...
            try {
                ...
                \Log::debug('--确认打款接口--异常', [$e->getMessage(), $e->getFile(), $e->getLine()]);
    ...
    private function _tryPayed()
    {
        switch ($this->withdrawModel->pay_way) {
            ...
            case Withdraw::CONSOL_WITHDRAW_BANK:
                $result = $this->consolWithdrawBank();
                break;
            case Withdraw::CONSOL_WITHDRAW_ALIPAY:
                $result = $this->consolWithdrawAlipay();
                break;
            case Withdraw::CONSOL_WITHDRAW_WECHAT:
                $result = $this->consolWithdrawWechat();
                break;
            case Withdraw::HUIIS_ALI:
            case Withdraw::HUIIS_WX:
            case Withdraw::HUIIS_BANK:
                $result = $this->huiisWithdrawPay();
                break;
            case Withdraw::GONG_MALL_WITHDRAW_ALI:
            case Withdraw::GONG_MALL_WITHDRAW_BANK:
                $result = $this->gongMallWithdrawPay();
                break;
            case Withdraw::INTEGRATION_PAY_SHARE_HUIFU_WITHDRAW_BANK:
                $result = $this->IntegrationPayShareHuifuWithdrawBankPay();
                break;
            case Withdraw::RENLIJIA_WITHDRAW:
                $result = $this->RenlijiaWithdrawPay();
                break;
            case Withdraw::HEMA_WITHDRAW_BANK:
                $result = $this->hemaPayWithdrawPay();
                break;
            default:
    ...
    public function huiisWithdrawPay()
    {
        $result = PayFactory::create(PayFactory::HUIIS)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function gongMallWithdrawPay()
    {
        $result = PayFactory::create(PayFactory::GONG_MALL_WITHDRAW_ALI)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function hemaPayWithdrawPay()
    {
        $result = PayFactory::create(PayFactory::HEMA_WITHDRAW_BANK)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function IntegrationPayShareHuifuWithdrawBankPay()
    {
        $result = PayFactory::create(PayFactory::INTEGRATION_PAY_SHARE_HUIFU_WITHDRAW_BANK)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function RenlijiaWithdrawPay()
    {
        $result = PayFactory::create(PayFactory::RENLIJIA_WITHDRAW)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function consolWithdrawBank()
    {
        $result = PayFactory::create(PayFactory::CONSOL_WITHDRAW_BANK)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
        ...
        //耕耘灵活用工提现要根据回调地址验证打款是否成功
    }

    private function consolWithdrawAlipay()
    {
        $result = PayFactory::create(PayFactory::CONSOL_WITHDRAW_ALIPAY)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
    ...
    private function consolWithdrawWechat()
    {
        $result = PayFactory::create(PayFactory::CONSOL_WITHDRAW_WECHAT)->doWithdraw(
            $this->withdrawModel->member_id,
            $this->withdrawModel->withdraw_sn,
            bcadd($this->withdrawModel->actual_amounts, 0, 2),
            'withdraw',
