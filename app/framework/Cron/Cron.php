...
class Cron
{
    ...
    public static function run($checkRundateOnce = true)
    {
        ...
        try {
            ...
            self::log('info',  gethostname().':'.$checkTime->getTimestamp() .' 定时任务开始');
            foreach (self::$cronJobs as $job) {
                ...
                if ($job['enabled'] && $job['expression']->isDue($checkTime)) {
                    ...
                    try {
                        self::log('info',  gethostname().':'.$checkTime->getTimestamp().':'.$job['name'] .' start');

                        if (!self::lockJob($job['name'])) {
                            // 避免重复运行
                            self::log('info',  gethostname().':'.$checkTime->getTimestamp().':'.$job['name'] .' running');
                            continue;
                        }
                        $return = $job['function']($checkTime->getTimestamp());
                        if (DB::transactionLevel() > 0) {
                            throw new \Exception('事务未关闭');
                        }
                        self::unlockJob($job['name']);
                        self::log('info',  gethostname().':'.$checkTime->getTimestamp().':'.$job['name'] .' finish');
                    } catch (\Exception $e) {
                        ...
                        self::log('error', get_class($e) . ' in job :'.$checkTime->getTimestamp().':' . $job['name'] . ': ' . $e->getMessage() . "\r\n" . $e->getTraceAsString());
                    }
            ...
            self::log('info',  gethostname().':'.$checkTime->getTimestamp() .' 定时任务结束');
