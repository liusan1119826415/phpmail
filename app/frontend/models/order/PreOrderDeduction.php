...
use app\frontend\modules\deduction\amount\CoinIntegerOrderDeductionAmount;
use app\frontend\modules\deduction\amount\IntegerOrderDeductionAmount;
use app\frontend\modules\deduction\amount\OrderDeductionAmount;
...
//        dump($this->getMinDeduction()->getMoney(),$this->getUsablePoint()->getMoney(),$this->getUsableFreightDeduction()->getMoney());
    ...
    public function getUsableFreightDeduction()
    {
        if (!isset($this->usableFreightDeduction)) {
            ...
            $memberMaxUsableCoin = bcmul($this->getMemberCoin()->getMaxUsableCoin()->getMoney(), 100) / 100;
    ...
    public function getUsablePoint()
    {
        ...
        if (!isset($this->usablePoint)) {
            ...
            //如果资产设置的转化比例小数点过低会造成资产小数位数超过两位,这样会造成 抵扣值和抵扣金额不对应,所以这里直接舍弃两位小数之后的数
            $memberMaxUsableCoin = bcmul($this->getMemberCoin()->getMaxUsableCoin()->getMoney(), 100) / 100;
...
//            }
            ...
            switch ($this->getDeduction()->getAffectDeductionAmount()) {
                case 'integer'://金额整数抵扣
                    $amountHandle = new IntegerOrderDeductionAmount($this,$amount);
                    break;
                case 'coin_integer'://虚拟币整数抵扣
                    $amountHandle = new CoinIntegerOrderDeductionAmount($this,$amount);
                    break;
                default:
                    $amountHandle = new OrderDeductionAmount($this,$amount);
                    break;
            }
            $amount = $amountHandle->getAmount();
    ...
    public function getOrderGoodsDeductionAmount()
    {
        ...
        if (($max_deduction = $this->getMaxDeduction()->getMoney()) == 0) {
            return 0;
        }
        $first_amount = $this->getMaxOrderGoodsDeduction()->getMoney() / $max_deduction;
        $first_amount = is_finite($first_amount) ? $first_amount : 0;
        $amount = $first_amount * $this->amount;
...
//        $amount = ($this->getMaxOrderGoodsDeduction()->getMoney() / ($this->getMaxDeduction()->getMoney()+$this->getMaxDispatchPriceDeduction()->getMoney())) * $this->amount;
    ...
    public function isChecked()
    {
        if (!isset($this->isChecked)) {
            ...
            if ($this->mustBeChecked()) {
                ...
                in_array($this->order->plugin_id, [0,92,44,120])){//no_deduction_ids 新添参数,状态为空和设置为1 则开启默认抵扣按钮
    ...
    public function beforeSaving()
    {
        ...
        $this->getMemberCoin()->consume($this->newCoin()->setMoney($this->amount), ['order_sn' => $this->order->order_sn,'order_id' => $this->order->id]);
        $this->code = (string)$this->code;
        $this->name = (string)$this->name;
        $this->amount = sprintf('%.2f', $this->amount);
        $this->coin = sprintf('%.2f', $this->coin);
