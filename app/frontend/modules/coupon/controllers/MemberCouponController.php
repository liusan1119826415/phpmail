    ...
    public function couponsOfMemberByStatusV2()
    {
        ...
        switch ($status) {
            case self::NOT_USED:
                ...
                $search_array[Coupon::TYPE_OVERDUE] = '快过期';
                break;
            case self::OVERDUE:
...
//            }
    ...
    protected function getTimeLimit($v)
    {
        ...
        if ($v['used'] == MemberCoupon::USED) { //已使用
            ...
            } elseif($v['belongs_to_coupon']['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE){
                $start=strtotime($v['get_time']) + $v['belongs_to_coupon']['wait_days'] * 86400;
                $end = $start+$v['belongs_to_coupon']['time_days'] * 86400;
                if ($now < $end&&$now>$start) { //优惠券在有效期内
                    $v['api_status'] = self::NOT_USED;
                    $v['start'] =date('Y-m-d', $start); //前端需要起止时间
                    $v['end'] = date('Y-m-d', $end); //前端需要起止时间
                } else { //优惠券在有效期外
                    $v['api_status'] = self::OVERDUE;
                }
    ...
    public function couponsForMember()
    {
        ...
        foreach ($coupons['data'] as &$item) {
            $item['has_many_member_coupon_count'] = MemberCoupon::uniacid()->select('id')->where('coupon_id', $item['id'])->count();
        }
    ...
    public static function handleAvailableCoupons($coupons)
    {
        ...
        foreach ($coupons as $v) {
            ...
            if(count(array_unique(explode(',',$v['gettimes'])))>1&&in_array($v['belongs_to_coupon']['time_limit'], [Coupon::COUPON_SINCE_RECEIVE,Coupon::COUPON_SINCE_RECEIVE_CAN_USE])){
...
//            $coupons[$k]['belongs_to_coupon']['discount'] = intval($coupons[$k]['belongs_to_coupon']['discount']);
    ...
    public function getGoodsCouponByCouponId()
    {
        ...
        if ($coupon['time_limit'] == Coupon::COUPON_SINCE_RECEIVE) {
        ...
        if ($coupon['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE) {
            $coupon['time_start'] = date('Y-m-d H:i:s', time()+$coupon['wait_days']*86400);
            if ($coupon['time_days'] == 0) {
                $times = 86400 * 36500;
                $coupon['time_end'] = date('Y-m-d H:i:s', time() + $times);
            } else {
                $times = 86400 * ($coupon['time_days']+$coupon['wait_days']);
                $coupon['time_end'] = date('Y-m-d H:i:s', time() + $times);
            }
        }
    ...
    public function getDetail()
    {
        ...
        $record = MemberCoupon::uniacid()->with(['member', 'belongsToCommonCoupon' => function ($query) {
            return $query->select(['id', 'name', 'coupon_method', 'deduct', 'discount', 'enough', 'use_type', 'category_ids', 'categorynames',
                ...
                'money', 'credit', 'plugin_id', 'content', 'is_complex','wait_days','coupon_img','scope_content_diy','usage_rules','coupon_discount_show']);
        ...
        if ($record->belongsToCommonCoupon->usage_rules) {
            $rule = $record->belongsToCommonCoupon->usage_rules;
        } else {
        ...
        if($coupon_set['coupon_show'] == 1&&in_array($record['belongsToCommonCoupon']['time_limit'],[Coupon::COUPON_SINCE_RECEIVE,Coupon::COUPON_SINCE_RECEIVE_CAN_USE])){
        ...
        $data = [
            ...
            'effective_time_str' =>  $record->effective_time_str,
        ];

        if (!empty($data['coupon']->scope_content_diy)) {
            $data['coupon']['coupon_type_name'] = $data['coupon']->scope_content_diy;
        }
        ...
        if (app('plugins')->isEnabled('package-deliver-write-off')) {
            $data['package_deliver_write_off'] = \Yunshop\PackageDeliverWriteOff\services\SettingService::couponWriteOff($record->belongsToCommonCoupon->use_type);
        }
    ...
    private function getSearchArray($arr)
    {
        ...
        if (!miniVersionCompare('1.1.144') || !versionCompare('1.1.144')) {//小程序更新比较慢，先使用旧版本
