...
class ShareCouponController extends ApiController
{
    ...
    protected function handleCoupon($data)
    {
        ...
        }elseif($data['time_limit'] == Coupon::COUPON_SINCE_RECEIVE_CAN_USE){//时间限制类型是"领取x天后n天内有效"
            $start=$now+($data['wait_days'] * 24 * 3600);
            $end = $start + ($data['time_days'] * 24 * 3600);
            $data['time_start']=$start==$now?'':$start;
            $data['time_end']=$start==$end?'不限时间使用':$end;
        } else {
