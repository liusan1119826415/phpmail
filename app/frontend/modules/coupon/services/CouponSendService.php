...
class CouponSendService
{
    ...
    protected $effective_time = 0; // 生效时间
...
//dd($data);
    ...
    public function sendCouponToMember($memberId, $couponId, $get_type = '0', $relation = '', $transferId = '',$return_model = false,$effective_time = 0)
    {//新发放优惠券接口
        if (empty($memberId) || !is_numeric($memberId)) {
        ...
        if (!empty($this->effective_time)) {
            $this->get_time = $this->effective_time;//有生效时间的，获取时间也改为生效时间
        }
        ...
        if ($return_model) {//需要返回模型
            return $this->updateMemberCoupon($data[0], $log_data[0]);
        }
        return $this->updateMemberCoupons($data, $log_data);
    }
    ...
    /**
     * todo 此方法用于需要返回会员优惠券模型时调用，单张发放
     * @param array $data
     * @param array $log_data
     * @return MemberCoupon|null
     */
    protected function updateMemberCoupon(array $data, array $log_data)
    {
        try {
            if (empty($data)) {
                return null;
            ...
            DB::beginTransaction();
            $couponLog = (new CouponLog())->fill($log_data);
            if (!$couponLog->save()) {
                throw new \Exception('优惠券发放记录保存失败');
            }
            $memberCoupon = (new MemberCoupon())->fill($data);
