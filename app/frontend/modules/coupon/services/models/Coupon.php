...
use app\frontend\modules\coupon\services\models\TimeLimit\SinceReceiveCanUse;
    ...
    private function getTimeLimitInstance()
    {
        switch ($this->memberCoupon->belongsToCoupon->time_limit) {
            ...
            case DbCoupon::COUPON_SINCE_RECEIVE:
                ...
                break;
            case DbCoupon::COUPON_SINCE_RECEIVE_CAN_USE:
                return new SinceReceiveCanUse($this);
            default:
//                if (config('app.debug')) {
...
//                }
    ...
    public function activate()
    {
        ...
        //这里添加单张使用优惠券缓存，缓存是为了分单也只能使用一张
        if (!$this->getMemberCoupon()->belongsToCoupon->is_complex) {
            MemberCouponService::setMemberCouponsSelect($this->getMemberCoupon());
        }
    }
    ...
    public function unique()
    {
        ...
        $memberCoupons = MemberCouponService::getCurrentMemberCouponSelect();
        if ($memberCoupons->isEmpty()) {
            return true;
        }

        foreach ($memberCoupons as $select) {
            if ($select->selected == true && $select->coupon_id == $this->getMemberCoupon()->coupon_id) {
        ...
        //获取已中的优惠券
//        $selects = MemberCouponService::getCurrentMemberCouponSelectCache($this->getPreOrder()->belongsToMember);
//        if ($selects->isEmpty()) {
//            return true;
//        }
//        foreach ($selects as $select) {
//            if ($select->coupon_id == $this->getMemberCoupon()->coupon_id) {
//                return false;
//            }
//        }
//        return true;
...
//        });
    ...
    public function isOptional()
    {
        ...
        //已过期
        if ($this->getMemberCoupon()->is_expired == 1) {
            trace_log()->coupon("优惠券{$this->getMemberCoupon()->id}", '已过期');
            return false;
        }
        //未生效
        if (!empty($this->getMemberCoupon()->effective_time) && $this->getMemberCoupon()->effective_time > time()) {
            trace_log()->coupon("优惠券{$this->getMemberCoupon()->id}", '未生效');
            return false;
        }
        //未使用
