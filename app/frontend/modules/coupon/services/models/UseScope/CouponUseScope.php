...
abstract class CouponUseScope
{
    ...
    protected function getOrderGoodsOfUsedCoupon()
    {
        ...
        if (!$this->orderGoods->isEmpty()) {
            $this->orderGoods = $this->orderGoods->filter(function ($goods) {
                $goodsCoupon = $this->coupon->getPreOrder()->orderGoods->where('goods_id', $goods->goods_id)->first()->goodsCoupon;
                if (!empty($goodsCoupon['no_use'])) {
                    return false;
                }
                return true;
