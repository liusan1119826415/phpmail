...
class PreOrderAddress extends OrderAddress
{
    ...
    protected function _init()
    {
        ...
        // 先用 fill 填充已有字段
        $this->fill($orderAddress->toArray());

        // 手动添加 project_name 和 address_id
        $this->setAttribute('project_name', $orderAddress->project_name);
        $this->setAttribute('address_id', $orderAddress->address_id);
        $this->setAttribute('address1', $orderAddress->address1);
        /*  $this->setAttribute('province', $orderAddress->province);
          $this->setAttribute('city', $orderAddress->city);
          $this->setAttribute('district', $orderAddress->district);*/
    }
    ...
    protected function getOrderAddress()
    {
        ...
        if (!isset($this->memberAddress)) {
            ...
            $this->memberAddress = $this->_getMemberAddressV2();
     ...
     * 获取项目配置地址
     */
    private function _getMemberAddressV2()
    {

        $member_address = $this->getProjectAddress();
        $mAddress = app(MemberAddressRepository::class)->modelInstance()->where('uid', \YunShop::app()->getMemberId())->where('project_id', request()->input('project_id'))->where('isdefault', 1)->first();

        $orderAddress = new OrderAddress();
        if ($mAddress) {
            /*$mAddress->username = $member_address['realname'];
            $mAddress->mobile = $member_address['phone'];
            $mAddress->province = $member_address['province_id'];
            $mAddress->city = $member_address['city_id'];
            $mAddress->district = $member_address['district_id'];
            $mAddress->address = $member_address['address'];
            $mAddress->save();*/
        } else {
            $address_data = [
                'uid' => \YunShop::app()->getMemberId(),
                'username' => $member_address['realname'],
                'project_id' => request()->input('project_id'),
                'mobile' => $member_address['phone'],
                'province' => $member_address['province_id'],
                'city' => $member_address['city_id'],
                'district' => $member_address['district_id'],
                'address' => $member_address['address'],
                'uniacid' => \YunShop::app()->uniacid,
            ];
            $mAddress = app(MemberAddressRepository::class)->modelInstance();
            $mAddress->setRawAttributes($address_data);
            $validator = $mAddress->validator($mAddress->getAttributes());
            $mAddress->save();
        }

        if ($mAddress) {
            $address = Address::whereIn('id', [$mAddress->province, $mAddress->city, $mAddress->district])->pluck('areaname')->toArray();
            $orderAddress->mobile = $mAddress->mobile;
            $orderAddress->province_id = $mAddress->province;
            $orderAddress->city_id = $mAddress->city;
            $orderAddress->district_id = $mAddress->district;
            $orderAddress->address = implode(' ', [$address[0], $address[1], $address[2], $mAddress->address]);
            $orderAddress->address1 = $mAddress->address;
            $orderAddress->project_name = $member_address['project_name'];
            $orderAddress->realname = $mAddress->username;
            $orderAddress->province = $address[0];
            $orderAddress->city = $address[1];
            $orderAddress->district = $address[2];
            $orderAddress->address_id = $mAddress->id;
        } else {


            $orderAddress->order_id = $this->order->id;
            $orderAddress->mobile = $member_address['phone'];
            $orderAddress->province_id = $member_address['province_id'];
            $orderAddress->city_id = $member_address['city_id'];
            $orderAddress->district_id = $member_address['district_id'];
            $orderAddress->address1 = $member_address['address'];
            $orderAddress->address = implode(' ', [$member_address['province'], $member_address['city'], $member_address['district'], $member_address['address']]);
            $orderAddress->project_name = $member_address['project_name'];
            $orderAddress->realname = $member_address['realname'];
            $orderAddress->province = $member_address['province'];
            $orderAddress->city = $member_address['city'];
            $orderAddress->district = $member_address['district'];
            $orderAddress->address_id = $mAddress->id;
    ...
    {
        $address_data = json_decode(urldecode($this->order->getRequest()->input('address', '[]')), true);
        $project_id = request()->input('project_id');
        $project = Project::where('id', $project_id)->first();
        if ($address_data) {
            $address_data['project_name'] = $project->name;
            return $address_data;
        } else {
            $address = Address::whereIn('id', [$project->province_id, $project->city_id, $project->district_id])->pluck('areaname')->toArray();
            return [
...
//        }
    ...
    public function save(array $options = [])
    {
        ...
        unset($this->project_name, $this->address_id, $this->address1);
