...
class IncomeWithdrawController extends ApiController
{
    ...
    private function needSign()
    {
        ...
        if ($plugin_set['withdraw_sign_role_type']) {
            ...
            $select_cid = CreateService::selectCid($uid);
            if ($select_cid) {
                $scene_list = Scene::uniacid()->where(['level_id'=>$level_id,'scene_type'=>2,'cid'=>$select_cid,'status'=>1])->get();
            }
    ...
    private function incomeType($incomeType)
    {
        ...
        switch ($incomeType) {
            ...
            case 'areaDividend':
                if(\Setting::get('withdraw.areaDividend.withdraw_type')==1){
                    return 'areaDividend';
                }
                return 'default';
            case 'merchant':
                if(\Setting::get('withdraw.merchant.withdraw_type')==1){
                    return 'merchant';
                }
                return 'default';
            default:
...
//        $incomeModel = Income::getIncomeInMonth($search)->where('member_id', \YunShop::app()->getMemberId())->get();
    ...
    {
        if (miniVersionCompare('1.1.146') && versionCompare('1.1.146')) {
            $incomeWithdrawMode = app('WithdrawButton')->getIncomeWithdrawButton(request()->income_type?:'');
            if ($incomeWithdrawMode) {
                return $this->successJson('获取数据成功!', [
                    'service_switch' => Setting::get('withdraw.income.service_switch') ?: 0,
                    'button' => $incomeWithdrawMode
                ]);
            }
            return $this->errorJson('未检测到数据!');
        }
