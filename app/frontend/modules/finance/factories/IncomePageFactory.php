...
class IncomePageFactory
{
     ...
     * @return string
     */
    public function getValidateAppUrl()
    {
        $url = $this->_income->getAppUrl();
        if ($url == 'baseRewardTemplate') {
            preg_match('/\.(.*?)\./',$this->_income->dataUrl(),$match);
            $url = $url . '/' . $match[1];
        }
        return $url;
    }
    ...
    public function getIncomeData($level = true)
    {
        ...
        return [
            'url' => $this->_income->getAppUrl(),
            'icon' => yz_tomedia(static_url('yunshop/designer/extension/' . $this->_income->getIcon())),
            'mark' => $this->getMark(),
            'title' => $this->getTitle(),
    ...
    private function getSet()
    {
        ...
        switch ($type) {
            case 5:
                $res = \Setting::get("popularize.wap." . $type_name);
                break;
            case 1:
                $res = \Setting::get("popularize.wechat." . $type_name);
                break;
            case 2:
                $res = \Setting::get("popularize.mini." . $type_name);
                break;
            case 9:
                $res = \Setting::get("popularize.app." . $type_name);
                break;
            case 8:
                $res = \Setting::get("popularize.alipay." . $type_name);
                break;
            default :
                $res = \Setting::get("popularize.wechat." . $type_name);
                break;
    ...
    private function pageData(): array
    {
        ...
        if (method_exists($this->_income, 'dateTimeButton')) {
            $data['date_time'] = intval($this->_income->dateTimeButton());
        }
        if (method_exists($this->_income, 'dataUrl')) {
            $data['data_url'] = $this->_income->dataUrl();
        }
        return $data;
    }
