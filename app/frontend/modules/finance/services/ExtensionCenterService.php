...
class ExtensionCenterService
{
    ...
    public static function getIncomePage()
    {
        ...
        $extension_basic_set = ['promoted_orders_show_phone' => Setting::get('popularize.basic.promoted_orders_show_phone') ?: '0'];
        ...
        return [
            ...
            'extension_basic_set' => $extension_basic_set,
    ...
    protected static function button()
    {
        ...
        $income_name = $lang_set['zh_cn']['income']['income_name']?:'收入';
        $withdraw_name = $lang_set['zh_cn']['income']['name_of_withdrawal']?:'提现';
...
//            ->whereIn('incometable_type', $incomeConfig);
        ...
        $income  = Income::uniacid()
            ->select(DB::raw('SUM(IF(status=1 and pay_status=2,amount,0)) as withdraw,'
...
//            ->whereIn('incometable_type', $incomeConfig)
        ...
        $income_name = $langData['income']['income_name']?:'收入';
        $withdraw_name = $langData['income']['name_of_withdrawal']?:'提现';
        return [
            [
                'title' => '累计'.$income_name,
                'url' => 'incomedetails',
                'mini_url' => '/packageA/member/extension/incomedetails/incomedetails',
                'value' => $total_amount ? : 0
            ],
            [
                'title' => '已'.$withdraw_name.$langData['income']['income_name']?:'收入',
                'url' => 'presentationRecord',
                'mini_url' => '/packageA/member/presentationRecord_v2/presentationRecord_v2?orderType=extension',
                'value' => $income &&$income->withdraw ? $income->withdraw : 0
            ],
            [
                'title' => '未'.$withdraw_name.$langData['income']['income_name']?:'收入',
                'url' => 'withdrawal',
                'mini_url' => '/packageA/member/withdrawal/withdrawal',
                'value' => sprintf("%01.2f", $un_withdraw)],
            [
                'title' => $withdraw_name.'中'.$langData['income']['income_name']?:'收入',
                'url' => 'presentationRecord',
                'mini_url' => '/packageA/member/presentationRecord_v2/presentationRecord_v2?orderType=extension',
...
//            ->where('child_time','>',0)
    ...
    {
        $plugin = app('MemberCenter')->getCenterAllData();
    ...
    {
        $order = Order::uniacid()
            ->select('yz_order.id','order_sn','goods_price','price','create_time','status','yz_order.uid')
            ->join('yz_member_children',function ($join) {
                $join->on('yz_order.uid','yz_member_children.child_id')
                    ->where('yz_member_children.member_id',self::$memberId)
                    ->where('level',1);
            })
            ->with(['belongsToMember:uid,mobile,nickname,avatar'])
...
//            ->where('child_time','>',1)//直推
        ...
        if ($uid) {
            ...
            foreach ($data['data'] as &$value) {
                $value['avatar'] = $value['avatar'] ? yz_tomedia($value['avatar']) : '';
                $value['order_count'] = 0;
                $value['order_price'] = 0;
