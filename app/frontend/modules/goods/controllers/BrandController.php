...
class BrandController extends ApiController
{
    ...
    public function getBrandGoods()
    {
        ...
        $list = Goods::select('id', 'id as goods_id', 'title', 'thumb', 'price', 'market_price','plugin_id','has_option')
            ->with(['hasManyOptions'])
            ->where("status", 1)
            ->where(function($query) {
                //$query->where("plugin_id", 0)->orWhere('plugin_id', 40)->orWhere('plugin_id', 92);
                $query->whereIn("plugin_id", [0,40,92,44,120]);
            })->where('brand_id', $id)->orderBy('display_order', 'desc')
            ->paginate(20);

        $list->map(function (Goods $good) {
            $good->append(['Vip_level_status']);
            $good['min_price'] = $good->price;
            if ($good->has_option && $good->hasManyOptions->isNotEmpty()) {
                $good['min_price'] = $good['price'] = $good->hasManyOptions->min('product_price');
            }
            return $good;
        });
        $list = $list->toArray();
