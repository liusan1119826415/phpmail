...
use app\common\services\goods\GoodsPriceShow;
...
use Yunshop\PointsPriceDisplay\services\PointsPrice;
...
use app\common\models\goods\GoodsSearchHistory;
use app\frontend\modules\goods\services\ImgSearchService;
class GoodsController extends ApiController
{
    ...
    public function __construct()
    {
     ...
     //猜你喜欢
    public function lovely()
    {
        $lovely_list = Goods::select("id","title","thumb")->with(['hasManyOptions'=>function($query){
        ...
        return $this->successJson('ok',$lovely_list);
    }
    ...
    {
        $start_time = time();
        $imgSearchService = new ImgSearchService();
        $base64Data = request()->input('base64Data');
        $page = request()->input('page', 1); // 获取分页参数（如果有）
        $goods_ids = request()->input('goods_ids', []); // 获取 goods_ids 参数（如果有）
        $member_id = \YunShop::app()->getMemberId();

        // 如果没有提供文件但提供了 goods_ids，则使用它们进行分页
        if (!$base64Data && !empty($goods_ids)) {
            return $this->successJson('ok',$imgSearchService->paginateGoods($goods_ids, $page));
        }
        //记录用户搜索记录
        ...
        if ($base64Data) {
            //检查当天搜索次数
            $number = GoodsSearchHistory::where('uid', $member_id)->whereBetween('created_at', $imgSearchService->getToday())
                ->count();
            if ($number >= 100) {
                return $this->errorJson('服务器繁忙请稍后再试.');
            }

            GoodsSearchHistory::create(['uid' => $member_id, 'thumb' => ""]);
           // \Log::debug("==========start_time_one=========",time()-$start_time);
            $step_start = time();
            $res = $imgSearchService->uploadAndSearch($base64Data);
            //\Log::debug("==========start_time_two=========",time()-$step_start);

            if ($res['code'] == 0) {
                $step_start = time();
                $imgSearchService->batchUpdateDistances($res['data']);
                $data = collect($res['data']);
               // \Log::debug("==========start_time_three=========",time()-$step_start);
                // 提取排序后的 ID
                $step_start = time();
                $goods_ids = $data->pluck('goods_id')->unique()->all();
                $data = $imgSearchService->paginateGoods($goods_ids, $page);
                //\Log::debug("==========start_time_four=========",time()-$step_start);
                // 返回分页结果
                return $this->successJson('ok',$data);
            } else {
                return $this->errorJson('图片搜索失败.');
            }
        }

        return $this->errorJson('图片上传失败或没有提供图片.');
    }
    ...
    public function searchGoods()
    {
        ...
        //没办法这里多加一个参数判断是否从优惠券过来搜索商品如果是则不限制商品类型。出新接口需要一个专门做优惠券使用的跳转
        //todo 不出新页面会存在的问题的导致插件里的商品显示的样式和标准的一样，可能会出现按商城标准下单
        if (!$requestSearch['couponid']) {
            if ($plugin_id) {
                if (is_array($plugin_id)) {
                    $build->whereIn('plugin_id', $plugin_id);
                } else {
    ...
    public function getGoodsBrandList()
    {
        ...
        $build = $build->orderBy($order_field, $order_by)->orderBy('id', 'desc')->paginate(20);
        $build->transform(function ($item) {
            $item->append('vip_level_status');
            return $item;
        });
        $goodsList = $build->toArray();
//        $goodsList->vip_level_status;
        ...
        // 爱心值价格显示
        if (app('plugins')->isEnabled('love-price-display')) {
            $goodsList['data'] = \Yunshop\LovePriceDisplay\services\LovePrice::searchGoodsData($goodsList['data']);
        }
    ...
    public function getGoodsSaleV2($goodsModel, $member)
    {
        ...
        if ($goodsModel->hasOneSale->all_point_deduct && $goodsModel->hasOneSale->has_all_point_deduct) {//商品设置
            $data['name'] = $lang['member_center']['credit1'] ? $lang['member_center']['credit1'] . '全额抵扣' : '积分全额抵扣';
            $data['key'] = 'all_point_deduct';
            $data['type'] = 'string';
            $data['value'] = '可使用' . $goodsModel->hasOneSale->all_point_deduct . '个' . ($lang['member_center']['credit1'] ? $lang['member_center']['credit1'] . '全额抵扣购买' : '积分全额抵扣购买');
            array_push($sale, $data);
            $data = [];
        }
        ...
        if ($goodsModel->hasOneSale->award_balance) {
            $data['name'] = $lang['member_center']['credit'] ?: '余额';
        ...
        $data['name'] = $lang['member_center']['credit1'] ?: '积分';
        $data['key'] = 'point';
        $data['type'] = 'array';
...
//        }
    ...
    public function getRoom()
    {
        ...
        $rooms = Room::select('yz_room.*', 'yz_room_record_file.id as back_id', 'yz_room_record_file.cover as back_cover')
            ->where(function ($querys) {
                $querys->whereIn('status', [2, 3])
                    ->when(\Setting::get('plugin.room_set_basic')['goods_show_room'] != 1, function ($query1) {
        ...
        foreach ($rooms as $key => $val) {
            ...
            $room[$key]['cover'] = isset($val->back_id) ? yz_tomedia($val->back_cover) : yz_tomedia($val->cover);
            ...
            if ($val->status == 2) {
                ...
                if ($val->back_cover) {
                    $room[$key]['cover'] = yz_tomedia($val->back_cover);
                }
            }
        }
        $json = $rooms->toArray();
    ...
    private function setGoodPoints($goodsModel)
    {
        ...
        return $tradeGoodsPointsServer->getPoint($points, $goodsModel->price, $goodsModel->cost_price, $goodsModel->id, 0);
    }
    ...
    {
        $tag_list = SearchFiltering::getAllFiltering();
        ...
        foreach ($tag_list as $key => $value) {
            ...
            array_unshift($item, $tag_data);
            $tag_list[$key]['value'] = $item;
        }
        $data['tag_list'] = $tag_list;
        $data['title'] = Setting::get('goods.goods-tag-set')['title'];;

        return $this->successJson('成功', $data);
    }

    public function searchTagGoods()
    {
        $goods_model = $this->getGoodsModel();
        $requestSearch['keyword'] = \YunShop::request()->keyword;
        $goods_ids = [];
        //同个规格组搜索用或，不同规格组用并
        if (!empty(\YunShop::request()->filter_ids)) {
            $filter_ids = explode(',', rtrim(\YunShop::request()->filter_ids, ','));
            //过滤掉0(全部)
            $filter_ids = array_filter($filter_ids, function ($value) {
                return $value !== 0;
            });
            $filter_item = SearchFiltering::selectRaw('GROUP_CONCAT(id) as ids')->whereIn('id', $filter_ids)->groupBy('parent_id')->get();

            foreach ($filter_item as $item) {
                $filter_ids = explode(',', $item->ids);
                $goods_ids_arr[] = $goods_filters = GoodsFiltering::whereIn('filtering_id', $filter_ids)->get()->pluck('goods_id')->toArray();
                //如果有一个为空则搜索不到商品
                if (empty($goods_filters)) {
                    $goods_ids_arr = [];
                    break;
                }
            }

            $count = count($goods_ids_arr);
            if ($count == 1) {
                $goods_ids = array_unique($goods_ids_arr[0]);
            } elseif ($count > 1) {
                // 用于存放结果的一维数组
                $goods_ids = [];
                // 用于记录出现次数的数组
                $valueCounts = [];
                // 遍历二维数组
                foreach ($goods_ids_arr as $subArray) {
                ...
                // 遍历统计数组，找出出现次数等于$count的元素
                foreach ($valueCounts as $value => $count_amount) {
        ...
        //增加默认搜索不隐藏的商品
        $requestSearch['is_hide'] = 1;
        $build = $goods_model->select(['thumb', 'title', 'id'])->SearchList($requestSearch)
            ->where("yz_goods.status", 1)
            ->orderBy('yz_goods.display_order', 'desc')
            ->orderBy('yz_goods.id', 'desc')
            ->orderBy('yz_goods.is_recommand', 'desc');
        if (!empty($goods_ids)) {
            $build->whereIn("yz_goods.id", $goods_ids);
        }
        $list = $build->paginate(20);
