...
class GoodsPosterController extends ApiController
{
    ...
    public function newGoodsPoster()
    {
        ...
        if ($this->goodsModel->vip_level_status['status'] == 1) {
            $goodsPrice = $this->goodsModel->vip_level_status['word'];
        }
    ...
    private function mergeGoodsImage($target, $thumb)
    {
        ...
        $img = imagecreatefromstring(\Curl::to($thumb)->withOption('FOLLOWLOCATION', 1)->get());
        $width = imagesx($img);
        $height = imagesy($img);
    ...
    private function mergeLogoImage($target)
    {
        ...
        $img = imagecreatefromstring(\Curl::to($logo)->withOption('FOLLOWLOCATION', 1)->get());
        if (!$img) {
            $this->shopText['left'] = 45;
            return $target;
        }
