...
class Comment extends \app\common\models\Comment
{
    ...
    {
        $commentConfig = CommentConfig::getSetConfig();
        $top_sort = $commentConfig->top_sort;
        //显示默认好评评论:1-显示，0-关闭
        $is_default_good_reputation_show = (int)$commentConfig->is_default_good_reputation_show ?: 0;
        $with = [
        ...
        $model = self::select(
            ...
            ->uniacid()
            ->with($with)
            ->where(function ($query) use ($is_default_good_reputation_show) {
                if (!$is_default_good_reputation_show) {
