...
abstract class GoodsDetailBaseService
{
    ...
    const IndependentModel = 0; //独立位
    const HeadModel = 1;  //首位
    const ExtensionModel = 2; //延伸位
    const TailModel = 3;  //尾位
    const ScreenModel = 5; //屏风
    ...
    public function getData()
    {
        ...
        //改为使用前面绑定的商品模型进行查询
        $class_name = get_class($this->goods_model);
        $this->goods_model->vip_price_show = (new $class_name)::find($this->goods_model->id)->vip_price;
        //        $this->goods_model->vip_price_show = \app\common\models\Goods::find($this->goods_model->id)->vip_price;

        // 获取所有 option_id
        $optionIds = $this->goods_model->hasManyOptions->pluck('id')->toArray();

         // 一次性查询所有相关的 GoodsOptionModel 数据
        $modelParams = GoodsOptionModel::whereIn('option_id', $optionIds)->orderBy('sort','asc')->get();

        // 预处理，将数据按 option_id 分组，减少后续查询
        $modelParamsGrouped = $modelParams->groupBy('option_id');

         // 遍历并处理 unserialize
        foreach ($this->goods_model->hasManyOptions as $item) {
            // 赋值 model_param，只获取当前 option_id 对应的数据
            $item->model_param = isset($modelParamsGrouped[$item->id]) ?
                $modelParamsGrouped[$item->id]->map(function ($param) {
                    $param->default_color = !empty($param->default_color) ? unserialize($param->default_color) : [];
                    $param->select_color = !empty($param->select_color) ? unserialize($param->select_color) : [];
                    $param->map_param = !empty($param->map_param) ? unserialize($param->map_param) : [];
                   // $param->meshs_name = !empty($param->meshs_name) ? unserialize($param->meshs_name) : [];

                    return $param;
                }) : collect();
            $item->d3model = yz_tomedia($item->d3ModelUrl_weld);
            $item->cad_plan_model = yz_tomedia($item->cad_plan_model);
            $item->d3MaxUrl = yz_tomedia($item->d3MaxUrl);

        }
        $this->goods_model->goods_style = GoodsStyleRelations::where('goods_id', $this->goods_model->id)->with(['belongsToStyle'])->where('type', 1)->get()->map(function ($item) {
            return $item->belongsToStyle->name ?? null;
        })->filter(); // 如果需要去除 null 值;
        $this->goods_model->material = GoodsStyleRelations::where('goods_id', $this->goods_model->id)->with(['belongsToStyle'])->where('type', 2)->get()->map(function ($item) {
            return $item->belongsToStyle->name ?? null;
        })->filter(); // 如果需要去除 null 值;
        //实拍图
        $this->goods_model->e_catalog_pdf = yz_tomedia($this->goods_model->e_catalog_pdf);
        $this->goods_model->real_image = !empty($this->goods_model->real_image)
            ? array_map(fn($url) => yz_tomedia($url), unserialize($this->goods_model->real_image))
            : [];

        $this->goods_model->thumb_url_img = !empty($this->goods_model->thumb_url)
            ? array_map(fn($url) => yz_tomedia($url), unserialize($this->goods_model->thumb_url))
            : [];
        //获取商品分类
        $category_ids = GoodsCategory::where('goods_id',$this->goods_model->id)->pluck('category_id')->toArray();
        $category_names = Category::whereIn('id',$category_ids)->pluck('name')->toArray();
        $this->goods_model->category_names = $category_names;
        $this->detail_data = $this->detail_data->merge(collect([
            'customer_service' => $this->getCustomerBaseService(),        //获取客服
            ...
            'bottom_button' => array_values($this->bottomButton()),            //是否已收藏
            'three_model' => $this->getThreeModel(),
            'product_series' => $this->getProductSeries(),
    ...
    public function getBrandInfo()
    {
        ...
        $data = Supplier::select('id',
            'store_name',
            'logo',
            'province_id',
            'city_id')->where('id', $this->goods_model->supp_id)->first();
        $data->logo = yz_tomedia($data->logo);
        $addresses = Address::whereIn('id', [$data->province_id, $data->city_id])
            ->pluck('areaname', 'id');
        $data->provenance = $addresses[$data->province_id] . $addresses[$data->city_id];
        $data->goods_total = Goods::where('supp_id', $this->goods_model->supp_id)->count();
        $follow = SupplierFollow::where('member_id', $memberId)
            ->where('supplier_id', $this->goods_model->supp_id)->first();
        $data->isFollow = $follow ? 1 : 0;
        $list = Goods::select("id", "thumb", "title")->where('supp_id', $this->goods_model->supp_id)->limit(9)->get();
        $recommend = $list->map(function ($item) {
            return [
                'id' => $item->id,
                'title' => $item->title,
        ...
        $data->brand_recommend = $recommend;
        return $data;
    }
    ...
    {
        $related_goods_id = $this->goods_model->related_goods_id ? explode(",", $this->goods_model->related_goods_id) : [];
        $list = Goods::select("id", "title", "price", "thumb")->with(['hasManyOptions'=>function($query){
            $query->select("id","goods_id","product_price");
        }])->whereIn('id', $related_goods_id)->get();
    ...
    public function getThreeModel()
    {
        ...
        // 获取所有规格项 ID
        $specItems = $this->goods_model->hasManySpecs->pluck('specitem')->flatten();

        // 提前获取所有相关的 GoodsOption
        $specIds = $specItems->pluck('id')->toArray();
        $goodsOptions = GoodsOption::where("modelType", 0)->whereIn("specs", $specIds)->get();

        // 提取所有 option_id
        $optionIds = $goodsOptions->pluck('id')->toArray();

        // 一次查询所有 GoodsOptionModel 并分组
        $modelParams = GoodsOptionModel::whereIn('option_id', $optionIds)->get()->groupBy('option_id')->map(function ($items) {
            return $items->sortBy('sort'); // 按 sort 升序排序
        });

        // 批量处理 unserialize(default_color) 和 unserialize(select_color)
        ...
        // 预加载 ParentModel，减少重复调用
        $parentModels = [];
        foreach ($specItems as $item) {
            $parentModels[$item->id] = $this->getParentModel($item->id);
        }

        // 遍历 GoodsOption
        foreach ($goodsOptions as $option) {
            $model_data = $modelParams->get($option->id, collect()); // 获取对应 option_id 的 model_data
...
//            if ($status == 8) {
//                $modelList[$option->title] = $parentModels[$option->specs]; // 直接取缓存的 ParentModel
//            } else {
                $modelList[$option->title][] = [
                    "id" => $option->id,
                    "url" => yz_tomedia($option->d3ModelUrl_weld),
                    "img" => $img,
                    "price" => $option->product_price,
                    "position" => ["x" => 0, "y" => 0, "z" => 0],
                    "rotate" => ["x" => 0, "y" => 0, "z" => 0],
                    "blockname" => $option->block_name,
                    "size" => $option->length . "*" . $option->width . self::MM,
                    "json" => json_encode([
                        'modelType' => $modelType,
                        'single' => $option->singleType,
                        'model_data' => $model_data
                    ]),
                    "models" => $parentModels[$option->specs] // 直接取缓存的 ParentModel
                ];
         //   }
        }
    ...
    private function getParentModel($specId, $img = null)
    {
        // 一次性查询 GoodsOption 数据
        $goodsOptions = GoodsOption::where("specs", $specId)->get();

        // 获取所有 option_id
        $optionIds = $goodsOptions->pluck('id')->toArray();

        // 一次性查询 GoodsOptionModel 数据并按 option_id 进行分组
        $modelParams = GoodsOptionModel::whereIn('option_id', $optionIds)->get()->groupBy('option_id')->map(function ($items) {
            return $items->sortBy('sort'); // 按 sort 升序排序
        });

        // 处理 unserialize(default_color) 和 unserialize(select_color)
        $modelParams->transform(function ($items) {
            return $items->map(function ($item) {
                $item->default_color = !empty($item->default_color) ? unserialize($item->default_color) : [];
                $item->select_color = !empty($item->select_color) ? unserialize($item->select_color) : [];
        ...
        foreach ($goodsOptions as $option) {
            $model_data = $modelParams->get($option->id, collect()); // 取对应 option_id 的 model_data
...
//                ];
//            } else {
                $parentModel[$modelTypeInfo['name']] = [
                    "id" => $option->id,
                    "url" => yz_tomedia($option->d3ModelUrl_weld),
                    "img" => yz_tomedia($option->thumb),
                    "position" => ["x" => 0, "y" => 0, "z" => 0],
                    ...
                    "price" => $option->product_price,
                    "size" => $option->length . "*" . $option->width . self::MM,
                    "json" => json_encode($modelDataArray)
                ];
            }
       // }
    ...
    {
        if ($this->goods_model->productType == 1 && $modelType == 0) {
            return ['code' => 0, 'name' => '独立位'];
        } elseif ($this->goods_model->productType == 2 && $modelType == 0) {
            return ['code' => 0, 'name' => '独立位'];
        } elseif ($this->goods_model->productType == 2 && $modelType == 2) {
            return ['code' => 2, 'name' => '延伸位'];
        } elseif ($this->goods_model->productType == 3 && $modelType == 0) {
            return ['code' => 0, 'name' => '独立位'];
        } elseif ($this->goods_model->productType == 3 && $modelType == 1) {
            return ['code' => 1, 'name' => '首位'];
        } elseif ($this->goods_model->productType == 3 && $modelType == 2) {
            return ['code' => 2, 'name' => '延伸位'];
        } elseif ($this->goods_model->productType == 3 && $modelType == 3) {
            return ['code' => 3, 'name' => '尾位'];
        } elseif ($this->goods_model->productType == 4 && $modelType == 0) {
            return ['code' => 0, 'status'=>8,'name' => '十字型'];
        } elseif ($this->goods_model->productType == 4 && $modelType == 1) {
            return ['code' => 1, 'status'=>8,'name' => 'T字型'];
        } elseif ($this->goods_model->productType == 4 && $modelType == 2) {
    ...
    public function getLoginGoodsData()
    {
        ...
        // $this->goods_model->setRelation('deliveryAddress', $this->deliveryAddress());
    ...
    public function deliveryAddress()
    {
        ...
        //不是商城标准商品不显示
        if (!in_array($this->goods_model->plugin_id, Goods::showPluginGoods())) {
        ...
        if ($goods_set['is_delivery_address'] && $this->goods_model->type == 1) {


            $memberAddress = Member::current()->defaultAddress;
...
//                return $query->select('goods_id', 'title', 'value', 'id')->orderby('displayorder', 'asc');
//            },
...
//        }
    ...
    private function getViewType()
    {
        ...
        if (app('plugins')->isEnabled('decorate') && \Setting::get('plugin.decorate.is_open') == "1") {
            ...
            if (!empty($pc_status)) {
                ...
                $this->view_code = $template->code ?? 'goods01';
            }
        }
    ...
    public function getCustomerBaseService()
    {
        if (isset($this->customer_data)) {
            return $this->customer_data;
        }
        return $this->customer_data = $this->getCustomerService();
    }
    ...
    public function getCustomerService()
    {
        ...
        $customer_data['service_QRcode'] = $customer_data['service_QRcode'] ?: '';
        $customer_data['service_mobile'] = $customer_data['service_mobile'] ?: '';
        $customer_data['customer_open'] = intval($customer_data['customer_open']);
    ...
    public function basicInformation()
    {
        ...
        //价格显示
        $this->goods_model->append('vip_level_status');
        ...
        if (!$this->goods_model->plugin_id && $this->goods_model->advert_pic) {
            $this->goods_model->advert_pic = yz_tomedia($this->goods_model->advert_pic);
        }
        if (!$this->goods_model->plugin_id && $this->goods_model->background_pic) {
            $this->goods_model->background_pic = yz_tomedia($this->goods_model->background_pic);
        }
    ...
    {
        $model = ImageLink::uniacid()->where('goods_id', $this->goods_model->id)->first();
        $this->goods_model->offsetSet('image_link', [
            'status' => $model->status ? 1 : 0,
            'image_web_link' => $model->image_web_link ? yz_tomedia($model->image_web_link) : '',
    ...
    public function setGoodsSetting()
    {
        ...
        $this->goods_model->is_show_min_share = empty($goodsSetting->is_show_min_share) ? 0 : $goodsSetting->is_show_min_share;
        ...
        $hide_total_sales = Setting::get('goods.hide_goods_sales');
        if (in_array($this->goods_model->plugin_id, [0, 67])) {
            $hide_total_sales = $this->goods_model->hide_goods_sales_alone ? $this->goods_model->hide_goods_sales : $hide_total_sales;
        }
    ...
    public function setOption()
    {
        ...
        $this->goods_model->hasManySpecs->transform(function ($specs) {
            $specs->hasManySpecsItem->transform(function ($item) {
                ...
                $item->option_id = GoodsOption::where('specs',$item['id'])->value("id");
                return $item;
            });
        ...
        $uniqid = uniqid();
        $newSpecItem = [
            'id' => $uniqid, // 如果需要唯一标识，可设置唯一ID
            'title' => '部件',
            'type' => 2,
            'goods_id' => $this->goods_model->id, // 继承当前商品的goods_id
            'specitem' => $this->get3DModel($uniqid, $this->goods_model->hasManySpecs->pluck('specitem'))
        ];

        // 添加到 `hasManySpecs`
        $this->goods_model->hasManySpecs->push($newSpecItem);
    }
    ...
    private function get3DModel($uniqid, $specitem)
    {
        ...
        if ($specitem) {
            foreach ($specitem as $item) {
                foreach ($item as $item2) {
                    $goodsOptions = GoodsOption::select("id")->where('modelType', 0)->where("specs", $item2->id)->first();
                    $list = GoodsOptionModel::where('option_id', $goodsOptions->id)->orderBy('sort','asc')->get();

                    foreach ($list as $value) {
                        $newSpecItem[$item2->id][] = [
                            "id" => $value->id,
    ...
    {
        (new MemberHistoryController())->store(request(), true,$this->goods_model->supp_id);
    }
        ...
        if (\Setting::get('shop.coupon.is_show_coupon')) {
            ...
            foreach ($coupon as $key => $value) {
                ...
                if (in_array($this->goods_model->id, $value['goods_ids']) || $this->goods_model->id == $value['goods_ids']) {
                    $min_price = $this->goods_model->min_price ?: $this->goods_model->price;
                    if ($value['enough'] > 0 && $min_price < $value['enough']) {
                        return $goods_coupon;
                    }
                    if ($value['coupon_method'] == 1) {
                        $goods_coupon['deduct_price'] = bcsub($min_price, $value['deduct'], 2);  //立减折扣//抵扣金额
                        ...
                        $goods_coupon['deduct_price'] = bcmul($min_price, $value['discount'] / 10, 2); //打折优惠
                        $goods_coupon['coupon_method'] = $value['coupon_method'];
                        $goods_coupon['discount'] = $value['discount'];
                        $goods_coupon['deduct'] = $value['deduct'];
                    }
    ...
    final public function setMemberDiscount()
    {
        ...
        $discount_style = \Setting::get('shop.member.discount_style') ?: 0;
        ...
        if ($good_style_set == 2) {
            ...
            if ($this->goods_model->has_option && $this->goods_model->hasManyOptions) {
                ...
                $data['next_level_name'] = $option->next_vip_name;
            } else {
                $data['discount_value'] = $this->goods_model->vip_price;
                $data['next_level_price'] = $this->goods_model->next_level_price;
                $data['next_level_name'] = $this->goods_model->next_level_name;
            }


            if ($this->member->level) {
                $data['level_name'] = $this->member->level->level_name;
            } else {
                $level = \Setting::get('shop.member.level_name');
                $data['level_name'] = $level ?: '普通会员';
                if (empty($data['next_level_name'])) {
                    $data['next_level_name'] = MemberLevel::value('level_name');
                }
            }
    ...
    //底部按钮
    public function bottomButton(): array
    {
        $data = $this->getDefaultBottomButton();
        if ($data['Chat']) {
            $customer_data = $this->getCustomerBaseService();
            if (request()->type == 2) {
                if (!$customer_data['service_mobile'] && !$customer_data['service_QRcode']) {
                    if ($customer_data['cservice']) {
                        $data['Chat']['action'] = 'slot';
                    } else {
                        $data['Chat']['action'] = 'concat';
                    }
                } else {
                    $data['Chat']['action'] = 'slot';
                }
            }
            $data['Chat']['url'] = $customer_data['cservice'] ?: '';
        }
        $method = 'setBottomButton';
        app('GoodsDetail')->make('DetailsCollection')->each(function ($class) use ($method, &$data) {
            if (method_exists($class, $method)) {
                $data = $class->$method($data);
            }
        });

        if ($this->goods_model->buy_limit['status'] == 1 && $this->goods_model->buy_limit['end_time'] == 0 && $data['BuyNow']) {
            $data['BuyNow']['disabled'] = true;
            unset($data['AddCart']);
        }
        return $data;
    }

    public function getDefaultBottomButton(): array
    {
        $data = [
            'Shop' => [
                'key' => 'Shop',
        ...
        $langData = Setting::get('shop.lang')['zh_cn'];
        if ($data['BuyNow']) {
            if ($langData['goods']['buy_now'] && $langData['goods']['buy_now'] != '立即购买') {
                $data['BuyNow']['name'] = $langData['goods']['buy_now'];
            }
        }
        $button_setting = GoodsSetting::getSet()['buy_button'];
        if ($button_setting) {
            $button_setting = explode('|', $button_setting);
            if (!in_array('cart', $button_setting)) {
                unset($data['AddCart']);
            }
            if (!in_array('buy', $button_setting)) {
                unset($data['BuyNow']);
            }
        }
        return $data;
