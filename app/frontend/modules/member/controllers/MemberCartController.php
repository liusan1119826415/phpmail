...
class MemberCartController extends ApiController
{
    ...
    public function index(Request $request, $integrated = null)
    {
        ...
        foreach ($cartList as $key => $cart) {
            ...
            //购物车商品是否失效
            $stock = $cart['option_id'] ? max($cart['goods_option']['stock'],0) : $cart['goods']['stock'];
            //商品下架 || 已删除 || 商品库存不足
            $invalid = (empty($cart['goods']['status']) ||
                $cart['goods']['deleted_at'] || ($stock <= 0) || ($cart['option_id'] && !$cart['goods']['has_option']) || (!$cart['option_id'] && $cart['goods']['has_option']));

            $cartList[$key]['is_failure'] = $invalid ? 1 : 0;
            ...
            if (!empty($cart['goods_option'])) {
                ...
                if ($cart['goods_option']['stock'] <= 0) {
                    $cartList[$key]['goods']['stock_status'] = 1; //库存不足
                }

            } else if ($cart['goods']['stock'] <= 0) {
