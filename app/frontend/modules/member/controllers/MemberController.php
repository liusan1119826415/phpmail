...
use app\common\events\member\SkipLoginRequestEvent;
use app\common\models\member\MemberChildren;
use app\common\models\MemberRelation;
use app\backend\modules\order\models\Order;
use app\common\components\ApiController;
use app\common\events\member\BeforeMemberBindMobileEvent;
...
use Yunshop\VideoShop\models\CreateMemberLog;
...
class MemberController extends ApiController
{
    ...
    {
        parent::__construct();
        if (versionCompare('1.1.145') && miniVersionCompare('1.1.145')) {
            $this->publicAction[] = 'memberData';
            $this->ignoreAction[] = 'memberData';
        }
    }
    ...
    public function getIsBindAccount(Request $request)
    {
//          $code = $request->input('code');
//
//          $data = app(MemberMiniAppService::class)->getIsBindAccount($code);
          $uniacid = \YunShop::app()->uniacid;
          $data['pc_url'] = 'https://' .$_SERVER['HTTP_HOST'] ."/plugins/shop_server/home?i={$uniacid}";
    ...
    public function addAgentApply()
    {
		...
		if ($become_check == 0) {
			$sub_member_model->is_agent = 1;
			$sub_member_model->status = 2;
			$sub_member_model->agent_time = time();
			$sub_member_model->inviter = 1;
		} else {
		...
		Member::setMemberRelation($sub_member_model->member_id, $sub_member_model->parent_id);

		MemberRelation::sendGeneralizeNotify($sub_member_model->member_id);
        (new SystemMsgService())->applyNotice($member_mode,'agent_apply');
...
//            $memberService->chkAccount(\YunShop::app()->getMemberId());
    ...
    public function prepBind()
    {
        ...
        if ($member_merge_set['is_merge_save_level'] == 1) {
            //手机号
        ...
        } elseif ($member_merge_set['is_merge_save_level'] == 5){
            //alipay
        ...
        } elseif ($member_merge_set['is_merge_save_level'] == 6 && (app('plugins')->isEnabled('video-shop'))) {
            //视频号
            $video_fans = CreateMemberLog::uniacid()->where('uid', $uid)->first();
            if ($video_fans) {
                if ($memberinfo_model) {
                    $save_uid = $uid;
                    $del_uid = $memberinfo_model->uid;
        ...
        } elseif ($member_merge_set['is_merge_save_level'] == 7 && (app('plugins')->isEnabled('douyin-set'))) {
            //抖音
            $tiktok_fans = MemberTiktokModel::getFansById($uid);
            if ($tiktok_fans) {
                if ($memberinfo_model) {
                    $save_uid = $uid;
                    $del_uid = $memberinfo_model->uid;
    ...
    public function bindMobile()
    {
        ...
        $idcard = \YunShop::request()->idcard;
        $idcard_addr = \YunShop::request()->idcard_addr;
        ...
        $member_model = MemberModel::getMemberById($uid);
        event(new BeforeMemberBindMobileEvent($member_model));
        if (!empty($member_model->mobile) && $member_model->mobile == $mobile) {
            if (Redis::exists('miniMemberMobile'.$mobile)) {
                Redis::del('miniMemberMobile'.$mobile);
            }
            return $this->successJson('手机号码已绑定');
        }
        $yz_member = MemberShopInfo::getMemberShopInfo($uid);
        if ($yz_member->is_old) {
            throw new AppException('会员数据有冲突，请联系客服');
        ...
        if ($type == 2 && request()->mini_auth_mobile == 1) {//授权获取的手机号不校验
            if (Redis::get('miniMemberMobile'.$mobile)) {
                $hasCheckCode = 0;//不校验
            } else {
                return $this->errorJson('绑定超时，请重新授权手机号');
            }
        }
        if ($hasCheckCode) {
            $check_code = MemberService::checkCode();
        ...
        $member_model->idcard = $idcard ?: '';
        $member_model->idcard_addr = $idcard_addr ?: '';
        $member_shop_info = MemberShopInfo::uniacid()->where('member_id', $member_model->uid)->first();
    ...
    public function phoneMemberSave($uid, $member_info_model, $member_model, $mobile)
    {
        ...
        MemberService::bindMobileValidate(request()->type, $mobile);
        //保存修改的信息
        $merge_data = [
            'uniacid' => \YunShop::app()->uniacid,
        ...
        $member_info_model->salt = $member_info_model->salt ?: $member_model->salt;
        $member_info_model->password = $member_info_model->password ?: $member_model->password;
        //删除会员
        ...
        $exception = DB::transaction(
            function () use ($give_up_uid, $hold_uid, $member_info_model, $merge_data, $yz_member) {
                ...
                if (Schema::hasTable('yz_member_tiktok')) {
                    DB::table('yz_member_tiktok')->where('member_id', $give_up_uid)->update(['member_id' => $hold_uid]);
                }
                //统一
    ...
    public function fansMemberSave($uid, $member_info_model, $member_model, $mobile)
    {
        ...
        $member_model->salt = $member_model->salt ?: $member_info_model->salt;
        $member_model->password = $member_model->password ?: $member_info_model->password;
        //删除会员
...
//                }
    ...
    public function memberInfo()
    {
        ...
        if (app('plugins')->isEnabled('project-party')&&$project_party_set['is_balance_transfer']) {
            $res = (new HttpService())->selectMember($member_id);
            if (!$res['status']) {
                return $this->errorJson('请求服务商系统失败'.$res['msg']);
            }
            $mobile = $res['data']['member_tel'];
            $member_info = MemberModel::select(['nickname', 'avatar', 'realname'])->uniacid()->where('mobile', $mobile)->first();
            if (empty($member_info)) {
                return $this->errorJson('会员不存在');
            }
        } else {
    ...
    public function anotherShare()
    {
        ...
        $url = yzAppFullUrl(
            ...
            ['pid' => $mid, 'order_ids' => $order_ids, 'share_type' => $another_share_type,'mid'=>$mid]
        );
    ...
    public function shareinfo()
    {
        ...
        $data[0]['no_check_h5_url']='';
        $data[0]['no_check_mini_url']='';
        $relation_set = \Setting::get('member.relation');
        $relation = Relation::uniacid()->first();
        if(!$relation->become_check&&$relation->become==1&&$member_info->status==2&&$member_info->is_agent==1){
            $data[0]['no_check_h5_url']=$relation_set['no_check_h5_url']?:yzAppFullUrl('member/extension');
            $data[0]['no_check_mini_url']=$relation_set['no_check_mini_url']?:'/packageG/pages/member/extension/extension';
        }
    ...
    public function memberInviteValidate()
    {
        ...
        if(!$parent && $set['can_bind_mobile'] == 1){
            $parent = Member::uniacid()
                ->select(['uid', 'mobile','nickname', 'avatar', 'realname'])
        ...
        if ($parent) {
            ...
            MemberShopInfo::change_relation(\YunShop::app()->getMemberId(), $parent_id);
            //增加邀请码使用记录
            ...
            return $this->errorJson('搜索不到推荐人!请重新填写');
        }
    }
    ...
    public function isValidatePage(Request $request, $integrated = null)
    {
        ...
        $data = [
            ...
            'invite_page' => 0,
            'is_invite' => 0,
            'is_login' => $member_id ? 1 : 0,
            'invite_mobile' => MemberModel::getMobile($member_id) ? 1 : 0, // 是否已绑定手机号
            'bind_address' => [
                'is_bind_address' => !\YunShop::app()->getMemberId() || $addressClass::uniacid()->where('uid', \YunShop::app()->getMemberId())->where('isdefault', 1)->first() ? 1 : 0,
                'bind_address_type' => 0,
                'bind_address_page' => [],
            ],
            'need_video' => 0
        ...
        if (app('plugins')->isEnabled('phone-bill-recharge') && Setting::get('phone-bill-recharge.set.plugin_enable')) {
            $data['phone_bill_recharge_member'] = \Yunshop\PhoneBillRecharge\services\MemberPhoneTips::getData();
        }
        ...
        //子账号登录不需要绑定手机号
        if (app('plugins')->isEnabled('account-fjyx') && \Yunshop\AccountFjyx\common\SubAccountLogon::validate($member_id)) {
            $member_set['is_bind_mobile'] = 0;
        }
        ...
        if (!is_null($member_set)) {
            ...
            $data['invite_page'] = $member_set['invite_page'] ? 1 : 0;
        ...
        if (app('plugins')->isEnabled('shop-esign-v2') && app('plugins')->isEnabled('yun-sign')) {
            $shop_contract = ShopContract::uniacid()->where(['member_id'=>$member_id,'status'=>0])->first();
            $data['shop_contract'] = [
                'contract_id' => $shop_contract->contract_id,
                'cid' => $shop_contract->cid,
                'member_center_status' => Setting::get('plugin.shop_esign_v2.member_center_status')?1:0
            ];
            //子账号不用签署合同
            if (app('plugins')->isEnabled('account-fjyx') && \Yunshop\AccountFjyx\common\SubAccountLogon::validate($member_id)) {
                unset($data['shop_contract']);
            }
    ...
    public function getShopSet()
    {
        $data = [
            ...
            'can_bind_mobile' => Setting::get('shop.member.can_bind_mobile') ? "1" : "0"
        ];
        return $this->successJson('ok', $data);
    }
...
//        $this->dataIntegrated($memberCenterData->getPluginData(''), 'plugins_data');
    ...
    public function getMemberList()
    {
        ...
        $this->apiData['set'] = [
            'member_order_status' => \Setting::get('shop.relation_base.member_order_status'),
            'lang' => [
                'share_member_name' => \Setting::get('shop.lang.zh_cn.agent.share_member_name') ?: '推荐人',
            ]
        ];
//        $request = Request();
...
//        $this->dataIntegrated($this->getMyReferralParents($request), 'my_referral_parents');
    ...
    public function miniMemberMobile()
    {
        ...
        if ($data['errcode'] == 0 && $data['errmsg'] == 'ok') {
            Redis::setex(('miniMemberMobile'.$data['phone_info']['purePhoneNumber']), 300, 1);
        }
    ...
    //第三方跳转商城携带参数处理
    public function skipLoginRequest()
    {
        $extra_data = request()->extra_data;
        $member_id = \YunShop::app()->getMemberId();
        event(new SkipLoginRequestEvent($member_id, $extra_data));
        return $this->successJson('成功', ['member_id' => $member_id]);
    }

    public function decorateMemberLevel()
    {
        $member_id = \YunShop::app()->getMemberId();
        $yz_member = MemberShopInfo::where('member_id', $member_id)->with('level')->first();
        $level = $yz_member->level->level;
        $current_level = MemberLevel::select(['id','level','level_name'])->where('level', $level)->first();
        if (!$level) {
            $level = 0;
            $current_level['level_name'] = '普通会员';
        }
        $next_level = MemberLevel::where('level', '>', $level)->orderBy('level')->first();
        $member_set = Setting::get('shop.member');
        $level_after = $member_set['level_after'];
        $level_type = $member_set['level_type'];
        if ($yz_member->validity) {
            $current_level->validity_time = Carbon::now()->addDays($yz_member->validity)->format('Y-m-d');
        }
        $res = [
        ...
        switch ($level_type) {
            case 1 :
                $order_count_query = Order::uniacid()->where('uid', $member_id);
                if ($level_after) {
                    $order_count_query = $order_count_query->where('status', '>=', 1);
                } else {
                    $order_count_query = $order_count_query->where('status', '=', 3);
                }
                $order_count = $order_count_query->count();
                $res['condition'] = [
                    'need' => $next_level->order_count,
                    'now' => $order_count,
                    'type_name' => '(单数)',
                ];
                break;
            case 2 :
                $level_goods_id = $next_level->goods_id;
                if ($level_goods_id) {
                    $level_goods_id = explode(',', $level_goods_id);
                    $order_goods_order_id = OrderGoods::uniacid()->whereIn('goods_id', $level_goods_id)->pluck('order_id')->all();
                    $order_query = Order::whereIn('id', $order_goods_order_id)->where('uid', $member_id);
                    if ($level_after) {
                        $order_query = $order_query->where('status', '>=', 1);
                    } else {
                        $order_query = $order_query->where('status', 3);
                    }
                    $res['condition'] = [
                        'need' => 1,
                        'now' => 0,
                ...
                break;
            case 3 :
                $member_child_uid = MemberChildren::where('member_id', $member_id)->where('level', '<=', 2)->pluck('child_id')->all();
                $order_amount_query = Order::uniacid()->whereIn('uid', $member_child_uid);
                if ($level_after) {
                    $order_amount_query = $order_amount_query->where('status', '>=', 1);
                } else {
                    $order_amount_query = $order_amount_query->where('status', '=', 3);
                }
                $order_amount = $order_amount_query->sum('price');
                $res['condition'] = [
                    'need' => $next_level->team_performance,
                    'now' => $order_amount,
                    'type_name' => '(业绩)',
                ];
                break;
            case 4 :
                $record = BalanceRechargeRecords::uniacid()
                    ->where(['member_id'=>$member_id,'status'=>1])
                    ->where('money', '>=', $next_level->balance_recharge)
                    ->where('type', '>', 0)
                    ->first();
                $check = 0;
                if ($record) {
                ...
                $res['condition'] = [
                    'need' => $next_level->balance_recharge?:0,
                    'now' => $check,
                    'type_name' => '(充值)',
                ];
                break;
            default :
                $order_amount_query = Order::uniacid()->where('uid', $member_id);
                if ($level_after) {
                    $order_amount_query = $order_amount_query->where('status', '>=', 1);
                } else {
                    $order_amount_query = $order_amount_query->where('status', '=', 3);
    ...
    public function getAssetsData()
    {
        ...
        if (!$asset_type) {
            return $this->errorJson('请传入正确参数asset_type');
        }
        $data = app('MemberCenter')->getAssetsData();
        $res = [];
