...
class QrcodeController extends ApiController
{
    ...
    public function getPoster()
    {
        ...
        $this->memberModel = $memberModel = Member::uniacid()->select('uid', 'avatar', 'nickname')->with('yzMember')->ofUid($this->uid)->first();
        //新海报
        if (\YunShop::plugin()->get('new-poster')) {
            $this->type = intval(request()->type);
            $this->poster_id = intval(request()->poster_id);
            if ($this->poster_id) {
                $this->posterModel = $posterModel = \Yunshop\NewPoster\models\Poster::uniacid()->where('id',$this->poster_id)->first();
            } else {
                $this->posterModel = $posterModel = \Yunshop\NewPoster\models\Poster::uniacid()->whereRaw('FIND_IN_SET('.$this->type.',center_show)')->first();
            }
            if (!$this->posterModel) {
                //默认二维码
        ...
        $image_url = $this->createPoster();
        if ($image_url) {
            return $this->successJson('ok', ['image_url' => $image_url,'center_show'=> '0']);
        } else {
