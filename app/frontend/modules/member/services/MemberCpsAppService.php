...
class MemberCpsAppService extends MemberService
{
    ...
    public function login()
    {
        ...
        $yz_token = \YunShop::request()->yz_token;
        $this->uniacid  = \YunShop::app()->uniacid;
        $uniacid = \YunShop::app()->uniacid;

        $page = \YunShop::request()->page ?: 0;
        $is_bird = \Setting::get('plugin.aggregation-cps.is_bird');
        $data = [
            'first_login' => 0,
            'is_bird' => $is_bird || !isset($is_bird) ? 1 : 0,
        ];
        if (\Request::isMethod('post')) {
            ...
            //token登录
            if ($yz_token) {
                $member_shop_info = MemberShopInfo::where('access_token_2',$yz_token)->first();
                if (!empty($member_shop_info)) {
                    $member_info = MemberModel::getMemberById($member_shop_info->member_id);
                    if (empty($member_info)) {
                        return show_json(7, '用户不存在1');
                    }
                    $member_info = $member_info->toArray();
                } else {
                    return show_json(7, '用户不存在');
            ...
            if (!empty($member_info)) {
                ...
                if (!empty($yz_member)) {
                    ...
                    if (((!empty($password) || !empty($code)) && !$this->secondAuth($page, $mobile)) || $weixin_code || $yz_token) {
    ...
    public function register($data)
    {
        ...
        event(new RegisterByMobile($array['memberModel']));
