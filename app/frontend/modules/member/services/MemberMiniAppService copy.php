...
class MemberMiniAppService extends MemberService
{
    ...
    {
		\Log::debug("--小程序登录请求参数--", [$_REQUEST['login_param']]);
    ...
    public function checkLogged($login = null)
    {
		if (MemberService::isLogged()) {
            ...
            if ($min_set['auth'] && $min_set['close_silent_login']) {
                return true;
            }
    ...
    public function newLogin()
    {
        ...
        $phone_code = \YunShop::request()->phone_code;
        
        $invite_code = \YunShop::request()->invite_code;
        \Log::debug("=====mini====invite_code",$invite_code);
        $invite_info = InvitationCode::where('code',$invite_code)->first();
        if($invite_info->is_limit == 1 && $invite_info->status == 1){
            if($invite_info->used_count>$invite_info->max_uses || $invite_info->expires_at < time()){
                return show_json(0, '邀请码已过期或者已超过绑定账户人数');
            }
        }
        $phoneData = $this->getPhoneNumber($phone_code);

        $mobile = $phoneData['phone_info']['phoneNumber'];

        \Log::debug("=====mini====code",$code);

        \Log::debug("=====mini====phone_code",$phone_code);

        \Log::debug("=====mini====mobile",$mobile);
        $data = array(
            'appid' => $min_set['key'],
            'secret' => $min_set['secret'],
            'js_code' => $code,
        ...
        if (!empty($user_info) && 0 == $user_info['errcode']) {
            ...
            try {
                ...
                if($mobile){
                    \app\common\models\Member::where('uid',$member_id)->update(['mobile'=>$mobile]);
                }
            } catch (ShopException $exception) {
                return show_json(0, $exception->getMessage());
            }
            ...
            if($invite_info->is_limit == 1){
                InvitationCode::where('code', $invite_code)
                    ->where('used_count', '<', $invite_info->max_uses) // 确保不会超过最大使用次数
                    ->increment('used_count'); // 原子操作，used_count 自增1
            }
            return show_json(1, $result, $result);
        } else {
            return show_json(0, '获取用户信息失败');
    ...
    public function mobileLogin()
    {
        ...
        $code = \YunShop::request()->code;
        if ($password) {
            return (new MemberMobileService())->login();
        } elseif ($code) {
            return (new SmsCodeService())->login();
        }
    }
    ...
    public function getIsBindAccount($code)
    {
        $min_set = \Setting::get('plugin.min_app');
        ...
        if (!empty($user_info) && 0 == $user_info['errcode']) {
            ...
            $memberUnionid = MemberUniqueModel::uniacid()->where('unionid',$unionid)->first();

            if($memberUnionid){

                return ['code'=>1,'message'=>'已绑定账户'];

            }else{
                return ['code'=>0,'message'=>'未绑定账户'];
            }
        }
    }
}
