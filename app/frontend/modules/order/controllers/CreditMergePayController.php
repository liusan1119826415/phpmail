...
class CreditMergePayController extends MergePayController
{
    ...
    public function credit2()
    {
         ...
         */
        $orderPay = OrderPay::lockForUpdate()->find(request()->input('order_pay_id'));
        ...
        if (request()->input('type') == 2) {
            $redirect = '/mircoApp/order_pay_success/index/index?pay='.request()->input('order_pay_id');
        } else {
            $redirect = Url::absoluteApp('orderPaySuccess',['pay'=> request()->input('order_pay_id')]);
        }
