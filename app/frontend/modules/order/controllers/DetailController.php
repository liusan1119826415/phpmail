...
class DetailController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $order->hasManyOrderGoods->each(function ($orderGoods) {
            ...
            $orderGoods->points = $this->setGoodPoints($goodModel, $orderGoods->price, $orderGoods->goods_cost_price,$orderGoods->goods_option_id);
        ...
        $invoice->invoice = empty($invoice->invoice) ? 0 : 1;
        ...
        //扫码点餐
        if (!is_null(\app\common\modules\shop\ShopConfig::current()->get('scan_code_meals_detail'))) {
            if (\Setting::get('scan-code-meals.is_open') == 1) {
                $class = array_get(
                    \app\common\modules\shop\ShopConfig::current()->get('scan_code_meals_detail'),
                    'class'
                );
                $function = array_get(
                    \app\common\modules\shop\ShopConfig::current()->get('scan_code_meals_detail'),
                ...
                $table_name = $class::$function($orderId);
...
//            $setCredit = \Setting::get('shop.shop.credit');
     ...
     * @return mixed|string
     */
    protected function setGoodPoints($goodsModel, $goods_market_price, $goods_cost_price,$option_id = 0)
    {
        $points = '';
        ...
        return $tradeGoodsPointsServer->getPoint($points, $goods_market_price, $goods_cost_price, $goodsModel, $option_id);
    }
