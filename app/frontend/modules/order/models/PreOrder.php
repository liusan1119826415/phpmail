...
use app\frontend\modules\order\OrderDiscountAfterDeduction;
...
class PreOrder extends Order
{
    ...
    /**
     * @var OrderDiscountAfterDeduction 抵扣后优惠类
     */
    protected $discountAfterDeduction;
    ...
    public function _getPriceNodes()
    {
        ...
        // 订单抵扣后优惠的节点
        $discountAfterDeductionNodes = $this->getDiscountsAfterDeduction()->map(function (BaseDiscount $discount) {
            $this->discountWeight++;
            return new OrderDiscountPriceNode($this, $discount, 9200 + $this->discountWeight);
        });
        ...
        $nodes = $nodes
            ...
            ->merge($discountAfterDeductionNodes)
...
//        $nodes->map(function ($a) {dump($a->getKey());});dd($nodes);
    ...
    public function getDiscountAfterDeduction()
    {
        if (!isset($this->discountAfterDeduction)) {
            $this->discountAfterDeduction = new OrderDiscountAfterDeduction($this);
        }
        return $this->discountAfterDeduction;
    }
...
//    }
    ...
    protected function initAttributes()
    {
        ...
        $shop_info = $this->getShopName();
        $supplier_id = Goods::where('id',$this->orderGoods->first()->goods->id)->value('supp_id');
        $attributes = array(
            'price' => $this->getPrice()*$trade['deposit_ratio']/100 ,//订单最终支付价格
            ...
            'discount_price' => $this->getDiscountAmount() + $this->getDiscountAfterDeductionAmount(),//订单优惠金额
            ...
            'prepaid_freight'=>0,//$this->getPrepaidFreight(),
            'prepaid_install_fee'=>0,//$this->getPrepaidInstallFee(),
          ...
          //  'note' => $this->getParams('note'),//订单备注
            'note'=>$this->getRequest()->input('note'),
            'shop_name' => $shop_info[0],// 店铺名称
            ...
            'project_id'=>$this->getRequest()->input('project_id'),   //项目id
            'status'=>\app\common\models\Order::WAIT_CONFIRM,//待确认图纸
            'supp_id'=>$supplier_id,
            'goods_volume'=>$this->getTotalVolume(),
            'contact_info'=>$shop_info[1],
            'delivery_address'=>$shop_info[2],
            'service_link' => ServiceUser::getDistributeService($supplier_id),
        );
    ...
    protected function getDiscountsAfterDeduction()
    {
        return $this->getDiscountAfterDeduction()->getDiscounts();
    }
     ...
     * @return Collection
     */
    protected function getDiscountAfterDeductionAmount()
    {
        return $this->getDiscountAfterDeduction()->getAmount();
    }
