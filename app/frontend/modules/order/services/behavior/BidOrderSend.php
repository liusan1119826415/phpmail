...
class BidOrderSend extends ChangeStatusOperation
{
    ...
    protected function updateTable()
    {
        ...
        if (isset($data['express_code'])) {
            ...
            $db_express_model->is_encryption = 0;
     ...
     * @return bool|void
     */
    protected function updateTableOld()
    {
        $data = $this->params ? $this->params : request()->input();
        ...
        if (isset($data['express_code'])) {
            ...
            //实体订单
            $db_express_model = Express::where('order_id',  $data['order_id'])->first();

            !$db_express_model && $db_express_model = new Express();

            $db_express_model->order_id =  $data['order_id'];

            $db_express_model->express_code = $data['express_code'] ?: '';

            $db_express_model->is_encryption = 0;
            if ($db_express_model->express_code == 'SF') {
                $db_express_model->is_encryption = $data['is_encryption'];
            }
