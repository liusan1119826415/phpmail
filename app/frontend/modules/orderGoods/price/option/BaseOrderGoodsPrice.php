...
use app\frontend\modules\orderGoods\OrderGoodsDiscountAfterDeductionPriceNode;
...
//        }
        ...
        // 订单优惠的节点
        $discountAfterDeductionNodes = $this->orderGoods->getDiscountsAfterDeduction()->map(function (BaseDiscountAfterDeduction $discount) {
            return new OrderGoodsDiscountAfterDeductionPriceNode($this, $discount);
        });
            ...
            if (!$this->orderGoods->goods->hasOneSale->has_all_point_deduct) {
                ...
                if(empty($orderCoinExchange)){
                    ...
                    $preOrderGoodsDiscount = new PreOrderGoodsDiscount([
                        ...
                        'name' => $lang['credit1'] ? $lang['credit1'] . '全额抵扣' : '积分全额抵扣',
                    ]);
                    ...
                    $orderCoinExchange = new PreOrderCoinExchange([
                        ...
                        'amount' => $this->getGoodsPrice() ?: 0,
                        'coin' => $this->orderGoods->goods->hasOneSale->all_point_deduct * $this->orderGoods->total,
                        'name' => $lang['credit1'] ? $lang['credit1'] . '全额' : '积分全额',
