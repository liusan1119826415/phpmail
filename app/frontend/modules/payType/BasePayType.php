...
//
//        }
        $order_type = $this->orderPay->orders->first()->order_type;
        if($order_type == 1){
            $goods_title = $this->orderPay->orders->first()->orderMainGoods[0]->pay_title;
        }elseif($order_type == 2){
            $goods_title = "标书制作费订单";
        }elseif($order_type == 3){
            $goods_title = "品牌使用费订单";
        ...
        return [
            ...
            'amount' => $this->orderPay->amount,//bcadd($this->orderPay->orders->sum('price'), 0, 2),
