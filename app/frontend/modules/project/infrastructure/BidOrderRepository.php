...
class BidOrderRepository extends BaseRepository implements BidOrderRepositoryInterface
{
       ...
       public function getList(array $search): array
       {
          ...
          $query = app('OrderManager')->make('Order')->uniacid()->select("*")->with(['projectBid'=>function($query){
           ...
           if (isset($search['status']) && ($search['status'] !== '' || $search['status'] === 0)) {
              ...
              /* if ($search['status'] !== "all" && $search['status'] != 2) {
                   $query->where('status', $search['status']);
           ...
           $data->transform(function ($item) use ($addresses) {
               ...
               $item->bid_type_name = $item->projectBid->bid_type == 1?"工厂直投":"授权投标";
    ...
    public function getAfterSales(array $search)
    {
       ...
       public function getApplyRefund(int $id): array
       {
           ...
           $orderDetail->projectBid->bid_type_name = $orderDetail->projectBid->bid_type == 1?"工厂直投":"授权投标";
           $orderDetail->reason = [
             "协商一致退款",
             "订单信息拍错（规格/尺寸/颜色等）",
             "不想要了",
       ...
       public function getDetail(int $id):array
       {
           ...
           $data['bid_type_name'] = $order_detail->projectBid->bid_type == 1?"授权投标":"工厂直投";
           ...
           if($order_detail->order_type == 2){
               $data['order_name'] = "标书制作费";
           }elseif($order_detail->order_type == 3){
               $data['order_name'] = " 品牌使用费";
           }elseif($order_detail->order_type == 4){
               $data['order_name'] = " 项目保证金";
           }
