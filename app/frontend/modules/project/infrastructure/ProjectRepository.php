...
class ProjectRepository extends BaseRepository implements ProjectRepositoryInterface
{
        ...
        if (!empty($requestData['floors'])) {
            foreach ($requestData['floors'] as &$floor) {
                if ($floor['imgBase64'] ?? null) {
                    $floor['thumb'] = self::uploadOssThumb($floor['imgBase64']);
                }
            }
        }
                            ...
                            if ($space['goods']) {
                                ...
                                foreach ($space['goods'] as $good) {
                                    $goods_data[] = [
                                        ...
                                        'space_id' => $space_model->id,  //空间id
                                        'floor_id' => $floorModel->id,  //楼层id
                                        'goods_id' => $good['goods_id'],
                                        'total' => $good['num'],
                                        'option_id' => $good['option_id'] ?: 0,
                                    ];
                                    //$this->addGoodsSpace($goods_data);
                                }
    ...
    public function editProject(array $data): bool
    {
        ...
        $existing = Project::where('name', $requestData['name'])->where('member_id',\YunShop::app()->getMemberId())
            ->where('id', '!=', $requestData['id'])  // 排除当前项目
            ->exists();
    ...
    public function updateFloorCad(array $requestData): bool
    {
        ...
        if ($requestData['imgBase64'] && $requestData['goods_total']) {
            $thumb = self::uploadOssThumb($requestData['imgBase64']);
            $floor->thumb = $thumb;
            $floor->goods_total = $requestData['goods_total'];
            $floor->save();
        }
        return $this->transaction(function () use ($requestData,$floor) {
            ...
            if (!empty($requestData['spaces'])) {
                ...
                foreach ($requestData['spaces'] as $k => $space) {
                    ...
                    if ($space['goods']) {
                        ...
                        foreach ($space['goods'] as $good) {
                            $goods_data[] = [
                                'project_id' => $floor->project_id,  //项目id
                                'space_id' => $space_model->id,  //空间id
                                'floor_id' => $floor->id,  //楼层id
                                'goods_id' => $good['goods_id'],
                                'total' => $good['num'],
                                'option_id' => $good['option_id'] ?: 0,
                            ];
    ...
    protected function deleteSpace($floor_id)
    {
    ...
    {
        $member_id = \YunShop::app()->getMemberId();
        \Log::debug("===request===",['project_id'=>$project_id,'floor_id'=>$floor_id,'space_id'=>$space_id,'member_id'=>$member_id]);
...
//            }
    ...
    public function delete(int $id): bool
    {
        ...
        try {
            $Project = Project::withTrashed()->find($id); // 查找包含已软删除的数据
    ...
    public function getList(array $search): array
    {
        ...
        $query = Project::select("id", "name", "price","activate", "status", "report_status", "contact_name", "order_status","order_id", "created_at", "updated_at")->where('member_id', $member_id);
        // 按项目名称模糊搜索
    ...
    ')
        ...
        // 给每条数据添加 cart_count 字段
        $data->transform(function ($item) use ($memberCarts,$orderGoods) {
            $item->no_goods = $memberCarts[$item->id]?1:0;
    ...
    {
        // 检查是否已经是 "_副本" 或 "_副本X" 结尾
        if (preg_match('/_副本(\d+)?$/', $name, $matches)) {
            $baseName = $name; // 如果已有“副本”，直接使用原来的
        } else {
            $baseName = $name . '_副本'; // 如果没有，则添加 _副本
        }
    ...
    public function copyProject(int $id): bool
    {
        ...
        return $this->transaction(function () use ($originalProject) {
            // 创建新项目
            $newProjectData = $originalProject->toArray();
            unset($newProjectData['id'],$newProjectData['created_at'],$newProjectData['updated_at'],$newProjectData['deleted_at']); // 去掉原 ID，避免冲突
            $newProjectData['name'] = $this->generateUniqueProjectName($newProjectData['name']); // 给项目名称添加副本标识
            ...
            $newProject = $this->save($newProjectData); // 创建新项目

            // 复制楼层及其空间和商品
            foreach ($originalProject->floors as $originalFloor) {
                $newFloorData = $originalFloor->toArray();
                unset($newFloorData['id'], $newFloorData['project_id']); // 去掉原 ID
                $newFloorData['project_id'] = $newProject->id;

                $newFloor = Floors::create($newFloorData); // 创建新楼层
                ...
                foreach ($originalFloor->spaces as $originalSpace) {
                    ...
                    $newSpace = FloorSpace::create($newSpaceData); // 创建新空间
    ...
    {
        $projectDetail = Project::select("id", "province_id","activate", "city_id", "district_id", "name", "price", "report_status", "bid_status", "factory_status", "order_status", "order_id","status", "address_detail", "contact_name", "phone")->with(['floors' => function ($query) {
...
//            ->where(app('CartContainer')->make('MemberCart')->getTable() . '.project_id', $projectDetail->id)
//            ->orderBy(app('CartContainer')->make('MemberCart')->getTable() . '.created_at', 'desc')
//            ->get();

//        $goods_data = $this->handleGoods($cartList);
//        $projectDetail->goods_data = $goods_data;
        //平面图
        ...
        //工厂考察信息
        $factory = $this->getFactory($projectDetail);
        $projectDetail->factory_inspection = $factory;
    ...
    protected  function getProjectStatusText($project)
    {
    ...
    protected function getFactory($projectDetail)
    {
        ...
        $selected_names = array_column(
            array_filter(self::projects_visit, fn($item) => in_array($item['id'], $selected_ids)),
            'name'
        );
        ...
        if ($factory->need_hotel_booking == 1) {
            $method = collect(self::payment_method)->firstWhere('id', $factory->payment_method);
    ...
    protected function getBidDetail($projectDetail)
    {
        ...
        $relatedCosts = [
            ...
            'bid_fee_status' => $ProjectBid->bid_order_status, //0 未支付  1 已支付
            'brand_fee' => $SupplierCharge->brand_fee ?: 0,
            'brand_fee_status' => $ProjectBid->brand_order_status, //0 未支付  1 已支付
            'project_fee' => $project_fee,
            'project_fee_status' => $ProjectBid->project_order_status,
            'unpaid_amount' => $unpaid_amount, //待付款金额
        ...
        $detail = [
            ...
            'approved_time' => $ProjectBid->approved_time,
    ...
    protected function getReportDetail($projectDetail)
    {
    ...
    protected function handleGoods($cartList)
    {
        if ($cartList) {
            ...
            return ['goods' => $newList, 'total_num' => $total_num, 'total_price' => $this->truncate_to_two_decimals($total_price)];
        }
    ...
    protected function addGoodsSpaceBatch(array $list)
    {
        ...
        $optionIds = array_filter(array_unique($optionIds)); // 去重并过滤空值

        // 一次性查询所有GoodsOptionModel数据
        $options = GoodsOptionModel::whereIn('option_id', $optionIds)
            ->select(['id', 'default_color','option_id'])
            ->get()
            ->groupBy('option_id') // 按id分组，处理一个option_id对应多条记录的情况
            ->toArray();
    ...
    public function savePlan(): bool
    {
        ...
        try {
            if ($data['floors']) {
                $floorIds = collect($data['floors'])->pluck('floor_id'); // 提取 floor_id
    ...
    public function getProjectData(int $space_id): array
    {
        ...
        $data = [];
        $data['activate_project'] = $this->getActiveProject();
