...
class ApplyController extends ApiController
{
    ...
    public function index()
    {
        ...
        //提现前事件（多条）
        event(new BeforeWithdrawManyApplyEvent($amount, $pay_way, $poundage, $withdraw_data, $this->getMemberId()));
        //插入提现
    ...
    private function validateWithdrawDeductionLove()
    {
        ...
        if (app('plugins')->isEnabled('love') && \Setting::get('love.withdraw_deduction_status')) {
            ...
            foreach ($this->withdraw_data as $income) {
                if (in_array($income['type'], \Setting::get('love.withdraw_deduction_dividend_code'),true)) {
                    $amount = bcsub($amount, $income['income']);
                }
            }


            $withdraw_deduction_love_rate = \Setting::get('love.withdraw_deduction_love_rate');
            $deductionLove = proportionMath($amount, $withdraw_deduction_love_rate);//需要扣除的爱心值
    ...
    private function cashLimitation()
    {
        ...
        if ($this->pay_way == 'wechat') {
            ...
            if (($number_of_submissions + $today_withdraw_count) > $wechat_frequency) {
                \Log::debug(
                    '提现到微信失败',
                    ['今天提现次数', $today_withdraw_count, '本次提现次数', $number_of_submissions, '每日限制次数', $wechat_frequency]
                );
            ...
            if (($number_of_submissions + $today_withdraw_count) > $alipay_frequency) {
                \Log::debug(
                    '提现到支付宝失败',
                    ['今天提现次数', $today_withdraw_count, '本次提现次数', $number_of_submissions, '每日限制次数', $alipay_frequency]
                );
    ...
    private function withdrawalLimitation()
    {
        if (app('plugins')->isEnabled('withdrawal-limit')) {
            ...
            if ($mark) {
                $memberModel = MemberWithdrawalLimit::uniacid()->where(
                    'member_id',
    ...
    private function _withdrawStart()
    {
        ...
        foreach ($this->withdraw_data as $key => $item) {
            ...
            //判断收入是否已提现
            $apply_count = WithdrawIncomeApply::whereIn(
                'income_id',
                array_filter(explode(',', $item['type_id']))
            )->whereIn('status', [0, 1, -1])->lockForUpdate()->count();
            if ($apply_count > 0) {
            ...
            //插入提现收入扣除爱心值记录
            if (!empty($this->need_deduction_love_data) && !WithdrawIncomeDeductionService::insert(
                    $withdrawModel,
                    $this->need_deduction_love_data,
                    proportionMath($withdrawModel->amounts, $this->need_deduction_love_data['rate'])
                )) {
            ...
            app('plugins')->isEnabled(
                'converge_pay'
            ) && $this->withdraw_set['free_audit'] == 1 && $this->pay_way == 'converge_pay' ? \Setting::set(
            ...
            if (isset($merge_servicetax_withdraw_id)
                ...
                $this_servicetax_amount = !$this->withdraw_set['service_tax_calculation'] ? bcsub(
                    $withdrawModel->amounts,
                    $withdrawModel->poundage,
                    2
                ) : $withdrawModel->amounts;
                if (bccomp($this_servicetax_amount, 0, 2) != 1) {
                    $this_servicetax_amount = 0;
                }
    ...
    private function validateData()
    {
        ...
        foreach ($this->withdraw_data as &$withdraw) {
            ...
            if ($withdraw['type'] == 'Yunshop\Commission\models\CommissionOrder') {
                $set = Setting::get('withdraw.commission');
                if ($set['max_roll_out_limit'] && $set['max_roll_out_limit'] > 0) {
                    $incomes = Income::where('member_id', $member_id)->whereIn('id', $income_ids)->orderBy(
                        'amount',
                        'desc'
                    )->get()->toArray();
