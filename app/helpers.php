...
if(!function_exists('generateLogisticsNumber')){
    function generateLogisticsNumber()
    {
        $prefix = 'WL';
        $timestamp = now()->format('YmdHis'); // 20250414123045
        $random = mt_rand(1000, 9999); // 4位随机数

        return $prefix . $timestamp . $random;
    }
}

if(!function_exists('processDwg')) {

    function processDwg($api_method, $params)
    {
...
function uploadOss($save_path, $file_name,$is_hight=0)
{
    ...
    $uploadService = new UploadService($is_hight);
    $upload_res = $uploadService->upload($uploadedFile, "file", "files");

    return $upload_res;
}

function uploadOssV2($save_path, $file_name,$is_hight=0,$thumb_type="detail",$upload_type="image")
{
    ...
    $uploadService = new UploadService($is_hight,$thumb_type);
    $upload_res = $uploadService->upload($uploadedFile,$upload_type);
...
if (!function_exists('getNoticeTitle')) {
     ...
     * @return string
     */
    function getNoticeTitle($order_type, $status)
    {
        $templateMap = [
            1 => 'PURCHASE_ORDER',
            2 => 'BIDDING_ORDER',
            3 => 'BRAND_USAGE_ORDER',
            4 => 'PROJECT_DEPOSIT_ORDER',
            5 => 'SERVICE_ORDER',
        ];

        if (!isset($templateMap[$order_type])) {
            return 'order_status_update';
        }

        // 使用完全限定类名
        $fqcn = 'app\common\modules\pcnotice\Template';
        $constantName = $templateMap[$order_type];
        $fullConstantName = $fqcn . '::' . $constantName;

        if (!defined($fullConstantName)) {
            return strtolower($constantName) . '_status_update';
        }

        $template = constant($fullConstantName);
        return $template[$status] ?? strtolower($constantName) . '_status_update';
    }
}
...
if (!function_exists("maskPhoneNumber")) {
    ...
    {
        return substr($phone, 0, 2) . str_repeat('*', 8) . substr($phone, -2);
    }
}
...
function yz_tomedia($src, $local_path = false, $upload_type = null, $host = '')
{
    ...
    $HttpHost = "https://oss.abangmi.com";
    //$HttpHost = "http://192.168.1.123:89";
    if ($host) {
        $HttpHost = $host;
    }
    $setting = [];
    ...
    function file_remote_upload($filename, $auto_delete_local = true, $remote)
    {
        ...
        if ($remote['type'] == '2') {
            ...
            try {
                ...
                \Log::error("====oss上传失败=====",['error_msg'=>$e->getMessage(),'filename'=>$filename]);
                return error(1, $e->getMessage());
            }
            if ($auto_delete_local) {
            ...
            if (!$result) {
                \Log::error("====oss上传失败2===",['result'=>$result,'filename'=>$filename]);
                return error(-1, $result);
            }
            return true;
        } elseif ($remote['type'] == '4') {
...
if (!function_exists('substrCut2')) {
    ...
    function substrCut2($user_name)
    {
        ...
        if ($strlen < 2) {
            ...
            $num = $strlen - 2;
            return $strlen == 2 ? $firstStr . str_repeat('*', mb_strlen($user_name, 'utf-8') - 1) : $firstStr . str_repeat("*", $num > 4 ? 4 : $num) . $lastStr;
        }
    }
}
 ...
 * 导出配置路径
 */
if (!function_exists('plugin_export_path')) {
    function plugin_export_path($plugin_name): string
    {
        return base_path() . '/plugins/' . $plugin_name . '/src/exports/config';
    }
}

if (!function_exists('shop_export_path')) {
    function shop_export_path(): string
    {
        return  base_path() . '/app/exports/config';
    }
}
 ...
 * 插件图片
 */
if (!function_exists('plugin_local_path')) {
    function plugin_local_path($plugins, $icon_name): string
    {
        return base_path() . '/static/local/' . $plugins . '/' . $icon_name;
    }
}
 ...
 * 推广中心入口图标
 */
if (!function_exists('extension_icon_path')) {
    function extension_icon_path($icon_name): string
    {
        return base_path() . '/static/yunshop/designer/extension/' . $icon_name;
    }
}
 ...
 * 支付回调入口文件
 */
if (!function_exists('payment_index')) {
    function payment_index(): string
    {
        return base_path() . '/payment/index.php';
    }
}
 ...
 * 对已存在的密钥进行遮掩，防止密钥泄露
 */
if (!function_exists('secretEncryption')) {
    function secretEncryption($data, $keys)
    {
        foreach ($keys as $key) {
            $data[$key] = $data[$key] ? str_repeat('*', 10) : '';
        }
 ...
 * 验证密钥设置是否更新
 */
if (!function_exists('checkSecret')) {
    function checkSecret($secrte): bool
    {
        $re = '/[*]{10}/';
 ...
 * 将各种时间格式转为时间戳
 */
if (!function_exists('yzExchangeTime')) {
    function yzExchangeTime($time)
    {
        if (!$time) {
            return null;
        }
...
if (!function_exists('redisLock')) {
    function redisLock($key, $second = 3, $value = 1)
    {
