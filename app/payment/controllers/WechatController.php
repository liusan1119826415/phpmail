...
class WechatController extends PaymentController
{
    ...
    public function notifyPc()
    {
        ...
        $pay_order_model = PayOrder::where('out_order_no',$post['out_trade_no'])->where('trade_no',$post['transaction_id'])->where('status',2)->first();
        if($pay_order_model){
            \Log::debug('订单已支付（数据库校验）',$post);
            echo "success";
            return;
        }

        $redisKey = "pay:callback:" . $post['transaction_id'];
        if (!$this->checkDuplicateWithRedis($redisKey)) {
            \Log::debug('订单已处理（Redis防重）', ['transaction_id' => $post['transaction_id']]);
            echo "success";
            return;
