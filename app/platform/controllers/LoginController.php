...
use app\platform\models\LoginUser;
...
class LoginController extends BaseController
{
    ...
    */
    ...
    public function sendLoginResponse(Request $request)
    {
        ...
        $logsParams = [$this->guard()->user()->uid, '超级管理员', 1];

        if ($this->guard()->user()->uid !== 1) {
            $logsParams = [$this->guard()->user()->uid, '其他用户', 2];
    ...
    //添加登录记录
    public static function addLogsAction($logsParams)
    {
        list($userId, $detail_remark, $admin_type) = $logsParams;
        ...
        if ($admin_type == $other) {
            ...
            //其他管理员所属公众号
            $uniacid = DB::table($userTable)->where('uid', $userId)->value('uniacid') ? : 0;

            $uid_arr = ['yz_supplier'];//uid
//            $user_id_arr = ['yz_user_role', 'yz_area_dividend_agent'];//user_id
            $user_id_arr = ['yz_area_dividend_agent'];//user_id
...
//                'yz_user_role' => ['remark' => '操作员', 'user_type' => ''],
            ...
            if ($role = AppUser::uniacid()->where('uid', $userId)->value('role')) {
                //更新默认备注
                if ($role == 'manager') {
                    $remark = '管理员';
                } else {
    ...
    {
        $mobile = app(LoginUser::class)->getUserType($userId);
        return $mobile;
    }
}
