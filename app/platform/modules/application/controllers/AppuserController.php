...
class AppuserController extends BaseController
{
	...
	public function delete()
	{
		...
		YzUserRole::where('user_id', $info->uid)->delete();
    ...
    public function checkname()
    {
		...
		foreach ($user as $key => $rows) {
			if ($rows->type == 3) {
				$isExts = app(LoginUser::class)->getUserType($rows->uid);
