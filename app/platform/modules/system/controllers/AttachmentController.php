...
class AttachmentController extends BaseController
{
    ...
    public function obs()
    {
        $obs = request()->obs;
        $key = trim($obs['key']);
        $secret = strexists($obs['secret'], '*') ? $this->remote['obs']['secret'] : trim($obs['secret']);
    ...
    public function sms()
    {
        ...
        if (request()->input()) {
            ...
            if ($data) {
                $sysSetting = SystemSetting::settingLoad('sms', 'system_sms');
                $secretEncryptionKeys = ['password','password2','secret','aly_secret','tx_appkey','elisoft_password'];
                foreach ($secretEncryptionKeys as $key) {
                    $data[$key] = checkSecret($data[$key]) ? $sysSetting[$key] : $data[$key];
                }
