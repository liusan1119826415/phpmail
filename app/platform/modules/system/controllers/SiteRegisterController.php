...
class SiteRegisterController extends BaseController
{
	...
	public function register()
	{
		...
		if ($data['ip'] && preg_match('/\d+\\.\d+\\.\d+\\.\d+/', $data['ip'])) {
			$ip = $data['ip'];
		} else {
			$ip = '0.0.0.0';
		}

		$url = config('auto-update.registerUrl') . "/free/{$data['name']}/{$data['trades']}/{$data['province']['areaname']}/{$data['city']['areaname']}/{$data['area']['areaname']}/{$data['address']}/{$data['mobile']}/{$data['captcha']}/{$ip}";
