...
class UpdateController extends BaseController
{
    ...
    public function verifyCheck()
    {
        ...
        $filter_file = ['composer.json', 'README.md', 'icon.jpg'];
            ...
            if (1 == $ret['result']) {
                ...
                if (!empty($ret['files'])) {
                    foreach ($ret['files'] as $file) {
						...
						if ($file['path'] == 'vendor/composer/installed.json' && md5_file($entry) != $file['md5']) {
				...
				$businessUpgrad = $this->systemUpgrade->checkBusinessVersioin($ret['business_version']);;
                ...
                $result = [
					...
					'businessUpgrad' => $businessUpgrad,
					'businessVersion' => $ret['business_version'],
                    'list' => $frontendUpgrad
                ];
            } else {
    ...
    public function fileDownload()
    {
        ...
        if (!empty($path)) {
			...
			$this->systemUpgrade->downloadLocalImages();
	...
	public function businessDownload()
	{
		$version = request()->input('version');
		$this->systemUpgrade->business($version);
