  ...
  <el-form :rules="rules" :model="form" ref="rform" label-position="left" label-width="110px">
    <div class="h-baseinfo-content" style="margin-left: 15px;background-color: #F8F9FB;padding:20px;">
      ...
      <el-row>
        ...
        <el-col :span="8">
          ...
          <el-form-item label="搭配组合">
            <!-- 选择关联商品 -->
            <el-badge :value="goods_info.good_names.length" :hidden="goods_info.good_names.length==0" type="primary" style="width: 90%;">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid #DCDFE6;width:90%;">
                  ...
                  <div style="position: relative;">
                    ...
                    <el-tooltip content="点击修改PDF页码" placement="top">
                      <span 
                        v-if="form.goods.pdf_page.length > 0" 
                        @click="editPageCount"
                        style="
                          position: absolute; 
                          top: -5px; 
                 ...
                 <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid #DCDFE6;width:126px">
                  <div style="position: relative;">
                    <el-button type="text" 
                       ...
                       <span 
                        v-if="form.goods.atlas?.data?.length > 0" 
                        style="
                          position: absolute; 
                          top: -5px; 
                          right: -1px; 
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid #DCDFE6;width:126px">
                  <div style="position: relative;">
                    <el-button type="text" 
                       ...
                       <span 
                        v-if="form.goods.maintenance_doc?.data?.length > 0" 
                        style="
                          position: absolute; 
                          top: -5px; 
                          right: -1px; 
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid #DCDFE6;width:126px">
                  <div style="position: relative;">
                    <el-button type="text" 
                       ...
                       <span 
                        v-if="form.goods.install_guide?.data?.length > 0" 
                        style="
                          position: absolute; 
                          top: -5px; 
                          right: -1px; 
      ...
      <el-row>
        <el-col :span="12">
          <!-- 上传商品图片 -->
          <el-form-item :label="lang.goods.thumb" required prop="main_url">
            <div style="display:flex;flex-direction:row;flex-wrap:wrap;">
              ...
              <div class="upload-img-box">
                ...
                <div class="upload-input-title">
                  <el-input v-model="form.goods.wiring_diagram_name" ref="wiring_diagram_name" placeholder="请输入图片类型" size="small" style="width: 140px;" />
                </div>
              </div>
              -->

               <div class="upload-img-box">
                <div class="upload-img-boxed" @click="openImagePreview(form.goods.wiring_diagram,'wiring_diagram','走线示意图','wiring_diagram')">
    ...
    <!-- 模型类型 -->
      <el-form-item label="模型类型">
        <el-radio-group v-model="form.goods.productType" @change="generateCombinations" style="margin-top:7px;">
          ...
          <el-radio :label="5">个性化组合
            <el-tooltip effect="dark" content="适用商品：不同单元可任意自由搭配组合的商品" placement="top" :open-delay="200">
              <i class="el-icon-question"></i>
            </el-tooltip>
          </el-radio>
      ...
      <div v-for="(specItem,specItemIndex) in goodsSpecs" :key="'specItem_' + specItemIndex" >
        ...
        <!-- 商品规格值 -->
        <el-form-item :label="lang.goods.spec_value">
          <spec-value-row
      ...
      <div v-for="(specDetail, index) in specDetails" :key="'specDetail_' + index">
        <!-- Tabs 组, 一个规格值组合有多个模型 -->
        <el-tabs v-show="showSelectedSpecDetail(specDetail.spec_item_id)" v-model="specDetail.activeTab">
              ...
              <el-row>
                <el-col :span="14">
                  <el-row :gutter="30">
                    ...
                    <el-col :span="6">
                      <!-- 人数位 -->
                      <el-form-item v-if="form.goods.productType==2 || form.goods.productType==3" label="人数位" required label-width="90px">
                        <el-select v-model="detail.option.singleType" :ref="'singleType_' + index + '_' + tabIndex" @change="updateSingleType(index, tabIndex, detail.option.singleType)" :disabled="[1,2,3].includes(detail.modelType)" placeholder="选择人数位" >
                          <el-option label="单人位" :value="1" />
                          <el-option label="双人位" :value="2" />
                        </el-select>
                      </el-form-item>
                    </el-col>
                  </el-row>
                  ...
                  <el-row :gutter="30">
                    <el-col :span="9">
                      ...
                      <el-form-item :label="lang.goods.upload_2d_model" required label-width="101px">
                        <div style="display:flex;align-items:center;border:1px solid #DCDFE6;margin-left: -1px;">
                          <div style="display:flex;width:calc(100% - 101px);padding-left:9px;color:#C0C4CC;background-color:#fff;
                            ...
                            <div :style="{ maxWidth: detail.option.cad_plan_modelName ? 'calc(100% - 26px)' : '100%' }" class="h-text-overflow">
                              <el-tooltip v-if="detail.option.cad_plan_modelName && detail.option.cad_plan_modelName.length > 12" :content="detail.option.cad_plan_modelName" placement="bottom">
                    ...
                    <el-col :span="9">
                      ...
                      <el-form-item label="上传白底图" required label-width="100px">
                        <div style="display:flex;align-items:center;border:1px solid #DCDFE6;">
                          <div style="display:flex;width:calc(100% - 101px);padding-left:10px;color:#C0C4CC;background-color:#fff;
                            ...
                            <div :style="{ maxWidth: detail.option.thumbName ? 'calc(100% - 26px)' : '100%' }" class="h-text-overflow">
                              <el-tooltip v-if="detail.option.thumbName && detail.option.thumbName.length > 12" :content="detail.option.thumbName" placement="bottom">
                  ...
                  <el-row :gutter="30">
                        ...
                        <div style="display:flex;align-items:center;border:1px solid #DCDFE6;">
                          <div style="display:flex;width:calc(100% - 101px);padding-left:20px;color:#C0C4CC;background-color:#fff;
                            ...
                            <div :style="{ maxWidth: detail.option.d3MaxName ? 'calc(100% - 26px)' : '100%' }" class="h-text-overflow">
                              <el-tooltip v-if="detail.option.d3MaxName && detail.option.d3MaxName.length > 12" :content="detail.option.d3MaxName" placement="bottom">
                    ...
                    <el-col :span="9">
                      <el-button style="width: 100%;" @click="showThree(index,tabIndex,detail)">DIY编辑器</el-button>
                    ...
                    <el-col>
                      <el-form-item label="DIY组件" label-width="100px">
                        <el-tooltip content="点击部件部分，可进入3D模型内页编辑部件颜色" placement="bottom">
                          ...
                          <div @click="showThree(index,tabIndex,detail)" class="h-color-display-wrapper">
                            <div v-for="(mparam, index) in detail.option.model_param" :key="'modelparam_' + index"
                              class="h-color-module">
                  ...
                  <el-row>
                    <el-col :span="24">
                      ...
                      <el-form-item label="材质说明" required label-width="120px">
                        <el-input v-model="detail.option.structure" :ref="'structure_' + index + '_' + tabIndex" :rows="5" type="textarea" style="width: 94%;" placeholder="请输入商品结构、配件及材质说明" />
                      </el-form-item>
                    </el-col>
                  </el-row>
                </el-col>
    ...
    <div>
      ...
      <el-table :data="goods_info.goods_list" style="width: 100%;height:500px;overflow:auto;margin-top:15px;" v-loading="loading">
        ...
        <el-table-column prop="refund_time" label="操作" align="center" width="320">
          <template slot-scope="scope">
            <span v-if="goods_info.good_ids.includes(scope.row.id)">
              已关联
            </span>
            <!-- 否则显示 "关联" 按钮 -->
            <el-button v-else-if="form.goods.id != scope.row.id" @click="chooseGoods(scope.row)">
              关联
            </el-button>
          </template>
        </el-table-column>
  ...
  <el-dialog :visible.sync="imagePreviewVisible" width="90%" top="20px" class="dialog-class" :close-on-click-modal="false">
    ...
    <div class="h-preview">
      <div class="h-preview-img">
        <div class="h-img">
          <el-image :src="currentPreviewImg?.image_link || currentPreviewImg?.thumb_link" fit="contain" >
      ...
      <div class="h-preview-list">
       <draggable v-model="previewImgList" v-bind="dragOptions" direction="horizontal" class="horizontal-list" item-key="thumb_link">
        ...
        <!-- 上传 -->
        <div class="py-10">
          <div class="upload-img-small-boxed" @click="openImageInput(1,img_type,['atlas','maintenance_doc','install_guide'].includes(formFieldName))">
...
<!-- 上传文件组件 -->
  ...
  <three-view :d3Show="d3Show" :d3ModelData="d3ModelData" @save="handleD3Save" @replace="handleD3Close"></three-view>
...
`,
  ...
  data() {
    ...
    return {
      ...
      specDetailSelectedIndex: 0, //当前选中的specDetails的Index
      selectedTopIndex: 0, //选中的顶级规格值索引
      selectedTopSpecValueId: 0, //选中的顶级规格值Id
      ...
      copyFieldOptions: [
        ...
        { label: "DIY模型部件", value: "model_param" },
      ],
  ...
  mounted() {
    ...
    if (
      ...
      this.initSpecifications(this.form.option);
    } else {
      this.addDefaultSpecification();
    }
    //初始化，绑定已关联商品
    if (!this.isArrayEmpty(this.form.goods.related_goods_id)) {
      // 将数组中的字符串转为整数
      const intArray = this.form.goods.related_goods_id.map(Number);
      this.goods_info.good_ids.push(...intArray);
      console.log("=====good_ids====", this.goods_info.good_ids);
    }
  ...
  methods: {
    ...
    addSiblingSpecValue(specIndex, list, depth) {
      ...
      const orderIndex = list.length
        ? Number(list[list.length - 1].order_index) + 1
        : 0;

      list.push(
        this.createNode(specIndex, orderIndex, parentId, parentPathId, level)
      );
    },
    ...
    addChildSpecValue(specIndex, parentNode) {
      ...
      // 顺序下标: 取子级“最后一个”的 orderIndex + 1；若没有子级则为 0
      const children = parentNode.children;
      const orderIndex = children.length
        ? Number(children[children.length - 1].order_index) + 1
        : 0;
    ...
    // parentId:父节点id; parentPathId: “从顶级到父级”的上级id用“_”拼接
    createNode(specIndex, orderIndex, parentId, parentPathId, level = 1) {
    ...
    // 排列组合并生成规格详细信息 isNewSpecGroup:重新生成组合
    generateCombinations(isNewSpecGroup = true, parentPathId = "") {
      ...
      combinations.forEach((combination) => {
        ...
        const modelTypes = this.generateModelTypes(comTitle);
          ...
          this.specDetails.push({
            ...
            modelTypes: modelTypes.map((model) => ({
              ...
              option: {
                ...
                // cost_price: "", // 成本价格
                singleType:
                  this.form?.goods?.productType == 2 ||
                  this.form?.goods?.productType == 3
                    ? 1
                    : null, //1:单人位; 0:双人位 (双模型、四模型才有)
                product_model: "", // 商品型号
                volume: "0.000", // 包装体积
                structure: "", // 材质说明
    ...
    flattenSpecLeafValues(nodes, ancestors = []) {
      ...
      nodes.forEach((node) => {
        const title = String(node.title || "").trim(); // 获取当前节点的标题
        // 递归传递父节点的 id 和 title，同时将当前节点的 id 和 title 作为路径
        const path = title
          ? [...ancestors.map((item) => item.title), title]
          : [...ancestors.map((item) => item.title)]; // 用 title 构建展示路径

        const hasChildren =
          Array.isArray(node.children) && node.children.length > 0; // 判断是否有子节点

        if (hasChildren) {
          // 继续递归子节点，传递父节点的 id 和 title
          res.push(
            ...this.flattenSpecLeafValues(node.children, [
              ...ancestors,
              { id: node.id, title: node.title }, // 将 id 和 title 都传递
            ])
          );
        } else {
          // 叶子节点：仅收集“有标题”的叶子，避免空值参与组合
          if (title) {
            // 递归拼接 id 路径（用 _ 拼接）
            ...
            res.push({
              ...
              title: title, // 叶子自身标题（给 size 解析等逻辑用）
              label: path.join(" / "), // 使用 title 拼接的展示路径
            });
          }
        }
      });
    ...
    removeSpecificationValue(siblings, index, depth) {
      this.$confirm("确定要删除此规格值吗？删除后无法恢复。", "提示", {
        type: "warning",
      })
        .then(() => {
          const valueId = siblings[index].id; //规格值Id
          // 移除规格值
          siblings.splice(index, 1);
          // 移除与该规格值相关的组合
          this.removeCombinationsForSpecValue(valueId, siblings.length);

          // 移除顶级规格值，处理选中索引和id
          if (
            depth === 1 &&
            index === this.selectedTopIndex &&
            siblings.length > 0
          ) {
            this.selectedTopIndex = siblings.length - 1;
            if (this.goodsSpecs.length > 0) {
              const firstSpecItem =
                this.goodsSpecs[0]?.spec_item[this.selectedTopIndex];
              if (firstSpecItem) {
                this.selectedTopSpecValueId = firstSpecItem.id;
              }
            }
          }
        })
    ...
    // 移除与某个规格值相关的组合  valueId:规格值Id; siblingLen:同级数据条数;
    removeCombinationsForSpecValue(valueId, siblingLen) {
      if (siblingLen == 0) {
        // 如果是同级最后一个规格值被移除，不移除，而是更新规格组合
        this.generateCombinations();
      } else {
    ...
    initSpecifications(options) {
      ...
      this.goodsSpecs.push(
        ...options.specs.map((spec) => ({
          ...
          // 这里递归处理规格值
          spec_item: this.deepSpecitemAddInitialTitle(spec.spec_item || []),
        }))
      );
      //商品规格详细信息
      this.specDetails = options.option.map((opt) => {
        ...
        const updatedModelTypes = opt.modelTypes.map((model) => ({
          ...
          option: {
            ...
            d3MaxName: this.extractFileName(model.option.d3MaxUrl), // 提取 文件名
            cad_plan_modelName: this.extractFileName(
              model.option.cad_plan_model
            ), // 提取 cad_plan_modelName
            thumbName: this.extractFileName(model.option.thumb), // 提取 thumbName
          },
        }));
        // 返回更新后的 specDetail 项
        return {
    ...
    deepSpecitemAddInitialTitle(items, ancestorsIds = []) {
      ...
      return items.map((node, index) => {
        ...
        // 仅当 children 为数组时递归；否则保持原值
        next.children = Array.isArray(node.children)
          ? this.deepSpecitemAddInitialTitle(
              node.children,
    ...
    // 显示规格值对应的规格组合
    showSelectedSpecDetail(id) {
      // 如果 顶级规格值Id 是 0，则初始化为 goodsSpecs 中第一个规格值的 id
      if (this.selectedTopSpecValueId === 0 && this.goodsSpecs.length > 0) {
        const firstSpecItem = this.goodsSpecs[0]?.spec_item[0];
        if (firstSpecItem) {
          this.selectedTopSpecValueId = firstSpecItem.id;
        }
      }
      // 查找匹配的规格组合，是否有当前规格组合，有就显示
      const found = this.specDetails.find((d) => {
        const specItemId = String(d.spec_item_id);
        const topSpecId = String(this.selectedTopSpecValueId);
        return (
          (specItemId === topSpecId ||
            specItemId.startsWith(topSpecId + "_")) &&
          specItemId === String(id)
        );
      });

      return found ? true : false;
    },
    // 选中规格值时，联动显示对应的规格详情 node:当前节点; depth:当前规格层级; index:当前选中的索引
    handleSpecValueSelect(node, depth, index) {
      // 设置选中的顶级索引
      if (depth === 1) {
        this.selectedTopIndex = index;
        this.selectedTopSpecValueId = node.id;
      }
      ...
      console.log("selectedPathIds:", this.selectedPathIds);
    },
    ...
    onPasteSpec(index, tabIndex) {
      ...
      // console.log('粘贴后的 option', option);
    },
    ...
    closeGoods(index) {
      this.goods_info.good_ids.splice(index, 1);
      this.goods_info.good_names.splice(index, 1);
    },
    chooseGoods(item) {
      let is_exist = 0;
      this.goods_info.good_ids.some((item1, index) => {
        if (item1 == item.id) {
          is_exist = 1;
      ...
      this.goods_info.good_ids.push(item.id);
      this.goods_info.good_names.push(item.title);
      this.goods_info.is_all_good = false;
    },
    escapeHTML(a) {
      a = "" + a;
      return a
    ...
    beforeFocusSelectSpecDetail(specDetail, index) {
      ...
      //
      const pathIds = String(specDetail.spec_item_id).split("_");
      this.selectedTopSpecValueId = pathIds[0];
      // 顶级索引
      let foundIndex = -1;
      for (const spec of this.goodsSpecs) {
        if (Array.isArray(spec.spec_item)) {
          const i = spec.spec_item.findIndex(
            (n) => String(n.id) === String(this.selectedTopSpecValueId)
          );
          if (i !== -1) {
            foundIndex = i;
            break; // 找到即退出
    ...
    addDefaul() {
      ...
      if (!this.form.goods.wiring_diagram_name) {
        this.$set(this.form.goods, "wiring_diagram_name", "走线示意图");
      }
    },
    // 商品分类回显
    goodsCategoryShow() {
    ...
    validate() {
      ...
      if (
        ...
        let saveGoods = {
          ...
          related_goods_id: this.goods_info.good_ids, //关联商品
          ...
          wiring_diagram_name:
            this.form.goods.wiring_diagram_name || "走线示意图", //走线示意图自定义名称
    ...
    async handleSuccess(res, file) {
      ...
      switch (this.img_type) {
        ...
        case "option_thumb":
          option.thumb = res.data.attachment;
          option.thumbName = res.data.filename;
          option.thumb_url =
            {
              thumb: res.data.attachment,
              main_thumb: res.data.webp_path,
        ...
        case "cad_plan_model":
          ...
          option.cad_plan_modelName = res.data.filename;
          console.log("====end==", option);
          break;

        case "d3model":
          option.d3Max = res.data.attachment;
          option.d3MaxName = res.data.filename;
        ...
        case "install_guide":
          // 处理 install_guide 类型的逻辑

          if (!this.form.goods.install_guide) {
            this.$set(this.form.goods, "install_guide", { isPdf: 0, data: [] });
          }

          // if (this.form.goods.install_guide.isPdf==1) {
          //   this.form.goods.install_guide.data = [];
          //   this.currentPreviewImg = null;
          // }

          this.form.goods.install_guide.isPdf = 0;
