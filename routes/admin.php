...
Route::group(['middleware' => ['auth:admin', 'authAdmin', 'checkPasswordSafe', 'shopBootStrap', 'check']], function () {
    ...
    Route::group(['prefix' => 'system', 'namespace' => 'platform\modules\system\controllers'], function (){
		...
		// 企业管理前端压缩包下载
		Route::any('update/businessDownload', 'UpdateController@businessDownload');
    });
    ...
    Route::group(['prefix' => 'user', 'namespace' => 'platform\modules\user\controllers'], function (){
        ...
        // 用户修改手机号
        Route::post('change_mobile', 'AdminUserController@changeMobile');
        // 用户主动修改密码
    ...
    Route::group(['namespace' => 'platform\modules\application\controllers'], function () {
		...
		//批量删除 加入回收站
		Route::get('application/batch', 'ApplicationController@batchDeleteApps');
		//回收站
...
// Python 客户端批量导入商品 API（无需登录鉴权）
Route::group(['prefix' => 'importGoods', 'namespace' => 'backend\controllers'], function () {
    Route::post('submit', 'BatchImportGoodsController@submit');
    Route::get('progress', 'BatchImportGoodsController@progress');
});

// Python 导入工具 API（文件上传 / 商品入库 / 健康检查）
Route::group(['prefix' => 'importApi', 'namespace' => 'platform\controllers'], function () {
    Route::any('ping', 'ImportApiController@ping');
    Route::any('upload', 'ImportApiController@upload');
    Route::any('importGoods', 'ImportApiController@importGoods');
    Route::any('checkGoods', 'ImportApiController@checkGoods');
});
